...
class PayNotifyCallBack extends WxPayNotify
{
    ...
    public function NotifyProcess($data, &$msg)
    {
        ...
        if (in_array($data['attach'], array('v', 'r', 'pd'))) {
            $order_type = $data['attach'];
        } else {
            $order_pay_info = Model('order')->getOrderPayInfo(array('pay_sn' => $data['out_trade_no']));
            if (empty($order_pay_info)) {
                $order_pay_info = Model('vr_order')->getOrderInfo(array('order_sn' => $data['out_trade_no']));
                if (empty($order_pay_info)) {
                    $order_type = 'pd';
                } else {
                    $order_type = 'v';
        ...
        \Ypk\QueueClient::push('addConsume', array('member_id' => $log_buyer_id, 'member_name' => $log_buyer_name, 'consume_amount' => ncPriceFormat($api_pay_amount), 'consume_time' => TIMESTAMP, 'consume_remark' => $log_desc));
        return true;
    }
}
