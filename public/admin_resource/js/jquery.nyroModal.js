jQuery(function ($, undefined) {
    ...
    var $w = $(window), $d = $(document), $b = $('body'), baseHref = $('base').attr('href'), _nmObj = {
        ...
        size: function () {
            var maxHeight = this.getInternal().fullSize.viewH - this.sizes.hMargin, maxWidth = this.getInternal().fullSize.viewW - this.sizes.wMargin;
        ...
        _getSpaceReposition: function () {
            var outer = this._getOuter($b), ie7 = browser.msie && browser.version < 8 && !(screen.height <= $w.height() + 23);
            return {
        ...
        _extractUrl: function (url) {
            ...
            if (url) {
                var hash = this._getHash(url), hashLoc = this._getHash(window.location.href), curLoc = window.location.href.substring(0, window.location.href.length - hashLoc.length), req = url.substring(0, url.length - hash.length);
                ret.sel = hash;
    ...
    $.nmFilters({
        gallery: {
            is: function (nm) {
                ...
                if (ret) {
                    var rel = nm.opener.attr('rel'), indexSpace = rel.indexOf(' '), gal = indexSpace > 0 ? rel.substr(0, indexSpace) : rel, links = $('[href][rel="' + gal + '"], [href][rel^="' + gal + ' "]');
    ...
    $.nmFilters({
        image: {
            is: function (nm) {
                ...
                var disable_cache = nm.opener.attr('disable-cache');
                if (disable_cache) {
                    url = url + '?v=' + nm.time;
                }
                $('<img />').load(function () {
    ...
    $.nmFilters({
        swf: {
            idCounter: 1, is: function (nm) {
                ...
                var url = nm.store.link.url, cont = '<div><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="' + nm.swfObjectId + '" width="' + nm.sizes.w + '" height="' + nm.sizes.h + '"><param name="movie" value="' + url + '"></param>', tmp = '';
...
jQuery(function ($, undefined) {
    $.nmFilters({
        iframe: {
            is: function (nm) {
                var target = nm.opener.attr('target') || '', rel = nm.opener.attr('rel') || '', opener = nm.opener.get(0);
