...
function update_data(id) {//更新
	var get_text = $.ajax({
		type: "POST",
		url: 'shop/web_channel/code_update',
...
function get_recommend_goods() {//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_recommend_goods_list").load('shop/web_channel/recommend_list?'+$.param({'id':gc_id,'goods_name':goods_name }));
	}
}
...
function select_recommend_goods(goods_id) {//商品选择
	...
	text_append += '<a href="'+SHOP_SITE_URL+'/goods&goods_id='+goods_id+'" target="_blank">';
