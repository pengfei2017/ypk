$(function(){
		...
		$.get('/shop/index/login', function(result){
		    if(result=='0'){
				...
				if($('#tracereply_'+data.txtid).css("display")=='none'){
					//加载评论列表
			        $("#tracereply_"+data.txtid).load('/shop/store_snshome/commenttop&id='+data.txtid);
	...
	$("[nc_type='scomment_del']").live('click',function(){
		...
		showDialog('您确定要删除该信息吗？','confirm', '', function(){
	        ...
	        ajax_get_confirm('','/shop/store_snshome/delcomment&scid='+data_str.scid+'&stid='+data_str.stid);
			return false;
		});
	});
	...
	$("[nc_type='sd_del']").live('click',function(){
        ...
        eval("data_str = "+data_str);
        var url = "/shop/store_snshome/deltrace&id="+data_str.txtid;
		showDialog('您确定要删除该信息吗？','confirm', '', function(){
			ajaxget(url);
			return false;
		});
