...
function selClass($this){
    ...
    $.getJSON('/shop/store_goods_add/ajax_goods_class', {gc_id : data_str.gcid, deep: deep}, function(data) {
        if (data != null) {
            ...
            $.each(data, function(i, n){
                if(parseInt(n.gc_virtual)==1){
                    $('#class_div_' + deep).children('ul').append('<li data-param="{gcid:'
                        + n.gc_id +',deep:'+ deep +',tid:'+ n.type_id +'}">'
                        +'<a class="" href="javascript:void(0)"><i class="icon-double-angle-right"></i>'
                        +n.gc_name+'&nbsp;(翻倍比例:'+n.commis_rate+')'
                        +'</a></li>')
...
$(function(){
    ...
    $('#commListArea').find('span[nctype="staple_name"]').die().live('click',function() {
        ...
        $.getJSON('/shop/store_goods_add/ajax_show_comm?stapleid=' + data_str.stapleid, function(data) {
            if (data.done) {
    ...
    $('#commListArea').find('a[nctype="del-comm-cate"]').die().live('click',function() {
        $this = $(this);
        eval('var data_str = ' + $this.parents('li').attr('data-param'));
        $.getJSON('/shop/store_goods_add/ajax_stapledel?staple_id='+ data_str.stapleid, function(data) {
            if (data.done) {
