...
$(function(){
    ...
    $('#goods_image').fileupload({
        dataType: 'json',
        url: '/shop/store_goods_add/image_upload?upload_type=uploadedfile',
    ...
    $('#add_album').fileupload({
        dataType: 'json',
        url: '/shop/store_goods_add/image_upload',
    ...
    $('a[nctype="meat_submit"]').click(function(){
        ...
        _data.type = 'text';
        _data.value = _c;
        _rs = mDataInsert(_data);nctype="meat_submit"
        if (!_rs) {
            return false;
        }
    ...
    $('a[nctype="mb_add_img"]').click(function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load('/shop/store_album/pic_list?item=mobile');
    });
    // 关闭 图片选择
    ...
    $('div[nctype="mobile_pannel"]').on('click', 'a[nctype="mp_rpl"]', function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load('shop/store_album/pic_list?item=mobile&type=replace');
    });
    // 插图图片
    ...
    $(".gcategory_service_list").change(function () {
        ...
        if ($(this).val() != -1) {
            ...
            if ( is_sending == true ){ //判断是否提交中
                ...
                is_sending=true;
                loadServiceList1(selected_name);
            }
        }
        else {
            $(".g_name").val("");
...
// 计算商品库存
function loadServiceList1(g_name) {
    var SiteUrl = "http://"+window.location.host+"/shop";
    $.post(SiteUrl+"/store_goods_add/load_service_category2",{"g_name":g_name},function (data) {
        if(data==""){
