...
function drop_cart_item(cart_id){
    ...
    showDialog('确认删除吗?', 'confirm', '', function(){
        $.getJSON('/shop/cart/del&cart_id=' + cart_id, function(result){
            if(result.state){
                //删除成功
                if(result.quantity == 0){//判断购物车是否为空
                    ...
                    if (parent_tr.children('tr').length == 2) {//只剩下医生名头和医生合计尾，则全部移除
                        parent_tr.remove();
                    }
...
function change_quantity(cart_id, input){
    ...
    $.getJSON('/shop/cart/update&cart_id=' + cart_id + '&quantity=' + _value, function(result){
