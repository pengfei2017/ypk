...
function loadCompare(isrefresh){
	...
	if(isrefresh == true){
		$("#comparelist").load('/shop/compare/showcompare');
	}
}
//添加对比商品
function addCompare(gid){
	...
	if (gid > 0){
       	$.ajax({
            type:"GET",
            dataType: "json",
            url:'/shop/compare/addcompare?id='+gid,
            async:false,
...
function delCompare(gid,type){	
	$.ajax({
        type:"GET",
        dataType: "json",
        url:'/shop/compare/delcompare?gid='+gid+'&type='+type,
        async:false,
        success: function(data){  
            if(data.done == true) {
            	//将对比按钮置为未对比状态
            	...
            	if (type == 'mini'){
                    ...
                    go('/shop/compare?gids='+data.gid_str);
                }
            }
            $("#lockcompare").val('unlock');//解除加入对比按钮的锁定
        }
...
function initCompare(){
	...
	//根据是否已加入对比，显示不同样式
	$.getJSON('/shop/compare/checkcompare', function(data){
        if(data){
