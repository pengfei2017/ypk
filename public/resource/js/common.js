...
function collect_store(fav_id, jstype, jsobj) {
    $.get('/shop/index/login', function (result) {
        if (result == '0') {
            login_dialog();
        } else {
            var url = '/shop/member_favorite_store/favoritestore';
                ...
                if (data.done) {
                    ...
                    if (jstype == 'store') {
                        $('[nc_store="' + fav_id + '"]').each(function () {
                            $(this).before('<span class="goods-favorite" title="该医生已收藏"><i class="have">&nbsp;</i></span>');
                            $(this).remove();
                        });
                    }
                }
...
function collect_goods(fav_id, jstype, jsobj) {
    $.get('/shop/index/login', function (result) {
        if (result == '0') {
            ...
            var url = '/shop/member_favorite_goods/favoritegoods';
            $.getJSON(url, {'fid': fav_id}, function (data) {
                if (data.done) {
                    showDialog(data.msg, 'succ', '', '', '', '', '', '', '', '', 2);
                    if (jstype == 'count') {
...
function load_cart_information() {
    $.getJSON('/shop/cart/ajax_load?callback=?', function (result) {
...
function drop_topcart_item(cart_id) {
    $.getJSON('/shop/cart/del?cart_id=' + cart_id + '&callback=?', function (result) {
        if (result.state) {
            var obj = $('.head-user-menu .my-cart');
            //删除成功
            if (result.quantity == 0) {
                ...
                if ($('#rtoolbar_cartlist > ul').children().size() != result.quantity) {
                    $("#rtoolbar_cartlist").load('/shop/cart/ajax_load?type=html');
                    return;
                }
            }
        } else {
...
function load_history_information() {
    $.getJSON('/shop/index/viewed_info', function (result) {
...
function weixin_login() {
    ...
    $.getScript("http://res.wx.qq.com/connect/zh_CN/htmledition/js/wxLogin.js", function () {
        var obj = new WxLogin({
            ...
            scope: "snsapi_login",
            redirect_uri: "/member/connect_wx/get_info",
            state: "",
            style: "",
            href: ""
        });
...
(function ($) {
    ...
    $.show_nc_login = function (options) {
        var settings = $.extend({}, {
            action: '/member/login/login?inajax=1',
            hash: '',
            formhash: '',
            anchor: ''
        }, options);
        ...
        if (connect_qq == '1') {
            add_html += '<a href="/member/connect_qq" title="QQ账号登录" class="qq"><i></i></a>';
        }
        if (connect_sn == '1') {
            add_html += '<a href="/member/connect_sina" title="新浪微博账号登录" class="sina"><i></i></a>';
        }
        if (connect_wx == '1') {
            add_html += '<a href="javascript:void(0);" onclick="weixin_login()" title="微信账号登录" class="wx"><i></i></a>';
        }
        add_html += '</span>';
        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="/member/login?inajax=1"></form>').find('form')
            ...
            .append('<dl><dt>验证码</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\'/admin/seccode/makecode?admin=1&amp;hash=' + settings.hash + '&amp;t=\' + Math.random()" name="codeimage" title="看不清，换一张" src="/admin/seccode/makecode?admin=1&amp;hash=' + settings.hash + '" class="fl ml10"><span>不区分大小写</span></dd></dl>')
            .append('<ul><li>›&nbsp;如果您没有登录账号，请先<a class="register" href="/member/login/register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="/member/login/forget_password">忘记密码</a>？，申请找回密码</li></ul>')
...
(function ($) {
    $.fn.membershipCard = function (options) {
        ...
        return this.each(function () {
            ...
            var _uri = '/shop/member_card?callback=?&uid=' + data_str.id + '&from=' + options.type;
            var _dl = '';
            $this.qtip({
                content: {
                    text: 'Loading...',
                        ...
                        success: function (data) {
                            if (data) {
                                ...
                                _dttmp.append('<i class="sex' + data.sex + '"></i>')
                                    .append('<a href="/shop/member_snshome?mid=' + data.id + '" target="_blank">' + data.name + '</a>' + (data.truename != '' ? '(' + data.truename + ')' : ''));
                                //show membergrade
                                ...
                                // avatar
                                $('<dd class="avatar"><a href="/shop/member_snshome?mid=' + data.id + '" target="_blank"><img src="' + data.avatar + '" /></a><dd>')
                                    .appendTo(_dl);
                                // info
                                var _info = '';
                                if (typeof connect !== 'undefined' && connect === 1 && data.follow != 2) {
                                    ...
                                    _info += '<a class="chat ' + class_html + '" title="点击这里给我发消息" href="JavaScript:check_chat_card(' + data.id + ');">' + text_html + '</a>';
                                }
                                if (data.qq != '') {
                                ...
                                if (data.follow != 2) {
                                    ...
                                    $('<div class="send-msg"> <a href="/member/member_message/sendmsg?member_id=' + data.id + '" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
                                }
        ...
        function follow(o) {
            ...
            eval("data_str = " + data_str);
            $.getJSON('/member/member_snsfriend/addfollow?callback=?&mid=' + data_str.mid, function (data) {
                if (data) {
        ...
        function onfollow(o) {
            var data_str = o.parent().attr('data-param');
            eval("data_str = " + data_str);
            $.getJSON('/member/member_snsfriend/delfollow?callback=?&mid=' + data_str.mid, function (data) {
                if (data) {
...
$(function () {
    ...
    if (act == "store_list") {
        $('#search ul.tab li span').eq(0).html('医生');
    ...
    $('#search ul.tab li').eq(1).click(function () {
        $(this).hide();
        if ($(this).find('span').html() == '医生') {
            $('#keyword').attr("placeholder", "请输入您要搜索的医生关键字");
            $('#search ul.tab li span').eq(0).html('医生');
            $('#search ul.tab li span').eq(1).html('商品');
            $('#search_act').attr("value", 'store_list');
        } else {
            ...
            $('#search ul.tab li span').eq(1).html('医生');
