...
function update_data(id) {//更新
	var get_text = $.ajax({
		type: "POST",
		url: '/shop_manager/web_config/code_update',
...
function update_dialog(id) {//初始化数据
	switch (id) {
		...
		case "brand_list":
			...
			$("#show_brand_list").load('/shop_manager/web_config/brand_list');//查询数据
...
function get_recommend_goods() {//查询商品
	...
	if (gc_id>0 || goods_name!='') { //+$.param({'id':gc_id,'goods_name':goods_name })
		$("#show_recommend_goods_list").load('shop_manager/web_config/recommend_list',{'id':gc_id,'goods_name':goods_name });
	}
}
...
function select_recommend_goods(goods_id) {//商品选择
	...
	text_append += '<a href="'+SHOP_SITE_URL+'/goods/index/'+goods_id+'" target="_blank">';
...
function get_recommend_brand() {//查询品牌
	...
	if (brand_name!='' || brand_initial!='') {
		...
		$("#show_brand_list").load('/shop_manager/web_config/brand_list',{'brand_name':brand_name,'brand_initial':brand_initial });
	}
}
...
function slide_adv(pic_id,pic_img) {//更新图片
	...
	if (pic_img!='') {
        ...
        $("#fwin_upload_adv").hide();
        ScreenLocker.unlock(); //隐藏遮罩层
    }
}
...
function get_goods_list(){//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		...
		$("#show_sale_goods_list").load('shop_manager/web_config/goods_list',{'id':gc_id,'goods_order':goods_order,'goods_name':goods_name });
	}
}
...
function select_sale_goods(goods_id){//商品选择
	...
	text_append += '<a href="'+SITEURL+'/goods/index/'+goods_id+'" target="_blank">';
  	text_append += goods_name+'</a>';
	text_append += '</div>';
