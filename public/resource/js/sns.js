	...
	$("#weibobtn_goods").die('click').live("click",function(){
		if($("#sharegoods_form").valid()){
			...
			if(cookienum >= max_recordnum && $("#sg_seccode").css('display') == 'none'){
				//显示验证码
				$("#sg_seccode").show();
				var hash = $("#sg_seccode").find("[name='hash']").val();
				$("#sg_seccode").find("[name='codeimage']").attr('src','index.php?act=seccode&op=makecode&hash='+hash+'&t=' + Math.random());
	...
	$("[nc_type='forwardbtn']").die('click').live('click',function(){
		...
		if($("#forwardform_"+data.txtid).valid()){
			...
			if(cookienum >= max_recordnum && $("#forwardseccode"+data.txtid).css('display') == 'none'){
				...
				var hash = $("#forwardseccode"+data.txtid).find("[name='hash']").val();
				$("#forwardseccode"+data.txtid).find("[name='codeimage']").attr('src','index.php?act=seccode&op=makecode&hash='+hash+'&t=' + Math.random());
