...
$(function(){
	...
	function initPage(){
		...
		$.ajax({
			type:'post',
			url:ApiUrl+"/member_order/order_list&page="+page+"&curpage="+curpage,
			...
			success:function(result){
				...
				var html = template.render('order-list-js_template', data);
				if (reset) {
				    reset = false;
    ...
    function deleteOrderId(order_id) {
        $.ajax({
            type:"post",
            url:ApiUrl+"/member_order/order_delete",
            data:{order_id:order_id,key:key},
            dataType:"json",
            success:function(result){
                if(result.datas && result.datas == 1){
...
function get_footer() {
    if (!footer) {
        footer = true;
        $.ajax({
            url: WapSiteUrl+'/js/js_template/footer.js',
            dataType: "script"
          });
    }
}