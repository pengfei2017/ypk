$(function(){
    ...
    $.getJSON(ApiUrl + '/member_order/order_info',{key:key,order_id:getQueryString('order_id')}, function(result) {
        ...
        $.ajax({
            type: 'post',
            url: ApiUrl + "/member_order/get_current_deliver",
    ...
    function cancelOrderId(order_id) {
        $.ajax({
            type:"post",
            url:ApiUrl+"/member_order/order_cancel",
    ...
    function sureOrderId(order_id) {
        $.ajax({
            type:"post",
            url:ApiUrl+"/member_order/order_receive",
            data:{order_id:order_id,key:key},
            dataType:"json",
            success:function(result){
                if(result.datas && result.datas == 1){
    ...
    function evaluationOrder() {
        ...
        location.href = WapSiteUrl + '/js_template/member/member_evaluation.html?order_id=' + orderId;
        
    }
    // 追加评价
    function evaluationAgainOrder() {
        var orderId = $(this).attr('order_id');
        location.href = WapSiteUrl + '/js_template/member/member_evaluation_again.html?order_id=' + orderId;
    }
    // 全部退款
    ...
    function viewOrderDelivery() {
        var orderId = $(this).attr('order_id');
        location.href = WapSiteUrl + '/js_template/member/order_delivery.html?order_id=' + orderId;
    }
    // 退款
    function goodsRefund() {
        var orderId = $(this).attr('order_id');
        var orderGoodsId = $(this).attr('order_goods_id');
        location.href = WapSiteUrl + '/js_template/member/refund.html?order_id=' + orderId +'&order_goods_id=' + orderGoodsId;
    }
    // 退货
    function goodsReturn() {
        var orderId = $(this).attr('order_id');
        var orderGoodsId = $(this).attr('order_goods_id');
        location.href = WapSiteUrl + '/js_template/member/return.html?order_id=' + orderId +'&order_goods_id=' + orderGoodsId;
    }
});