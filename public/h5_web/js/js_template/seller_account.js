$(function() { 
    ...
    $.ajax({
        type: "post",
        url: ApiUrl + "/seller_store/store_info",
        data: {
            key: a
        },
        dataType: "json",
        success: function(a) {
			if(a.code==400){
					...
					$.sDialog({
							...
							okBtn: true,
							okFn:function(){window.location.href = WapSiteUrl + "/js_template/seller/store_goods_list.html";},
							cancelBtn: false
						});
					return;
			 } 
    ...
    $(".btn").click(function() {
            ...
            $.ajax({
                type: "post",
                url: ApiUrl + "/seller_store/store_edit",
                data: {
                    key: a,
                    store_qq: e,
                    store_ww: r,
                ...
                success: function(a) {
                    if (a&&a.code==200) {
						$.sDialog({
							...
							content: "编辑成功",
							okBtn: true,
							okFn:function(){window.location.href = WapSiteUrl + "/js_template/seller/seller_account.html";},
							cancelBtn: false
						 }); 
                    } else {
						$.sDialog({
