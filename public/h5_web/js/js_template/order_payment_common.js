        ...
        success: function (result) {
            ...
            if (!$.isEmptyObject(result.datas.pay_info.payment_list)) {
                ...
                for (var i = 0; i < result.datas.pay_info.payment_list.length; i++) {
                    ...
                    if (_payment_code == 'alipay_wap' && readytoAliPay) {
            ...
            $('#toPay').click(function () {
                ...
                if (_use_password) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: ApiUrl + '/member_buy/check_pd_pwd',
                        dataType: 'json',
                        ...
                        success: function (result) {
                            ...
                            goToPayment(pay_sn, act == 'member_buy' ? 'pay_new' : (act == 'member_vr_buy' ? 'vr_pay_new' : 'pd_pay_new'));
                        }
                    });
                } else {
                    goToPayment(pay_sn, act == 'member_buy' ? 'pay_new' : (act == 'member_vr_buy' ? 'vr_pay_new' : 'pd_pay_new'));
...
function goToPayment(pay_sn, op) { //虚拟订单请求的是vr_pay_newAction
	$("#coverDiv").css('display','block');
     $("#toPay").attr('disabled','disabled');
     $("#toPay").attr('background-color','#ccc');

     //var postUrl=ApiUrl + '/member_payment/' + op + '&key=' + key + '&pay_sn=' + pay_sn + '&password=' + password + '&rcb_pay=' + rcb_pay + '&pd_pay=' + pd_pay + '&payment_code=' + payment_code;
     //alert(a);
     //$.get(postUrl,null,function (res) {
     ...
     //     location.href = postUrl;
     // },1500);
    location.href = ApiUrl + '/member_payment/' + op + '&key=' + key + '&pay_sn=' + pay_sn + '&password=' + password + '&rcb_pay=' + rcb_pay + '&pd_pay=' + pd_pay + '&payment_code=' + payment_code;
}
