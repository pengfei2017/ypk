...
$(function() {
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + "/store/store_info",
        data: {key: key, store_id: store_id},
        dataType: 'json',
        success: function(result) {
            var data = result.datas;
            //显示页面title
            var title = data.store_info.store_name + ' - 医生首页';
            ...
            $('#store_kefu').click(function(){
                window.location.href = WapSiteUrl+'/js_template/member/chat_info.html?t_id=' + result.datas.store_info.member_id;
            });
            //店主推荐
    ...
    $('#goods_rank_tab').find('a').click(function(){
        ...
        $.ajax({
            type: 'post',
            url: ApiUrl + '/store/store_goods_rank',
    ...
    function getStoreNewGoods(){
        ...
        load_class_newgoods.loadInit({'url':ApiUrl + '/store/store_new_goods','getparam':param,'tmplid':'newgoods_tpl','containerobj':$("#newgoods"),'iIntervalId':true,'resulthandle':'tidyStoreNewGoodsData'});
    }
    //加载医生促销活动
    function getStoreActivity(){
        $.ajax({
            type: 'post',
            url: ApiUrl + "/store/store_promotion",
    ...
    $("#store_voucher").click(function(){
        if (!$("#store_voucher_con").html()) {
            $.ajax({
                type: 'post',
                url: ApiUrl + '/voucher/voucher_tpl_list',
