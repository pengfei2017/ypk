...
//          },
    ...
    $('#refister_mobile_btn').click(function(){
        ...
        check_sms_captcha(mobile, captcha,inviteId);
        return false;
...
function send_sms(mobile, sec_val, sec_key) {
    $.getJSON(ApiUrl+'/connect/get_sms_captcha', {type:1,phone:mobile,sec_val:sec_val,sec_key:sec_key}, function(result){
...
//跳转到设置登录密码的页码
function check_sms_captcha(mobile, captcha,inviteId) {
    $.getJSON(ApiUrl + '/connect/check_sms_captcha', {type:1,phone:mobile,captcha:captcha }, function(result){
        if (!result.datas.error) {
            window.location.href = 'register_mobile_password.html?mobile=' + mobile + '&captcha=' + captcha+'&inviteId='+inviteId;
        } else {
            //loadSeccode();
            errorTipsShow('<p>' + result.datas.error + '<p>');
        }
