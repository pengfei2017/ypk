...
$(function(){
    ...
    $("#header").on('click', '.header-inp', function(){
        location.href = WapSiteUrl + '/js_template/search.html?keyword=' + keyword;
    });
    if (keyword != '') {
    ...
    $('#product_list').on('click', '.goods-store a',function(){
        ...
        $.getJSON(ApiUrl + '/store/store_credit', {store_id:store_id}, function(result){
            var html = '<dl>'
...
function get_list() {
    ...
    $.getJSON(ApiUrl + '/goods/goods_list' + window.location.search.replace('?','&'), param, function(result){
    	if(!result) {
    		result = [];
    		result.datas = [];
...
function search_adv() {
    $.getJSON(ApiUrl + '/index/search_adv', function(result) {
    	...
    	$('#search_submit').click(function(){
    		...
    		window.location.href = WapSiteUrl + '/js_template/product_list.html' + queryString;
    	});
