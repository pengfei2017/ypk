  ...
  function get_detail(goods_id) {
      ...
      $.ajax({
         ...
         success:function(result){
            ...
            if(!data.error){
              ...
              $("#add-cart").click(function (){
                ...
                var quantity = parseInt($(".buy-num").val());
                 if(!key){
                    ...
                    $.ajax({
                       url:ApiUrl+"/member_cart/cart_add",
              ...
              if (data.goods_info.is_virtual == '1') {
                  $("#buy-now").click(function() {
                      ...
                      $.ajax({
                          type:'post',
                          url:ApiUrl+'/member_vr_buy/buy_step1',
                          data:json,
                          dataType:'json',
                          success:function(result){
                              ...
                              if(result.res_msg=="have_buy"){
                                  $.sDialog({
                                      skin:"red",
                                      content:'您今天已经购买此服务，请勿重复购买',
                                      okBtn:false,
                                      cancelBtn:false
                                  });
                                  setTimeout(function () {
                                      location.href = WapSiteUrl+'/js_template/product_detail.html?goods_id='+goods_id;
                                  },2500);
                              }
                              else {
                                  location.href = WapSiteUrl+'/js_template/order/vr_buy_step1.html?goods_id='+goods_id+'&quantity='+buynum;
                              }
                          }
                      });
                  });
                  ...
                  $("#buy-now").click(function (){
                     ...
                     if(!key){
                      ...
                      if (buynum > data.goods_info.goods_storage) {
                        ...
                        $.ajax({
                            type:'post',
                            url:ApiUrl+'/member_buy/buy_step1',
                            data:json,
                            dataType:'json',
                            success:function(result){
                                if (result.datas.error) {
                                    ...
                                    location.href = WapSiteUrl+'/js_template/order/buy_step1.html?goods_id='+goods_id+'&buynum='+buynum;
                                }
                            }
                        });
                     }
            ...
            // 联系客服
            $('.kefu').click(function(){
                window.location.href = WapSiteUrl+'/js_template/member/chat_info.html?goods_id=' + goods_id + '&t_id=' + result.datas.store_info.member_id;
            })
         }
      });
  }
  ...
  $('#product_detail_html').on('click', '#get_area_selected', function(){
      $.areaSelected({
          success : function(data){
              ...
              $.getJSON(ApiUrl + '/goods/calc', {goods_id:goods_id,area_id:area_id},function(result){
...
function virtual() {
    ...
    $.getJSON(ApiUrl + '/goods/store_o2o_addr', {store_id:store_id},function(result){
    	if (!result.datas.error) {
