...
$(function(){
	...
	function initPage(){
		...
		$.ajax({
			type:'post',
			url:ApiUrl+"/member_vr_order/order_list&page="+page+"&curpage="+curpage,
    ...
    $.ajax({
        type:'get',
        url:ApiUrl+"/member_payment/payment_list",
        data:{key:key},
        dataType:'json',
        success:function(result){
            var validPayments = {};
    ...
    //上传病历
    $('#order-list').on('click','.upload-case-history-order',function () {
        var orderId = $(this).attr('order_id'); //订单id
        var store_id = $(this).attr('store_id'); //医生id
        location.href = WapSiteUrl + '/js_template/member/upload_case_history.html?order_id=' + orderId+"&store_id="+store_id;
    });
});
function get_footer() {
    if (!footer) {
