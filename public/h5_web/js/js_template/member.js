$(function () {
    ...
    if (key) {
        $.ajax({
            type: 'post',
            url: ApiUrl + "/member_index",
            data: {key: key},
            dataType: 'json',
            //jsonp:'callback',
            success: function (result) {
                checkLogin(result.login);
                if(result.datas && result.datas.resMsg && result.datas.resMsg=="nostore"){
                    alert("资料未完善或者申请尚在审核中");
                ...
                var html = '<div class="member-info">'
                    ...
                    +result.datas.member_info.member_tree_level_name
                    + '</div>'
                ...
                var html = '<li><a href="predepositlog_list.html"><i class="cc-06"></i><p>余额</p></a></li>'
                ...
                //判断用户身份
                var member_type_id = parseInt(result.datas.member_type_id);
                var member_id=parseInt(result.datas.member_info.member_id);
                if (member_type_id == 1) { //表示是普通客户
                    $('.completeInfo').remove();  //完善资料
                    $('.myTeamDoctor').remove();  //我的团队（医护圈）
                    $('.addService').remove();   //发布服务
                    $('.addChatCard').remove();  //发布聊天卡
                    $('.serviceBuyRecord').remove(); //服务购买记录
                    $('.customOrders').remove(); //产品订单
                    $('.caseHistory').remove(); //移动病历
                }else {
                    $(".completeInfoCustom").remove();
                }
                if(member_id==1){ //表示是系统管理员
                    $('.completeInfo').remove();  //完善资料
                    $('.addService').remove();   //发布服务
                    $('.addChatCard').remove();  //发布聊天卡
                    $('.serviceBuyRecord').remove(); //服务购买记录
