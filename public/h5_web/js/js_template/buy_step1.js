...
$(function() {
    ...
    $('#list-address-valve').click(function(){
        $.ajax({
            type:'post',
            url:ApiUrl+"/member_address/address_list",
            data:{key:key},
            dataType:'json',
            async:false,
        ...
        $.ajax({//提交订单信息
            ...
            success:function(result){
                ...
                $('#container-fcode').find('.submit').click(function(){
                    ...
                    $.ajax({//提交订单信息
                        type:'post',
                        url:ApiUrl+'/member_buy/check_fcode',
                        dataType:'json',
    ...
    $('#add_address_form').find('.btn').click(function(){
        if($.sValid()){
            ...
            $.ajax({
                type:'post',
                url:ApiUrl+"/member_address/address_add",
                data:param,
                dataType:'json',
                success:function(result){
                    if (!result.datas.error) {
    ...
    $('#invoice-need').click(function(){
        ...
        $.ajax({//获取发票内容
            type:'post',
            url:ApiUrl+'/member_invoice/invoice_content_list',
            data:{key:key},
            dataType:'json',
            success:function(result){
                checkLogin(result.login);
        ...
        $.ajax({
            type:'post',
            url:ApiUrl+'/member_invoice/invoice_list',
            data:{key:key},
            dataType:'json',
            success:function(result){
                checkLogin(result.login);
                ...
                $('.del-invoice').click(function(){
                    ...
                    $.ajax({
                        type:'post',
                        url:ApiUrl+'/member_invoice/invoice_del',
    ...
    $('#invoice-div').find('.btn-l').click(function(){
        if ($('#invoice-need').hasClass('sel')) {
            if (invoice_id == 0) {
                ...
                $.ajax({
                    type:'post',
                    url:ApiUrl+'/member_invoice/invoice_add',
                    data:param,
                    dataType:'json',
                    success:function(result){
    ...
    $('#ToBuyStep2').click(function(){
        ...
        $.ajax({
            type:'post',
            url:ApiUrl+'/member_buy/buy_step2',
            data:{
                key:key,
                ifcart:ifcart,
                cart_id:cart_id,
            ...
            success: function(result){
                ...
                if (result.datas.payment_code == 'offline') {
                    window.location.href = WapSiteUrl + '/js_template/member/order_list.html';
                } else {
