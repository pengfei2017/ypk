...
class PredepositController extends ControllerBase
{
    ...
    {
        parent::initialize();
        $this->translation = getTranslation('common,layout,member,predeposit,export');
        $this->view->setVar('lang', $this->translation);
    }

    public function indexAction()
    {
        $this->predepositAction();
    }
...
//        Tpl::showpage('pd.list');
     ...
     * 充值编辑(更改成收到款)
     */
    public function recharge_editAction()
    {
        $id = intval($_GET['id']);
        if ($id <= 0) {
            showMessage(getLang('admin_predeposit_parameter_error'), getUrl('shop_manager/predeposit/predeposit'), '', 'error');
        }
        //查询充值信息
        ...
        if (empty($info)) {
            showMessage(getLang('admin_predeposit_record_error'), getUrl('shop_manager/predeposit/predeposit'), '', 'error');
        }
        if (!chksubmit()) {
            //显示支付接口列表
...
//            Tpl::showpage('pd.edit');
            $this->view->pick('predeposit/pd_edit');
            exit();
        }
        ...
        if (!$payment_info || $payment_info['payment_code'] == 'offline' || $payment_info['payment_code'] == 'offline') {
            showMessage(getLang('payment_index_sys_not_support'), '', 'html', 'error');
        }
        ...
        $log_msg = getLang('admin_predeposit_recharge_edit_state') . ',' . getLang('admin_predeposit_sn') . ':' . $info['pdr_sn'];
        ...
        try {
            ...
            if (!$state) {
                throw new \Exception(getLang('predeposit_payment_pay_fail'));
            }
            //变更会员预存款
            $data = array();
            ...
            showMessage(getLang('admin_predeposit_recharge_edit_success'), getUrl('shop_manager/predeposit/predeposit'));
        } catch (\Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg, 0);
            showMessage($e->getMessage(), getUrl('shop_manager/predeposit/predeposit'), 'html', 'error');
        }
    }
     ...
     * 充值查看
     */
    public function recharge_infoAction()
    {
        $id = intval($_GET['id']);
        if ($id <= 0) {
            showMessage(getLang('admin_predeposit_parameter_error'), getUrl('shop_manager/predeposit/predeposit'), '', 'error');
...
//        Tpl::showpage('pd.info', 'null_layout');
        ...
        $this->view->pick('predeposit/pd_info');
...
//        Tpl::showpage('pd_log.list');
        $this->view->pick('predeposit/pd_log_list');
    }
...
//        Tpl::showpage('pd_cash.list');
        $this->view->pick('predeposit/pd_cash_list');
    }

    /**
     * 删除提现记录
     */
    public function pd_cash_delAction()
    {
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_pd = Model('predeposit');
            ...
            try {
                ...
                if (!$result) {
                    throw new \Exception(getLang('admin_predeposit_cash_del_fail'));
                }
                //退还冻结的预存款
     ...
     * 更改提现为支付状态
     */
    public function pd_cash_payAction()
    {
        $id = intval($_GET['id']);
        if ($id <= 0) {
            showMessage(getLang('admin_predeposit_parameter_error'), getUrl('shop_manager/predeposit/pd_cash_list'), '', 'error');
        }
        $model_pd = Model('predeposit');
        $condition = array();
        $condition['pdc_id'] = $id;
        ...
        if (!is_array($info) || count($info) < 0) {
            showMessage(getLang('admin_predeposit_record_error'), getUrl('shop_manager/predeposit/pd_cash_list'), '', 'error');
        }
        ...
        $log_msg = getLang('admin_predeposit_cash_edit_state') . ',' . getLang('admin_predeposit_cs_sn') . ':' . $info['pdc_sn'];

        try {
            $model_pd->beginTransaction();
            $result = $model_pd->editPdCash($update, $condition);
            if (!$result) {
                throw new \Exception(getLang('admin_predeposit_cash_edit_fail'));
            }
            //扣除冻结的预存款
            $data = array();
            $data['member_id'] = $member_info['member_id'];
            ...
            $model_pd->commit();
            $this->log($log_msg, 1);
            showMessage(getLang('admin_predeposit_cash_edit_success'), getUrl('shop_manager/predeposit/pd_cash_list'));
        } catch (\Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg, 0);
            showMessage($e->getMessage(), getUrl('shop_manager/predeposit/pd_cash_list'), 'html', 'error');
        }
    }
     ...
     * 查看提现信息
     */
    public function pd_cash_viewAction()
    {
        $id = intval($_GET['id']);
        $model_pd = Model('predeposit');
        $condition = array();
...
//        Tpl::showpage('pd_cash.view', 'null_layout');
        $this->view->pick('predeposit/pd_cash_view');
    }
    ...
    public function export_step1Action()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                $this->view->setVar('murl', getUrl('shop_manager/predeposit/predeposit'));
    ...
    private function createExcel($data = array())
    {
        ...
        //header
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_yc_no'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_yc_member'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_yc_ctime'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_yc_ptime'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_yc_pay'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_yc_money'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_yc_paystate'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_yc_memberid'));
        ...
        $excel_obj->generateXLS(getLang('exp_yc_yckcz') . '-' . $curpage_name . date('Y-m-d-H', time()), $excel_data);
    }
     ...
     *
     */
    public function export_cash_step1Action()
    {
        $condition = array();
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                Tpl::output('murl', getUrl('shop_manager/predeposit/pd_cash_list'));
//                Tpl::setDirquna('shop');
//                Tpl::showpage('export.excel');
    ...
    private function createCashExcel($data = array())
    {
        ...
        //header
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_tx_no'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_tx_member'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_tx_money'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_tx_ctime'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_tx_state'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_tx_memberid'));
        foreach ((array)$data as $k => $v) {
            $tmp = array();
        ...
        // $excel_obj->addWorksheet($excel_obj->charset(getLang('exp_tx_title'),CHARSET));
        // $excel_obj->generateXML($excel_obj->charset(getLang('exp_tx_title'),CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
        $curpage_name = '';
        if (!empty($_GET['curpage'])) {
            $curpage_name = $_GET['curpage'] . '-';
        }
        $excel_obj->generateXLS('order-' . $curpage_name . date('Y-m-d-H', time()), $excel_data);
    }

    /**
     * 预存款明细信息导出
     */
    public function export_mx_step1Action()
    {
        $condition = array();
        if ($_GET['id'] != '') {
            $id_array = explode(',', $_GET['id']);
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                Tpl::output('list', $array);
                Tpl::output('murl', getUrl('shop_manager/predeposit/pd_log_list'));
//                Tpl::setDirquna('shop');
//                Tpl::showpage('export.excel');
                $this->view->pick('common/export_excel');
            } else {  //如果数量小，直接下载
...
//        import('libraries.excel');
        ...
        //header
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_mx_member'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_mx_ctime'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_mx_av_money'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_mx_freeze_money'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_mx_system'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('exp_mx_mshu'));
        foreach ((array)$data as $k => $v) {
            $tmp = array();
        ...
        $excel_data = $excel_obj->charset($excel_data, CHARSET);
//        $excel_obj->addArray($excel_data);
//        $excel_obj->addWorksheet($excel_obj->charset(getLang('exp_mx_rz'),CHARSET));
//        $excel_obj->generateXML($excel_obj->charset(getLang('exp_mx_rz'),CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
        $curpage_name = '';
        if (!empty($_GET['curpage'])) {
            $curpage_name = $_GET['curpage'] . '-';
        }
    ...
    public function get_xmlAction()
    {
        ...
        foreach ($recharge_list as $value) {
            ...
            $operation .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('recharge_info','查看充值编号“" . $value['pdr_sn'] . "”的明细','" . getUrl('shop_manager/predeposit/recharge_info', array('id' => $value['pdr_id'])) . "', '640')\"><i class='fa fa-list-alt'></i>查看</a>";
     ...
     * 输出提现XML数据
     */
    public function get_cash_xmlAction()
    {
        $model_pd = Model('predeposit');
        $condition = array();
        $if_start_date = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $_GET['stime']);
        ...
        foreach ($cash_list as $value) {
            ...
            $param['operation'] .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('cash_info','查看提现编号“" . $value['pdc_sn'] . "”的明细', " . getUrl('shop_manger/predeposit/pd_cash_view', array('id' => $value['pdc_id'])) . "', 640)\" ><i class='fa fa-list-alt'></i>查看</a>";
