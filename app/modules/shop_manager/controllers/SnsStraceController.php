...
class SnsStraceController extends ControllerBase
{
    public function initialize(){
        parent::initialize();
        $this->translation = getTranslation('layout,snstrace,sns_strace,common');
        $this->view->setVar('lang', $this->translation);
    }

    public function indexAction() {
        $this->stracelistAction();
    }

    /**
     * 动态列表
     */
    public function stracelistAction(){
//        Tpl::setDirquna('shop');
//        Tpl::showpage('sns_strace.index');
        $this->view->pick('sns_strace/sns_strace');
    }
    ...
    public function get_xmlAction() {
        ...
        foreach ($strace_list as $value) {
            ...
            $operation .= "<li><a href='javascript:void(0);' onclick='ajax_form(\"login\",\"动态内容\",\"". getUrl('shop_manager/sns_strace/strace_info', array('id' => $value['strace_id'])) ."\",640)'>动态内容</a></li>";
            $operation .= "<li><a href='".getUrl('shop_manager/sns_strace/scomm_list',array('st_id'=>$value['strace_id']))."'>查看评论</a></li>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajaxget(\"" . getUrl('shop_manager/sns_strace/strace_edit',array('id'=> $value['strace_id'], 'value' => ($value['strace_state'] == 1 ? 0 : 1))) . "\")'>".($value['strace_state'] == 1 ? '屏蔽动态' : '显示动态')."</a></li>";
            $operation .= "</ul></span>";
     ...
     * 动态详细信息
     */
    public function strace_infoAction() {
        $model_stracelog = Model('store_sns_tracelog');
        $id = $_GET['id'];
...
//        Tpl::showpage('sns_strace.info', 'null_layout');
        ...
        $this->view->pick('sns_strace/strace_info');
    }

    /**
     * 删除动态
     */
    public function strace_delAction(){
        // 验证参数
     ...
     * 编辑动态
     */
    public function strace_editAction(){
        // 验证参数
        $st_id = intval($_GET['id']);
        if($st_id <= 0){
     ...
     * 评论列表
     */
    public function scomm_listAction(){
...
//        Tpl::showpage('sns_scomment.index');
        $this->view->pick('sns_strace/sns_scomment_index');
    }

    /**
     * 输出XML数据
     */
    public function get_scomm_xmlAction() {
        ...
        foreach ($scomm_list as $value) {
            ...
            $operation .= "<span class='btn'><em><i class='fa fa-cog'></i>" . getLang('nc_set') . " <i class='arrow'></i></em><ul>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajax_form(\"login\",\"评论内容\",\"". getUrl('shop_manager/sns_strace/scomm_info', array('id' => $value['scomm_id'])) ."\",640)'>评论内容</a></li>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajaxget(\"" . getUrl('shop_manager/sns_strace/scomm_edit',array('id'=> $value['scomm_id'], 'value' => ($value['scomm_state'] == 1 ? 0 : 1))) . "\")'>".($value['scomm_state'] == 1 ? '屏蔽评论' : '显示评论')."</a></li>";
            $operation .= "</ul></span>";
            $param['operation'] = $operation;
     ...
     * 评论详细
     */
    public function scomm_infoAction() {
        $model_scomment = Model('store_sns_comment');
        $id = $_GET['id'];
...
//        Tpl::showpage('sns_scomment.info', 'null_layout');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->pick('sns_strace/sns_scomment_info');
    }
    /**
     * 删除评论
     */
    public function scomm_delAction(){
        $id = intval($_GET['id']);
        if($id <= 0){
            exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
        }
     ...
     * 评论编辑
     */
    public function scomm_editAction(){
        $id = intval($_GET['id']);
        if($id <= 0){
            showDialog(getLang('nc_common_op_fail'));
        }
