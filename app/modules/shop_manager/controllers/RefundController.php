...
class RefundController extends ControllerBase
{
    const EXPORT_SIZE = 1;
    private $links = array(
        array('url' => array('module' => 'shop_manager', 'controller' => 'refund'), 'text' => '待处理'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'refund', 'action' => 'refund_all'), 'text' => '所有记录'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'refund', 'action' => 'reason'), 'text' => '退款退货原因'),
    );
    ...
    public function initialize()
    {
        ...
        $this->view->setVar('top_link', $this->sublink($this->links, $url_param_array['action']));
    }
    public function indexAction()
    {
//        Tpl::setDirquna('shop');
//        Tpl::showpage('refund_manage.list');
        $this->view->pick('refund/refund_manage_list');
    }

    /**
     * 待处理列表
     */
    public function get_manage_xmlAction()
    {
        $model_refund = Model('refund_return');
        $condition = array();
        ...
        foreach ($refund_list as $k => $refund_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn orange\" href=\"".getUrl('shop_manager/refund/edit',array('refund_id'=>$refund_info['refund_id']))."\"><i class=\"fa fa-gavel\"></i>处理</a>";
            ...
            if ($refund_info['goods_id'] > 0) {
                $list['goods_name'] = "<a class='open' title='{$refund_info['goods_name']}' href='" . getUrl('shop_manager/goods/index', array('goods_id' => $refund_info['goods_id'])) .
...
//        Tpl::showpage('refund_all.list');
        $this->view->pick('refund/refund_all_list');
    }
    ...
    public function get_all_xmlAction()
    {
        ...
        foreach ($refund_list as $k => $refund_info) {
            ...
            $list['operation'] .= "<a class=\"btn green\" href=\"".getUrl('shop_manager/refund/view',array('refund_id'=>$refund_info['refund_id']))."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $list['refund_sn'] = $refund_info['refund_sn'];
            $list['refund_amount'] = ncPriceFormat($refund_info['refund_amount']);
            if (!empty($refund_info['pic_info'])) {
                $info = unserialize($refund_info['pic_info']);
    ...
    public function editAction()
    {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                $param['param'] = array(
                    'refund_url' => getUrl('shop_manager/member_refund/view', array('refund_id' => $refund['refund_id'])),
                    'refund_sn' => $refund['refund_sn']
                );
                QueueClient::push('sendMemberMsg', $param);

                $this->log('退款确认，退款编号' . $refund['refund_sn']);
                $this->showMessage(getLang('nc_common_save_succ'), getUrl('shop_manager/refund/index'));
            } else {
...
//        Tpl::showpage('refund.edit');
        $this->view->pick('refund/refund_edit');
...
//        Tpl::showpage('refund.view');
        $this->view->pick('refund/refund_view');
    }
...
//        Tpl::showpage('refund_reason.list');
        $this->view->pick('refund/refund_reason_list');
    }
     ...
     *
     */
    public function add_reasonAction()
    {
        $model_refund = Model('refund_return');
        if (chksubmit()) {
            $reason_array = array();
            ...
            if ($state) {
                ...
                showMessage(getLang('nc_common_save_succ'), getUrl('shop_manager/refund/reason'));
            } else {
                showMessage(getLang('nc_common_save_fail'));
            }
        }
...
//        Tpl::showpage('refund_reason.add');
        $this->view->pick('refund/refund_reason_add');
    }
     ...
     *
     */
    public function edit_reasonAction()
    {
        $model_refund = Model('refund_return');
        $condition = array();
...
//        Tpl::showpage('refund_reason.edit');
        $this->view->pick('refund/refund_reason_edit');
     ...
     *
     */
    public function del_reasonAction()
    {
        $model_refund = Model('refund_return');
        $condition = array();
        $condition['reason_id'] = intval($_GET['reason_id']);
        ...
        if ($state) {
            ...
            showMessage(getLang('nc_common_del_succ'), getUrl('shop_manager/refund/reason'));
        } else {
    ...
    public function export_step1Action()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                $this->view->setVar('murl', 'javascript:history.back(-1)');
