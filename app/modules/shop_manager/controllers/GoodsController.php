...
{
    private $links = array(
        array('url' => array('module' => 'shop_manager', 'controller' => 'goods', 'action' => 'goods'), 'text' => '所有商品'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'goods', 'action' => 'lockup_list'), 'text' => '违规下架'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'goods', 'action' => 'waitverify_list'), 'text' => '等待审核'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'goods', 'action' => 'goods_set'), 'text' => '商品设置'),
    );
    ...
    {
        parent::initialize();
        $this->translation = getTranslation('common,layout,goods,setting');
    }
    ...
    {
        if(!empty($_REQUEST['type'] && $_REQUEST['type']=="waitverify")){
            $this->waitverify_listAction();
        }else{
            //父类列表，只取到第二级
...
//        Tpl::showpage('goods.index');
            $this->view->pick('goods/goods_index');
        }
    }
     ...
     * 违规下架商品管理
     */
    public function lockup_listAction()
    {
...
//        Tpl::showpage('goods.index');
     ...
     * 等待审核商品管理
     */
    public function waitverify_listAction()
    {
...
//        Tpl::showpage('goods.index');
        ...
        foreach ($goods_list as $value) {
            ...
            switch ($_GET['type']) {
                ...
                default:
                    ...
                    $operation .= "<a class='btn orange' href='javascript:void(0);' onclick=\"fg_lonkup1('" . $value['goods_commonid'] . "')\"><i class='fa fa-ban'></i>添加卖点</a>";
                    break;
            }
            $operation .= "<span class='btn'><em><i class='fa fa-cog'></i>设置 <i class='arrow'></i></em><ul>";
            $operation .= "<li><a href='" . getUrl('shop/goods/index', array('goods_id' => $storage_array[$value['goods_commonid']]['goods_id'])) . "' target=\"_blank\">查看商品详细</a></li>";
     ...
     * 商品添加卖点
    */
    public function goods_hotAction(){
        if (chksubmit()) {
            ...
            $update2 = array();
            $update2['goods_jingle'] =  htmlentities(trim($_POST['close_reason']));
            $where['goods_commonid'] = $commonid;
            $goods = Goods::findFirst('goods_commonid='.$commonid);
            $goodsCommon = GoodsCommon::findFirst('goods_commonid='.$commonid);
            if($goods&&$goodsCommon){
                $goods->save(array('goods_jingle'=>$update2['goods_jingle']));
                $goodsCommon->save(array('goods_jingle'=>$update2['goods_jingle']));
                $goods_array = $goods->toArray();
                $goods_name = $goods_array['goods_name'];
                $goods_id = $goods_array['gc_id'];
                $goodsClass = GoodsClass::findFirst(array('conditions'=>'gc_name='.$goods_name,'conditions'=>'gc_parent_id='.$goods_id));
                $goodsClass->save(array('goods_jingle'=>$update2['goods_jingle']));
            }
            showDialog(getLang('nc_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();CUR_DIALOG.close()');
        }
        $common_info = Model('goods')->getGoodsCommonInfoByID($_GET['id']);
        Tpl::output('common_info', $common_info);
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->pick('goods/goods_hot');
    }
    /**
     * 商品状态
     ...
     * 违规下架
     */
    public function goods_lockupAction()
    {
        if (chksubmit()) {
            $commonid = intval($_POST['commonid']);
            if ($commonid <= 0) {
...
//        Tpl::showpage('goods.close_remark', 'null_layout');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->pick('goods/goods_close_remark');
    }

    /**
     * 删除商品
     */
    public function goods_delAction()
    {
     ...
     * 审核商品
     */
    public function goods_verifyAction()
    {
        if (chksubmit()) {
            $commonid = intval($_POST['commonid']);
            if ($commonid <= 0) {
...
//        Tpl::showpage('goods.verify_remark', 'null_layout');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->pick('goods/goods_verify_remark');
    }

    /**
     * ajax获取商品列表
     */
    public function get_goods_sku_listAction()
    {
        ...
        foreach ($goods_list as $key => $val) {
            ...
            foreach ($goods_spec as $k => $v) {
                $spec_array[] = '<div class="goods_spec">' . $spec_name[$k] . getLang('nc_colon') . '<em title="' . $v . '">' . $v . '</em>' . '</div>';
            }
...
//        Tpl::showpage('goods.sku_list', 'null_layout');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->pick('goods/goods_sku_list');
    }

    /**
     * 商品设置
     */
    public function goods_setAction()
    {
        ...
        if (chksubmit()) {
            ...
            if ($result === true) {
                $this->log(getLang('nc_edit,nc_goods_set'), 1);
                showMessage(getLang('nc_common_save_succ'));
            } else {
                $this->log(getLang('nc_edit,nc_goods_set'), 0);
...
//        Tpl::showpage('goods.setting');
        $this->view->pick('goods/goods_setting');
    }
    ...
    public function export_csvAction()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                Tpl::output('murl', getUrl('shop_manager/goods/index'));
