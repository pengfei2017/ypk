...
class OrderController extends ControllerBase
{
    ...
    {
        parent::initialize();
        $this->translation = getTranslation('common,trade,layout,state');
...
//        Tpl::showpage('order.index');
    ...
    public function get_xmlAction()
    {
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            $list = array();
            $operation_detail = '';
            $list['operation'] = "<a class=\"btn green\" href=\"" . getUrl('shop_manager/order/show_order', array('order_id' => $order_info['order_id'])) . "\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            ...
            if ($order_info['if_system_receive_pay']) {
                ...
                $operation_detail .= "<li><a href=\"" . getUrl('shop_manager/order/change_state', array('state_type' => 'receive_pay', 'order_id' => $order_info['order_id'])) . "\">{$op_name}</a></li>";
            }
            if ($operation_detail) {
    ...
    public function change_stateAction()
    {
        ...
        if ($order_id <= 0) {
            $this->showMessage($this->translation->_('miss_order_number'), $_POST['ref_url'], 'html', 'error');
        }
        $model_order = Model('order');
        ...
        if (!$result['state']) {
            $this->showMessage($result['msg'], $_POST['ref_url'], 'html', 'error');
        } else {
            $this->showMessage($result['msg'], $_POST['ref_url']);
        }
    }
    ...
    private function _order_cancel($order_info)
    {
        ...
        if (!$if_allow) {
            return queue_callback(false, '正在取消订单，请稍候...');
        }
        ...
        if ($result['state']) {
            $this->log(getLang('order_log_cancel') . ',' . getLang('order_number') . ':' . $order_info['order_sn'], 1);
        }
        if ($result['state']) {
...
//            Tpl::showpage('order.receive_pay');
            $this->view->render('order','order_receive_pay');
            exit();
        }
        //预定支付尾款时需要用到已经支付的状态
        ...
        if ($result['state']) {
            $this->log('将订单改为已收款状态,' . $this->translation->_('order_number') . ':' . $order_info['order_sn'], 1);
            //记录消费日志
    ...
    public function show_orderAction()
    {
        $order_id = intval($_GET['order_id']);
        if ($order_id <= 0) {
            $this->showMessage($this->translation->_('miss_order_number'));
        }
        $model_order = Model('order');
        ...
        foreach ($order_info['extend_order_goods'] as $value) {
            ...
            $value['goods_url'] = getUrl('shop_manager/goods/index', array('goods_id' => $value['goods_id']));
        ...
        $this->view->setVar('order_log', $log_list);
        ...
        $this->view->setVar('return_list', $return_list);

        //退款信息
        $refund_list = $model_refund->getRefundList($condition);
        $this->view->setVar('refund_list', $refund_list);
        ...
        if (!empty($order_info['extend_order_common']['daddress_id'])) {
            ...
            $this->view->setVar('daddress_info', $daddress_info);
        }
        ...
        $this->view->setVar('order_info', $order_info);
//        Tpl::setDirquna('shop');
//        Tpl::showpage('order.view');
        $this->view->pick('order/order_view');
    }
    ...
    public function export_step1Action()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                $this->view->setVar('murl', getUrl('shop_manager/order/index'));
    ...
    private function createExcel($data = array())
    {
        ...
        $excel_data[0][] = array('styleid' => 's_title', 'data' => '医生ID');
        ...
        $excel_obj->generateXLS('订单-' . $curpage_name . date('Y-m-d-H', time()), $excel_data);
    }
