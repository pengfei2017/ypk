...
class SnstraceController extends ControllerBase
{
    public function initialize(){
        parent::initialize();
        $this->translation = getTranslation('common,layout,member,snstrace');
        $this->view->setVar('lang', $this->translation);
    }

    public function indexAction() {
        $this->tracelistAction();
    }
...
//        Tpl::showpage('snstrace.index');
        $this->view->pick('snstrace/snstrace_index');
    }
    ...
    public function get_xmlAction() {
        ...
        foreach ((array)$help_list as $value) {
            ...
            $operation .= "<span class='btn'><em><i class='fa fa-cog'></i>" . getLang('nc_set') . " <i class='arrow'></i></em><ul>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajax_form(\"login\",\"动态内容\",\"". getUrl('shop_manager/snstrace/traceinfo', array('id' => $value['trace_id'])) ."\",640)'>动态内容</a></li>";
            $operation .= "<li><a href='".getUrl('shop_manager/snstrace/commentlist',array('tid'=>$value['trace_id']))."'>查看评论</a></li>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajaxget(\"" . getUrl('shop_manager/snstrace/traceedit',array('id'=> $value['trace_id'], 'value' => ($value['trace_state'] == 1 ? 0 : 1))) . "\")'>".($value['trace_state'] == 1 ? '显示动态' : '屏蔽动态')."</a></li>";
            $operation .= "</ul></span>";
    ...
    private function getTracePrivacy() {
        return array(
            '0' => getLang('admin_snstrace_privacy_all'),
            '1' => getLang('admin_snstrace_privacy_friend'),
            '2' => getLang('admin_snstrace_privacy_self')
        );
    }
     ...
     * 动态详细信息
     */
    public function traceinfoAction() {
        $tid = intval($_GET['id']);
        ...
        if(!empty($trace_info['trace_content'])){
            ...
            //将收藏商品和医生连接剔除
            $trace_info['trace_content'] = str_replace(getLang('admin_snstrace_collectgoods'), "", $trace_info['trace_content']);
            $trace_info['trace_content'] = str_replace(getLang('admin_snstrace_collectstore'), "", $trace_info['trace_content']);
        }
        Tpl::output('trace_info', $trace_info);
//        Tpl::setDirquna('shop');
//        Tpl::showpage('snstrace.info', 'null_layout');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->pick('snstrace/snstrace_info');
    }
    ...
    public function tracedelAction(){
        ...
        if($result){
            ...
            $this->log(getLang('nc_del,admin_snstrace_comment'),1);
    ...
    public function traceeditAction(){
        $tid = intval($_GET['id']);
        if($tid <= 0){
            showDialog(getLang('admin_snstrace_pleasechoose_edit'));
        }
        //删除动态
        $tracelog_model = Model('sns_tracelog');
        ...
        if($result){
            ...
            $this->log(getLang('nc_edit,admin_snstrace_comment'),1);
            showDialog(getLang('nc_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();');
        }else{
...
//        Tpl::showpage('snscomment.index');
        $this->view->pick('snstrace/snscomment_index');
     ...
     * 输出XML数据
     */
    public function get_comment_xmlAction() {
        ...
        foreach ((array)$help_list as $value) {
            ...
            $operation .= "<span class='btn'><em><i class='fa fa-cog'></i>" . getLang('nc_set') . " <i class='arrow'></i></em><ul>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajax_form(\"login\",\"评论内容\",\"". getUrl('shop_manager/snstrace/comminfo', array('id' => $value['comment_id'])) ."\",640)'>评论内容</a></li>";
            $operation .= "<li><a href='javascript:void(0);' onclick='ajaxget(\"" . getUrl('shop_manager/snstrace/commentedit',array('id'=> $value['comment_id'], 'value' => ($value['comment_state'] == 1 ? 0 : 1))) . "\")'>".($value['comment_state'] == 1 ? '屏蔽评论' : '显示评论')."</a></li>";
            $operation .= "</ul></span>";
            $param['operation'] = $operation;
...
//        Tpl::showpage('snscomment.info', 'null_layout');
        $this->view->pick('snstrace/snscomment_info');
    }
    ...
    public function commentdelAction(){
        $cid = intval($_GET['id']);
        if($cid <= 0){
            exit(json_encode(array('state'=>false,'msg'=>getLang('admin_snstrace_pleasechoose_del'))));
        }
        //删除评论
        $comment_model = Model('sns_comment');
        $result = $comment_model->delComment(array('comment_id'=>$cid));
        if($result){
            $this->log(getLang('nc_del,admin_snstrace_pl'),1);
            exit(json_encode(array('state'=>true,'msg'=>'删除成功')));
        }else{
            exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
        }
    ...
    public function commenteditAction(){
        ...
        if($result){
            $this->log(getLang('nc_edit,admin_snstrace_pl'),1);
            showDialog(getLang('nc_common_op_succ'), '', 'succ', '$("#flexigrid").flexReload();');
        }else{
            showDialog(getLang('nc_common_op_fail'));
        }
