...
class StatTradeController extends ControllerBase
{
    private $links = array(
...
//        array('url'=>'act=stat_trade&op=sale','lang'=>'stat_sale'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_trade', 'action' => 'income'), 'lang' => 'stat_sale_income'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_trade', 'action' => 'predeposit'), 'lang' => 'stat_predeposit'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_trade', 'action' => 'sale'), 'lang' => 'stat_sale'),
    );
...
//        import('function.datehelper');
    ...
    public function incomeAction()
    {
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            //header
            $excel_data[0][0] = array('styleid' => 's_title', 'data' => '医生名称');
            ...
            $excel_data[0][5] = array('styleid' => 's_title', 'data' => '医生费用');
            ...
            $excel_obj->addWorksheet($excel_obj->charset('医生佣金统计', CHARSET));
            $excel_obj->generateXML($excel_obj->charset('医生佣金统计', CHARSET) . date('Y-m-d-H', time()));
            exit();
        } else {
            Tpl::output('top_link', $this->sublink($this->links, 'income'));
//            Tpl::setDirquna('shop');
//            Tpl::showpage('stat.income');
            $this->view->pick('stat_trade/stat_income');
        }
    }
     ...
     * 输出平台总数据
     */
    public function get_plat_incomeAction()
    {
        $model = Model('stat');
        $where = array();
        if ($_GET['store_name']) {
        ...
        echo '<li title="医生费用：' . number_format($plat_data[0]['os_store_cost_totals'], 2) . '元"><h4>医生费用</h4><h2 id="count-number" class="timer" data-speed="1500" data-to="' . $plat_data[0]['os_store_cost_totals'] . '"></h2><h6>元</h6></li>';
     ...
     * 输出销售收入统计XML数据
     */
    public function get_income_xmlAction()
    {
        $model = Model('stat');
        $where = array();
        if ($_GET['store_name']) {
        ...
        if (!empty($list) && is_array($list)) {
            ...
            foreach ($list as $k => $v) {
                ...
                $out_array['ob_store_name'] = '<a href="' . getUrl('shop_manager/show_store/index', array('store_id' => $v['ob_store_id'])) . '" target="_blank">' . $v['ob_store_name'] . '</a>';
...
//            Tpl::showpage('stat.predeposit');
            $this->view->pick('stat_trade/stat_predeposit');
     ...
     * 预存款统计
     */
    public function get_predeposit_highchartsAction($_group)
    {
        $where = array();
        $curr_arr = array();
        $up_arr = array();
...
//        Tpl::showpage('stat.linelabels','null_layout');
        ...
        $this->view->pick('stat_trade/stat_linelabels');
    }

    /**
     * 获取平台总预存款统计
     */
    public function get_plat_predepositAction()
    {
        $model = Model('stat');
        $where = array();
        if (trim($_GET['pd_type']) == 'cash_pay') {
     ...
     * 输出销售收入统计XML数据
     */
    public function get_predeposit_xmlAction()
    {
        $model = Model('stat');
        $where = array();
        if (trim($_GET['pd_type']) == 'cash_pay') {
    ...
    public function saleAction()
    {
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            //统计数据标题
            $statlist = array();
            $statlist['headertitle'] = array('订单号', '买家', '医生名称', '下单时间', '订单总额', '订单状态');
...
//            Tpl::showpage('stat.sale');
            $this->view->pick('stat_trade/stat_sale');
        }
    }
     ...
     * 输出平台订单总数据
     */
    public function get_plat_saleAction()
    {
        $model = Model('stat');
     ...
     * 输出订单统计XML数据
     */
    public function get_sale_xmlAction()
    {
        $model = Model('stat');
     ...
     * 订单走势
     */
    public function sale_trendAction()
    {
        $model = Model('stat');
        //默认统计当前数据
        if (!$this->search_arr['search_type']) {
