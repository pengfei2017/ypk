...
class StatGeneralController extends ControllerBase
{
    private $links = array(
...
//        array('url'=>'act=stat_general&op=orderprange','lang'=>'stat_orderpricerange'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_general', 'action' => 'general'), 'lang' => 'stat_generalindex'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_general', 'action' => 'setting'), 'lang' => 'stat_goodspricerange'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_general', 'action' => 'orderprange'), 'lang' => 'stat_orderpricerange'),
    );
...
//        Tpl::showpage('stat.general.index');
        $this->view->pick('stat_general/stat_general_index');
    }
    ...
    public function settingAction()
    {
        ...
        if (chksubmit()) {
            ...
            if ($result === true) {
                $this->log(getLang('nc_edit,stat_setting'), 1);
                showMessage(getLang('nc_common_save_succ'));
            } else {
                $this->log(getLang('nc_edit,stat_setting'), 0);
...
//        Tpl::showpage('stat.setting');
        $this->view->pick('stat_general/stat_setting');
    }

    /**
     * 统计设置
     */
    public function orderprangeAction()
    {
        $model_setting = Model('setting');
        if (chksubmit()) {
            $update_array = array();
...
//        Tpl::showpage('stat.setting.orderprange');
        $this->view->pick('stat_general/stat_setting_orderprange');
    }
}
