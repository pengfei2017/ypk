...
class PointsController extends ControllerBase{
    ...
    public function initialize(){
        parent::initialize();
        $this->translation = getTranslation('common,points,layout,setting,member');
        //判断系统是否开启积分功能
        if (getConfig('points_isuse') != 1){
            showMessage(getLang('admin_points_unavailable'),getUrl('shop_manager/setting/index'),'','error');
        }
    }

    public function indexAction() {
        $this->pointslogAction();
    }
    ...
    public function addpointsAction(){
        if (chksubmit()){
            ...
            if (!is_array($member_info) || count($member_info)<=0){
                showMessage(getLang('admin_points_userrecord_error'),getUrl('shop_manager/points/addpoints'),'','error');
            }

            $pointsnum = intval($_POST['pointsnum']);
            if ($_POST['operatetype'] == 2 && $pointsnum > intval($member_info['member_points'])){
                showMessage(getLang('admin_points_points_short_error').$member_info['member_points'],getUrl('shop_manager/points/addpoints'),'','error');
            }
            ...
            if ($_POST['pointsdesc']){
                ...
                $insert_arr['pl_desc'] = getLang('admin_points_system_desc');
            }
            $result = $obj_points->savePointsLog('system',$insert_arr,true);
            if ($result){
                $this->log(getLang('admin_points_mod_tip').$member_info['member_name'].'['.(($_POST['operatetype'] == 2)?'':'+').strval($insert_arr['pl_points']).']',null);
                showMessage(getLang('nc_common_save_succ'),getUrl('shop_manager/points/addpoints'));
            }else {
                showMessage(getLang('nc_common_save_fail'),getUrl('shop_manager/points/addpoints'),'','error');
            }
        }else {
//            Tpl::setDirquna('shop');
//            Tpl::showpage('points.add');
            $this->view->pick('points/points_add');
        }
    }
...
//        Tpl::showpage('points.log');
        $this->view->pick('points/points_log');
    }
...
//        Tpl::showpage('points.setting');
        $this->view->pick('points/points_setting');
    }
