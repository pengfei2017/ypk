...
{
    private $links = array(
        array('url' => array('module' => 'shop_manager', 'controller' => 'evaluate', 'action' => 'evalgoods_list'), 'text' => '来自买家的评价'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'evaluate', 'action' => 'evalstore_list'), 'text' => '医生动态评价'),
    );
    public function initialize() {
        parent::initialize();
        $this->translation = getTranslation('evaluate,common,layout');
        $url_param_array = getUrlParamArray();
        if ($url_param_array['action'] == 'index') $url_param_array['action'] = 'evalgoods_list';
        Tpl::output('top_link',$this->sublink($this->links,$url_param_array['action']));
    }
    public function indexAction() {
        $this->evalgoods_listAction();
    }

    /**
     * 商品来自买家的评价列表
     */
    public function evalgoods_listAction() {
//
//        Tpl::setDirquna('shop');
//        Tpl::showpage('evalgoods.index');
        $this->view->pick('evaluate/evalgoods_index');
    }

    /**
     * 商品来自买家的评价列表
     */
    public function get_goods_xmlAction() {
        ...
        foreach ($evalgoods_list as $k => $evalgoods_info) {
            ...
            $list['geval_goodsname'] = "<a class='open' title='{$evalgoods_info['geval_goodsname']}' href='". getUrl('shop_manager/goods/index', array('goods_id' => $evalgoods_info['geval_goodsid'])) ."' target='blank'>{$evalgoods_info['geval_goodsname']}</a>";
            $list['geval_storename'] = "<a class='open' title='{$evalgoods_info['geval_storename']}' href='". getUrl('shop_manager/show_store/index', array('store_id'=>$evalgoods_info['geval_storeid'])) ."' target='blank'>{$evalgoods_info['geval_storename']}</a>";
     ...
     * 删除商品评价
     */
    public function evalgoods_delAction() {
     ...
     * 医生动态评价列表
     */
    public function evalstore_listAction() {

//        Tpl::setDirquna('shop');
//        Tpl::showpage('evalstore.index');
        $this->view->pick('evaluate/evalstore_index');
    }
    ...
    public function get_store_xmlAction() {
        ...
        foreach ($evalstore_list as $k => $evalstore_info) {
            ...
            $list['seval_storename'] = "<a class='open' title='{$evalstore_info['seval_storename']}' href='". getUrl('shop_manager/show_store/index', array('store_id'=>$evalstore_info['seval_storeid'])) ."' target='blank'>{$evalstore_info['seval_storename']}</a>";
     ...
     * 删除医生评价
     */
    public function evalstore_delAction() {
        if (preg_match('/^[\d,]+$/', $_GET['seval_id'])) {
            ...
            if ($result) {
                $this->log('删除医生评价，评价编号[ID:'.implode(',', $_GET['seval_id']).']',null);
                exit(json_encode(array('state'=>true,'msg'=>'删除成功')));
            } else {
                exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
            }
