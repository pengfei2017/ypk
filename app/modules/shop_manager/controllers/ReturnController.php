...
class ReturnController extends ControllerBase
{
    ...
    private $links = array(
        array('url' => array('module' => 'shop_manager', 'controller' => 'return','action' => 'return_manage'), 'text' => '待处理'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'return','action' => 'return_all'), 'text' => '所有记录'),
    );
    public function initialize(){
        ...
        if ($url_param_array['action'] == 'index') $url_param_array['action'] = 'return_manage';
        Tpl::output('top_link',$this->sublink($this->links,$url_param_array['action']));
    }

    public function indexAction() {
        $this->return_manageAction();
    }

    /**
     * 待处理列表
     */
    public function return_manageAction() {
//        Tpl::setDirquna('shAction');
//        Tpl::showpage('return_manage.list');
        $this->view->pick('return/return_manage_list');
    ...
    public function get_manage_xmlAction() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] = "<a class=\"btn orange\" href=\"".getUrl('shop_manager/return/edit',array('return_id'=>$return_info['refund_id']))."\"><i class=\"fa fa-gavel\"></i>处理</a>";
            ...
            $list['goods_name'] = "<a class='open' title='{$return_info['goods_name']}' href='". getUrl('shop_manager/goods/index', array('goods_id' => $return_info['goods_id'])) ."' target='blank'>{$return_info['goods_name']}</a>";
     ...
     * 所有记录
     */
    public function return_allAction() {
//        Tpl::setDirquna('shop');
//        Tpl::showpage('return_all.list');
        $this->view->pick('return/return_all_list');
    }
    ...
    public function get_all_xmlAction() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] .= "<a class=\"btn green\" href=\"".getUrl('shop_manager/return/view',array('return_id'=>$return_info['refund_id']))."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $list['refund_sn'] = $return_info['refund_sn'];
            $list['refund_amount'] = ncPriceFormat($return_info['refund_amount']);
            if(!empty($return_info['pic_info'])) {
                $info = unserialize($return_info['pic_info']);
            ...
            $list['buyer_message'] = "<span title='{$return_info['buyer_message']}'>{$return_info['buyer_message']}</span>";
            $list['add_times'] = date('Y-m-d H:i:s',$return_info['add_time']);
            $list['goods_name'] = "<a class='open' title='{$return_info['goods_name']}' href='". getUrl('shop/goods/index', array('goods_id' => $return_info['goods_id'])) ."' target='blank'>{$return_info['goods_name']}</a>";
            $list['goods_num'] = $return_info['return_type'] == 2 ? $return_info['goods_num']:'';
            $state_array = $model_refund->getRefundStateArray('seller');
            $list['seller_state'] = $state_array[$return_info['seller_state']];
    ...
    public function editAction() {
        ...
        if (chksubmit()) {
            if ($return['refund_state'] != '2') {//检查状态,防止页面刷新不及时造成数据错误
                $this->showMessage(Language::get('nc_common_save_fail'));
            }
            ...
            if ($state) {
                ...
                $param['param'] = array(
                    'refund_url' => getUrl('shop_manager/member_refund/view', array('return_id' => $return['refund_id'])),
                    'refund_sn' => $return['refund_sn']
                );
                QueueClient::push('sendMemberMsg', $param);

                $this->showMessage(getLang('nc_common_save_succ'),getUrl('shop_manager/return/return_manage'));
            } else {
...
//        Tpl::showpage('return.edit');
        $this->view->pick('return/return_edit');
    }
...
//        Tpl::showpage('return.view');
        $this->view->pick('return/return_view');
    }
