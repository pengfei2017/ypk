...
use Ypk\Modules\Admin\Controllers\CommonController;
use Ypk\Modules\Admin\Controllers\ControllerBase;
use Ypk\Tpl;

class OwnshopController extends ControllerBase
{
    ...
    {
        $this->view->pick('ownshop/ownshop');
    }
    ...
    public function addAction()
    {
        if (chksubmit())
        {
            ...
            if (!$this->checkSellerName($_POST['seller_name']))
                showMessage('店主商家账号名称已被其它医生占用', '', 'html', 'error');
            ...
            $this->log("新增自营医生: {$saveArray['store_name']}");
            showMessage('操作成功', getUrl('shop_manager/ownshop/list'));
            return;
        }
        $this->view->pick('ownshop/ownshop_add');
    }
    ...
    public function delAction() {
        ...
        if (empty($store_info)) {
            exit(json_encode(array('state'=>false,'msg'=>'自营医生不存在')));
        }

        if (!$store_info['is_own_shop']) {
            exit(json_encode(array('state'=>false,'msg'=>'不能在此删除非自营医生')));
        }
        ...
        if ($count > 0) {
            exit(json_encode(array('state'=>false,'msg'=>'已经发布商品的自营医生不能被删除')));
        }
        ...
        $this->log("删除自营医生: " . $store_info['store_name']);
    ...
    public function editAction()
    {
        ...
        if (!$storeArray['is_own_shop']) {
            showMessage('不能在此管理非自营医生', '', 'html', 'error');
        }
        ...
        if (chksubmit()) {
            ...
            */
            ...
            $this->log("编辑自营医生: {$saveArray['store_name']}");
            showMessage('操作成功', getUrl('shop_manager/ownshop/list'));
        }

        if (empty($storeArray))
            showMessage('医生不存在', '', 'html', 'error');

        Tpl::output('store_array', $storeArray);
//        Tpl::setDirquna('shop');
//        Tpl::showpage('ownshop.edit');
        $this->view->pick('ownshop/ownshop_edit');
    }
...
//        Tpl::showpage('ownshop.bind_class');
        $this->view->pick('ownshop/ownshop_bind_class');
    }
    ...
    public function bind_class_addAction()
    {
        ...
        if($result) {
            ...
            $this->log('增加自营医生经营类目，类目编号:'.$result.',医生编号:'.$store_id);
    ...
    private function _bind_class_del($bid)
    {
        ...
        $this->log('删除自营医生经营类目，类目编号:'.$bid.',医生编号:'.$store_bind_class_info['store_id']);
        return $data;
    }

    public function bind_class_updateAction()
    {
        $bid = intval($_GET['id']);
        if($bid <= 0) {
    ...
    public function get_xmlAction() {
        ...
        foreach ($store_list as $value) {
            $param = array();
            $operation = "<a class='btn blue' href='".getUrl('shop_manager/ownshop/edit',array('id'=>$value['store_id']))."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            if ($value['bind_all_gc'] != 1 || empty($stores_with_goods[$value['store_id']])) {
                ...
                if ($value['bind_all_gc'] != 1) {
                    $operation .= "<li><a href='".getUrl('shop_manager/ownshop/bind_class',array('id'=>$value['store_id']))."'>编辑经营类目</a></li>";
                }
                if (empty($stores_with_goods[$value['store_id']])) {
                    $operation .= "<li><a href=\"javascript:void(0);\" onclick=\"fg_delete('".$value['store_id']."');\">删除自营医生</a></li>";
                }
                $operation .= "</ul></span>";
            }
