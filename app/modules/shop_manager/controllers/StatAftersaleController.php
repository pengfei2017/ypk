...
class StatAftersaleController extends ControllerBase
{
    private $links = array(
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_aftersale', 'action' => 'refund'), 'lang'=>'stat_refund'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_aftersale', 'action' => 'evalstore'), 'lang'=>'stat_evalstore'),
    );
...
//        Tpl::showpage('stat.aftersale.refund');
        $this->view->pick('stat_aftersale/stat_aftersale_refund');
    }
    /**
     * 退款统计
     */
    public function refundlistAction(){
        ...
        $statheader[] = array('text'=>'医生名','key'=>'store_name','class'=>'alignleft');
        ...
        foreach ((array)$refundlist_tmp as $k => $v){
            ...
            foreach ((array)$statheader as $h_k=>$h_v){
                ...
                if ($h_v['key'] == 'goods_name'){
                    $tmp[$h_v['key']] = '<a href="'.getUrl('shop_manager/goods/index', array('goods_id' => $v['goods_id'])).'" target="_blank">'.$v['goods_name'].'</a>';
                }
            }
            $statlist[] = $tmp;
        }
     ...
     * 退款统计
     */
    public function get_refund_highchartsAction(){
        $where = array();
...
//        Tpl::showpage('stat.linelabels','null_layout');
        ...
        $this->view->pick('stat_aftersale/stat_linelabels');
    }
    /**
     * 输出退款统计XML数据
     */
    public function get_refundlist_xmlAction(){
        $where = array();
        $model = Model('stat');
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where['add_time'] = array('between',$searchtime_arr);
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                ...
                if ($v['goods_id'] > 0) {
                    $out_array['goods_name'] = '<a href="'.getUrl('shop_manager/goods/index', array('goods_id' => $v['goods_id'])).'" target="_blank">'.$v['goods_name'].'</a>';
                }
     ...
     * 医生动态评分统计
     */
    public function evalstoreAction(){
        //医生分类
        Tpl::output('class_list', read_file_cache('store_class', true));
        ...
        if ($this->search_arr['exporttype'] == 'excel'){
            ...
            $excel_data = $excel_obj->charset($excel_data,CHARSET);
            $excel_obj->addArray($excel_data);
            $excel_obj->addWorksheet($excel_obj->charset('医生动态评分统计',CHARSET));
            $excel_obj->generateXML($excel_obj->charset('医生动态评分统计',CHARSET).date('Y-m-d-H',time()));
            exit();
        }
        Tpl::output('top_link',$this->sublink($this->links, 'evalstore'));
//        Tpl::setDirquna('shop');
//        Tpl::showpage('stat.aftersale.evalstore');
        $this->view->pick('stat_aftersale/stat_aftersale_evalstore');
    }

    /**
     * 输出医生动态评分统计XML数据
     */
    public function get_evalstore_xmlAction(){
        $model = Model('stat');
        $where = array();
        if(intval($_GET['store_class']) > 0){
            $where['sc_id'] = intval($_GET['store_class']);
