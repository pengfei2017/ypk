...
class SnsMemberController extends ControllerBase
{
    public function initialize(){
        parent::initialize();
        $this->translation = getTranslation('common,layout,member,sns_member');
    }
    public function indexAction(){
        $this->member_tagAction();
    }

    /**
     * 标签列表
     */
    public function member_tagAction(){
        // 实例化模型
        $model = Model();
        if(chksubmit()){
...
//        Tpl::showpage('sns_membertag.index');
        $this->view->pick('sns_member/sns_membertag_index');
    }
    ...
    public function tag_addAction(){
        if(chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["membertag_name"], "require"=>"true", "message"=>getLang('sns_member_tag_name_not_null')),
                array("input"=>$_POST["membertag_sort"], "require"=>"true", 'validator'=>'Number', "message"=>getLang('sns_member_tag_sort_is_number')),
            );
            ...
            if($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>getUrl('shop_manager/sns_member/tag_add'),
                            'msg'=>getLang('sns_member_add_once_more'),
                        ),
                        array(
                            'url'=>getUrl('shop_manager/sns_member/member_tag'),
                            'msg'=>getLang('sns_memner_return_list'),
                        )
                    );
                    $this->log(getLang('nc_add,sns_member_tag').'['.$_POST['membertag_name'].']',1);
                    showMessage(getLang('nc_common_op_succ'),$url);
                }else {
                    showMessage(getLang('nc_common_op_fail'));
                }
            }
...
//        Tpl::showpage('sns_membertag.add');
        $this->view->pick('sns_member/sns_membertag_add');
    }
    ...
    public function tag_editAction(){
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if ($result){
                    $this->log(getLang('nc_edit,sns_member_tag').'['.$_POST['membertag_name'].']',1);
                    showMessage(getLang('nc_common_op_succ'),getUrl('shop_manager/sns_member/member_tag'));
                }else {
                    showMessage(getLang('nc_common_op_fail'));
                }
            }
...
//        Tpl::showpage('sns_membertag.edit');
        $this->view->pick('sns_member/sns_membertag_edit');
    ...
    public function tag_delAction(){
        ...
        if ($result){
            $this->log(getLang('nc_del,sns_member_tag').'[ID:'.$mtag_id.']',1);
     ...
     * 推荐标签
     */
    public function tag_recommendAction() {
        // 验证
        $mtag_id = intval($_GET['id']);
        if($mtag_id <= 0){
            showDialog(getLang('param_error'),'reload');
        }
    ...
    public function get_xmlAction() {
        ...
        foreach ($tag_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn red' href='".getUrl('shop_manager/sns_member/tag_del',array('id'=>$value['mtag_id']))."'><i class='fa fa-trash-o'></i>删除</a><span class='btn'><em><i class='fa fa-cog'></i>" . getLang('nc_set') . " <i class='arrow'></i></em><ul><li><a href='".getUrl('shop_manager/sns_member/tag_edit',array('id'=>$value['mtag_id']))."'>编辑标签</a></li><li><a href='".getUrl('shop_manager/sns_member/tag_member',array('id'=>$value['mtag_id']))."'>标签会员</a></li><li><a href='javascript:void(0);' onclick='ajaxget(\"" . getUrl('shop_manager/sns_member/tag_recommend',array('id'=> $value['mtag_id'], 'recommend' => ($value['mtag_recommend'] == 1 ? 0 : 1))) . "\")'>" . ($value['mtag_recommend'] == 1 ? '取消推荐' : '推荐标签') . "</a></li></ul></span>";
     ...
     * 标签所属会员列表
     */
    public function tag_memberAction(){
        // 验证
        $mtag_id = intval($_GET['id']);
        if($mtag_id <= 0){
            showMessage(getLang('param_error'),'','','error');
...
//        Tpl::showpage('sns_membertag.memberlist');
        $this->view->pick('sns_member/sns_membertag_memberlist');
    }

    /**
     * 推荐标签会员
     */
    public function tag_mrecommendAction() {
        // 验证
        $mtag_id = intval($_GET['id']);
     ...
     * 删除添加标签会员
     */
    public function mtag_delAction(){
        $mtag_id = intval($_GET['id']);
        $member_id = intval($_GET['member_id']);
        if($mtag_id <= 0 || $member_id <= 0){
            showMessage(getLang('miss_argument'));
        }
        // 条件
        $where = array(
            'mtag_id'=>$mtag_id,
     ...
     * 输出XML数据
     */
    public function get_tm_xmlAction() {
        // 验证
        $mtag_id = intval($_GET['id']);
        if($mtag_id <= 0){
            showMessage(getLang('param_error'),'','','error');
        ...
        foreach ($tag_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn red' href='".getUrl('shop_manager/sns_member/mtag_del',array('id'=>$value['mtag_id'],'member_id'=>$value['member_id']))."'><i class='fa fa-list-alt'></i>删除</a><a class='btn green' href='javascript:void(0);' onclick='ajaxget(\"" . getUrl('shop_manager/sns_member/tag_mrecommend',array('id'=> $value['mtag_id'],'member_id' => $value['member_id'], 'recommend' => ($value['recommend'] == 1 ? 0 : 1))) . "\")'><i class='fa " . ($value['recommend'] == 1 ? 'fa-thumbs-o-down' : 'fa-thumbs-o-up') . "'></i>" . ($value['recommend'] == 1 ? '取消推荐' : '推荐会员') . "</a>";
