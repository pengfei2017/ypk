...
class MemberExpController extends ControllerBase{
     ...
     * 设置经验值获取规则
     */
    public function expsettingAction(){
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                $this->log(getLang('nc_edit,nc_exppoints_manage,nc_exppoints_setting'),1);
...
//        Tpl::showpage('member_exp.setting');
        $this->view->pick('member_exp/member_exp_setting');
    }
    /**
     * 积分日志列表
     */
...
//        Tpl::showpage('member_exp.log');
        $this->view->pick('member_exp/member_exp_log');
    }
     ...
     * 会员管理
     */
    public function member_gradeAction(){
        $model_setting = Model('setting');
        $list_setting = $model_setting->getListSetting();
        ...
        if (chksubmit()){
            ...
            if ($result){
                $this->log(getLang('nc_edit,nc_member_grade'),1);
                showDialog(getLang('nc_common_save_succ'),'reload','succ');
            } else {
                $this->log(getLang('nc_edit,nc_member_grade'),0);
...
//            Tpl::showpage('member_exp.grade');
            $this->view->pick('member_exp/member_exp_grade');
        }
    }
}