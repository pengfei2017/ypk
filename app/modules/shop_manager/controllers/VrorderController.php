...
class VrorderController extends ControllerBase
{
    /**
     * 每次导出订单数量
     * @var int
    ...
    {
        parent::initialize();
        $this->translation = getTranslation('common,trade,layout');
...
//        Tpl::showpage('vr_order.index');
        $this->view->pick('vr_order/vr_order');
    }
    ...
    public function get_xmlAction()
    {
        ...
        foreach ($order_list as $k => $order_info) {
            $list = array();
            $operation_detail = '';
            $list['operation'] = "<a class=\"btn green\" href=\"" . getUrl('shop_manager/vr_order/show_order', array('order_id' => $order_info['order_id'])) . "\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            ...
            if ($if_system_receive_pay) {
                $operation_detail .= "<li><a href=\"" . getUrl('shop_manager/vr_order/change_state', array('state_type' => 'receive_pay', 'order_id' => $order_info['order_id'])) . "\">收到货款</a></li>";
            }
            if ($operation_detail) {
    ...
    private function _order_cancel($order_info)
    {
        ...
        $this->log('关闭了订单,' . getLang('order_number') . ':' . $order_info['order_sn'], 1);
    ...
    private function _order_receive_pay($order_info, $post)
    {
        ...
        if (!chksubmit()) {
            ...
            $this->view->render('vr_order', 'order_receive_pay');
            $this->view->disable();
            exit();
        } else {
            $result = $logic_vr_order->changeOrderStatePay($order_info, 'admin', $post);
            if ($result['state']) {
                $this->log('将订单改为已收款状态,' . getLang('order_number') . ':' . $order_info['order_sn'], 1);
                //记录消费日志
    ...
    public function show_orderAction()
    {
        ...
        if ($order_id <= 0) {
            showMessage(getLang('miss_order_number'));
        }
        $model_vr_order = Model('vr_order');
...
//        Tpl::showpage('vr_order.view');
        $this->view->pick('vr_order/vr_order_view');
    }
    ...
    public function export_step1Action()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                $this->view->setVar('murl', getUrl('shop_manager/vr_order/index'));
    ...
    private function createExcel($data = array())
    {
        ...
        $excel_obj->generateXLS('虚拟订单-' . $curpage_name . date('Y-m-d-H', time()), $excel_data);
    }
