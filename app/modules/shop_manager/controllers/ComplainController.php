...
class ComplainController extends ControllerBase
{
    ...
    private $links = array(
        array('url' => array('module' => 'shop_manager', 'controller' => 'complain', 'action' => 'complain_new_list'), 'text' => '新投诉'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'complain', 'action' => 'complain_appeal_list'), 'text' => '待申诉'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'complain', 'action' => 'complain_talk_list'), 'text' => '对话中'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'complain', 'action' => 'complain_handle_list'), 'text' => '待仲裁'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'complain', 'action' => 'complain_finish_list'), 'text' => '已关闭'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'complain', 'action' => 'complain_subject_list'), 'text' => '主题设置'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'complain', 'action' => 'complain_setting'), 'text' => '时效设置'),
    );
    public function initialize() {
        parent::initialize();
        $this->translation = getTranslation('complain,common,layout');
        $url_param_array = getUrlParamArray();
        if ($url_param_array['action'] == 'index') $url_param_array['action'] = 'complain_new_list';
    ...
    public function indexAction() {
        $this->complain_new_listAction();
    }

    /*
     * 未处理的投诉列表
     */
    public function complain_new_listAction() {
     ...
     * 待申诉的投诉列表
     */
    public function complain_appeal_listAction() {
     ...
     * 对话的投诉列表
     */
    public function complain_talk_listAction() {
     ...
     * 待仲裁的投诉列表
     */
    public function complain_handle_listAction() {
     ...
     * 已关闭的投诉列表
     */
    public function complain_finish_listAction() {
...
//        Tpl::showpage('complain.list') ;
        $this->view->pick('complain/complain_list');
    }

    /*
     * 获取投诉列表
    */
    public function get_new_xmlAction() {
        $page = new Page();
        ...
        foreach ($complain_list as $k => $complain_info) {
            $list = array();
            if ($_GET['state'] == 99) {
                $list['operation'] = "<a class=\"btn orange\" href=\"".getUrl('shop_manager/complain/complain_progress',array('complain_id'=>$complain_info['complain_id']))."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            } else {
                $list['operation'] = "<a class=\"btn orange\" href=\"".getUrl('shop_manager/complain/complain_progress',array('complain_id'=>$complain_info['complain_id']))."\"><i class=\"fa fa-gavel\"></i>处理</a>";
            }
            ...
            $list['accused_name'] = "<a class='open' title='{$complain_info['accused_name']}' href='". getUrl('shop_manager/show_store/index', array('store_id'=>$complain_info['accused_id'])) ."' target='blank'>{$complain_info['accused_name']}</a>";
     ...
     * 进行中的投诉
     */
    public function complain_progressAction() {
...
//        Tpl::showpage('complain.info');
        $this->view->pick('complain/complain_info');
    }

    /*
     * 审核提交的投诉
     */
    public function complain_verifyAction() {
        ...
        if(intval($complain_info['complain_state']) === self::STATE_NEW) {
            ...
            if($model_complain->updateComplain($update_array,$where_array)) {
                $this->log(getLang('complain_verify_success').'['.$complain_id.']',1);
                ...
                showMessage(getLang('complain_verify_success'),getUrl('shop_manager/complain/complain_new_list'));
            } else {
                showMessage(getLang('complain_verify_fail'),getUrl('shop_manager/complain/complain_new_list'));
            }
        } else {
     ...
     * 关闭投诉
     */
    public function complain_closeAction() {
        //获取输入的数据
        $complain_id = intval($_POST['complain_id']);
        ...
        $obj_validate->validateparam = array(
            array("input"=>$final_handle_message, "require"=>"true","validator"=>"Length","min"=>"1","max"=>"255","message"=>getLang('final_handle_message_error')),
        );
        ...
        if($current_state !== self::STATE_FINISH) {
            ...
            if($model_complain->updateComplain($update_array,$where_array)) {
                ...
                $this->log(getLang('complain_close_success').'['.$complain_id.']',1);
                showMessage(getLang('complain_close_success'),$this->get_complain_state_link($current_state));
            } else {
                showMessage(getLang('complain_close_fail'),$this->get_complain_state_link($current_state));
            }
        } else {
            showMessage(getLang('param_error'),'');
        }
     ...
     * 投诉主题列表
     */
    public function complain_subject_listAction() {
        /*
         * 实例化分页
         */
        $page = new Page();
...
//        Tpl::showpage('complain_subject.list') ;
        $this->view->pick('complain/complain_subject_list');
    }

    /*
     * 添加投诉主题页面
     */
    public function complain_subject_addAction() {
        $this->show_menu('complain_subject_add');
//        Tpl::setDirquna('shop');
//        Tpl::showpage('complain_subject.add') ;
        $this->view->pick('complain/complain_subject_add');
     ...
     * 保存添加的投诉主题
     */
    public function complain_subject_saveAction() {
        //获取提交的内容
        ...
        $obj_validate = new Validate();
        $obj_validate->validateparam = array(
            array("input"=>$input['complain_subject_content'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>getLang('complain_subject_content_error')),
            array("input"=>$input['complain_subject_desc'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"100","message"=>getLang('complain_subject_desc_error')),
        );
        $error = $obj_validate->validate();
        if ($error != '') {
            showMessage($error);
            ...
            $this->log(getLang('complain_subject_add_success').'['.$_POST['complain_subject_content'].']',1);
            showMessage(getLang('complain_subject_add_success'),getUrl('shop_manager/complain/complain_subject_list'));
        }
    }
     ...
     * 删除投诉主题,伪删除只是修改标记
     */
    public function complain_subject_dropAction() {
        $complain_subject_id = trim($_GET['complain_subject_id']);
        if(empty($complain_subject_id)) {
            showMessage(getLang('param_error'),getUrl('shop_manager/complain/index'));
        }
        $model_complain_subject= Model('complain_subject');
        $update_array = array();
        ...
        if($model_complain_subject->updateComplainSubject($update_array,$where_array)) {
            $this->log(getLang('complain_subject_delete_success').'[ID:'.$_GET['complain_subject_id'].']',1);
            showMessage(getLang('complain_subject_delete_success'),getUrl('shop_manager/complain/complain_subject_list'));
        } else {
            showMessage(getLang('complain_subject_delete_fail'),getUrl('shop_manager/complain/complain_subject_list'));
        }
    }
    /*
     * 根据投诉id获取投诉对话列表
     */
    public function get_complain_talkAction() {
        $complain_id = intval($_POST['complain_id']);
        $complain_info = $this->get_complain_info($complain_id);
        ...
        foreach($complain_talk_list as $talk) {
            ...
            switch($talk['talk_member_type']) {
                case 'accuser':
                    $talk_list[$i]['talk'] .= getLang('complain_accuser');
                    break;
                case 'accused':
                    $talk_list[$i]['talk'] .= getLang('complain_accused');
                    break;
                case 'admin':
                    $talk_list[$i]['talk'] .= getLang('complain_admin');
                    break;
                default:
                    $talk_list[$i]['talk'] .= getLang('complain_unknow');
            }
            if(intval($talk['talk_state']) === 2) {
                $talk['talk_content'] = getLang('talk_forbit_message');
                $forbit_link = '';
            } else {
                $forbit_link = "&nbsp;&nbsp;<a href='#' onclick=forbit_talk(".$talk['talk_id'].")>".getLang('complain_text_forbit')."</a>";
            }
            $talk_list[$i]['talk'].= '('.$talk['talk_member_name'].')'.getLang('complain_text_say').':'.$talk['talk_content'].$forbit_link;
            $i++;
        }
        if (strtoupper(CHARSET) == 'GBK') {
            $talk_list = getTranslation($talk_list);
        }
     ...
     * 发布投诉对话
     */
    public function publish_complain_talkAction() {
        $complain_id = intval($_POST['complain_id']);
        ...
        if($talk_len > 0 && $talk_len < 255) {
            ...
            if (strtoupper(CHARSET) == 'GBK') {
                $complain_talk = getUTF8($complain_talk);
            }
     ...
     * 屏蔽对话
     */
    public function forbit_talkAction() {
     ...
     * 投诉设置
     **/
    public function complain_settingAction() {
        //读取设置内容 $list_setting
...
//        Tpl::showpage('complain.setting') ;
        $this->view->pick('complain/complain_setting');
    }

    /**
     * 投诉设置保存
     **/
    public function complain_setting_saveAction() {
        $model_setting = Model('setting');
        ...
        if($model_setting->updateSetting($update_array)) {
            $this->log(getLang('complain_setting_save_success'),1);
            showMessage(getLang('complain_setting_save_success'),'');
        } else {
            showMessage(getLang('complain_setting_save_fail'),'');
        }
    }
    ...
    private function get_complain_state_text($complain_state) {
        switch(intval($complain_state)) {
            case self::STATE_NEW:
                return getLang('complain_state_new');
                break;
            case self::STATE_APPEAL:
                return getLang('complain_state_appeal');
                break;
            case self::STATE_TALK:
                return getLang('complain_state_talk');
                break;
            case self::STATE_HANDLE:
                return getLang('complain_state_handle');
                break;
            case self::STATE_FINISH:
                return getLang('complain_state_finish');
                break;
            default:
                showMessage(getLang('param_error'),'');
        }
    ...
    private function get_complain_state_link($complain_state) {
        switch(intval($complain_state)) {
            case self::STATE_NEW:
                return getUrl('shop_manager/complain/complain_new_list');
                break;
            case self::STATE_APPEAL:
                return getUrl('shop_manager/complain/complain_appeal_list');
                break;
            case self::STATE_TALK:
                return getUrl('shop_manager/complain/complain_talk_list');
                break;
            case self::STATE_HANDLE:
                return getUrl('shop_manager/complain/complain_handle_list');
                break;
            case self::STATE_FINISH:
                return getUrl('shop_manager/complain/complain_finish_list');
                break;
            default:
                showMessage(getLang('param_error'));
        }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'complain_new_list'=>array('menu_type'=>'link','menu_name'=>getLang('complain_new_list'),'menu_url'=>getUrl('shop_manager/complain/complain_new_list')),
            'complain_appeal_list'=>array('menu_type'=>'link','menu_name'=>getLang('complain_appeal_list'),'menu_url'=>getUrl('shop_manager/complain/complain_appeal_list')),
            'complain_talk_list'=>array('menu_type'=>'link','menu_name'=>getLang('complain_talk_list'),'menu_url'=>getUrl('shop_manager/complain/complain_talk_list')),
            'complain_handle_list'=>array('menu_type'=>'link','menu_name'=>getLang('complain_handle_list'),'menu_url'=>getUrl('shop_manager/complain/complain_handle_list')),
            'complain_finish_list'=>array('menu_type'=>'link','menu_name'=>getLang('complain_finish_list'),'menu_url'=>getUrl('shop_manager/complain/complain_finish_list')),
            'complain_subject_list'=>array('menu_type'=>'link','menu_name'=>getLang('complain_subject_list'),'menu_url'=>getUrl('shop_manager/complain/complain_subject_list')),
            'complain_subject_add'=>array('menu_type'=>'link','menu_name'=>getLang('complain_subject_add'),'menu_url'=>getUrl('shop_manager/complain/complain_subject_add')),
            'complain_setting'=>array('menu_type'=>'link','menu_name'=>getLang('complain_setting'),'menu_url'=>getUrl('shop_manager/complain/complain_setting')),
            'complain_progress'=>array('menu_type'=>'link','menu_name'=>getLang('complain_progress'),'menu_url'=>'###'),
        );
