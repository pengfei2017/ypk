...
class StatMemberController extends ControllerBase
{
    private $links = array(
...
//        array('url'=>'act=stat_member&op=buying','lang'=>'stat_buying'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_member', 'action' => 'newmember'), 'lang' => 'stat_newmember'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_member', 'action' => 'analyze'), 'lang' => 'stat_memberanalyze'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_member', 'action' => 'scale'), 'lang' => 'stat_scaleanalyze'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_member', 'action' => 'area'), 'lang' => 'stat_areaanalyze'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_member', 'action' => 'buying'), 'lang' => 'stat_buying'),
    );
    ...
    {
        parent::initialize();
        $this->translation = getTranslation('layout,stat,common');
...
//        import('function.datehelper');
        ...
        //处理搜索时间
        if (!isset($this->search_arr['action']) || in_array($_REQUEST['action'], array('newmember', 'analyze', 'scale', 'area', 'buying', 'get_scale_xml'))) {
    ...
    public function newmemberAction()
    {
        ...
        if ($this->search_arr['search_type'] == 'day') {
            ...
            Tpl::output('actionurl', getUrl('shop_manager/stat_member/newmember', array('search_type' => 'day', 'search_time' => date('Y-m-d', $this->search_arr['day']['search_time']))));
        }
        ...
        if ($this->search_arr['search_type'] == 'week') {
            ...
            Tpl::output('actionurl', getUrl('shop_manager/stat_member/newmember') . '?search_type=week&searchweek_year=' . $this->search_arr['week']['current_year'] . '&searchweek_month=' . $this->search_arr['week']['current_month'] . '&searchweek_week=' . $this->search_arr['week']['current_week']);
        }
        ...
        if ($this->search_arr['search_type'] == 'month') {
            ...
            Tpl::output('actionurl', getUrl('shop_manager/stat_member/newmember') . '?search_type=month&searchmonth_year=' . $this->search_arr['month']['current_year'] . '&searchmonth_month=' . $this->search_arr['month']['current_month']);
        }
...
//            Tpl::showpage('stat.newmember');
            $this->view->pick('stat_member/stat_newmember');
        }
    }
...
//        Tpl::showpage('stat.memberanalyze');
        $this->view->pick('stat_member/stat_memberanalyze');
     ...
     * 会员分析异步详细列表
     */
    public function analyzeinfoAction()
    {
        $model = Model('stat');
        $where = array();
     ...
     * 输出会员分析详细列表XML数据
     */
    public function get_analyzeinfo_xmlAction()
    {
        $model = Model('stat');
        $where = array();
        $searchtime_arr_tmp = explode('|', $this->search_arr['t']);
    ...
    public function showmemberAction()
    {
        ...
        if ($this->search_arr['exporttype'] == 'excel') {
            ...
            $excel_obj->setStyle(array('id' => 's_title', 'Font' => array('FontName' => '宋体', 'Size' => '12', 'Bold' => '1')));
            //header
            $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('member_index_name'));
            $excel_data[0][] = array('styleid' => 's_title', 'data' => '注册时间');
            $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('member_index_login_time'));
            $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('member_index_last_login'));
            $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('member_index_points'));
            $excel_data[0][] = array('styleid' => 's_title', 'data' => getLang('member_index_prestore'));
            //data
            foreach ($member_list as $k => $v) {
                $excel_data[$k + 1][] = array('data' => $v['member_name'] . '(' . getLang('member_index_true_name,nc_colon') . $v['member_truename'] . ')');
                ...
                $excel_data[$k + 1][] = array('data' => getLang('member_index_available,nc_colon') . $v['available_predeposit'] . getLang('currency_zh') . '(' . getLang('member_index_frozen,nc_colon') . $v['freeze_predeposit'] . getLang('currency_zh') . ')');
            }
            $excel_data = $excel_obj->charset($excel_data, CHARSET);
            $excel_obj->addArray($excel_data);
...
//        Tpl::showpage('stat.info.memberlist');
        $this->view->pick('stat_member/stat_info_memberlist');
     ...
     * 输出订单统计XML数据
     */
    public function get_member_xmlAction()
    {
        $model = Model('stat');
        $where = array();
        if (!empty($_GET['t'])) {
...
//        Tpl::showpage('stat.memberscale');
        $this->view->pick('stat_member/stat_memberscale');
    }

    /**
     * 输出会员规模XML数据
     */
    public function get_scale_xmlAction()
    {
        if (!$this->search_arr['search_type']) {
            $this->search_arr['search_type'] = 'day';
        }
...
//        Tpl::showpage('stat.memberarea');
        $this->view->pick('stat_member/stat_memberarea');
    }
     ...
     * 输出区域分析详细XML数据
     */
    public function get_arealist_xmlAction()
    {
        $model = Model('stat');
        $where = array();
        $where['order_isvalid'] = 1;//计入统计的有效订单
     ...
     * 区域分析之地图数据
     */
    public function area_mapAction()
    {
        $model = Model('stat');
        $where = array();
        $where['order_isvalid'] = 1;//计入统计的有效订单
...
//        Tpl::showpage('stat.map','null_layout');
        $this->view->pick('stat_member/stat_map');
    }
    ...
    public function buyingAction()
    {
        ...
        $field = '1';
        $pricerange_arr = ($t = trim(getConfig('stat_orderpricerange'))) ? unserialize($t) : '';
        if ($pricerange_arr) {
...
//        Tpl::showpage('stat.buying');
        $this->view->pick('stat_member/stat_buying');
    }
}
