...
class StatStoreController extends ControllerBase
{
    private $links = array(
...
//        array('url'=>'act=stat_store&op=storearea','lang'=>'stat_storearea'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_store', 'action' => 'newstore'), 'lang' => 'stat_newstore'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_store', 'action' => 'hotrank'), 'lang' => 'stat_storehotrank'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_store', 'action' => 'storesales'), 'lang' => 'stat_storesales'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_store', 'action' => 'degree'), 'lang' => 'stat_storedegree'),
        array('url' => array('module' => 'shop_manager', 'controller' => 'stat_store', 'action' => 'storearea'), 'lang' => 'stat_storearea'),

    );

    private $search_arr;//处理后的参数
    private $store_class;//医生分类
...
//        import('function.datehelper');
        ...
        //医生分类
        $this->store_class = read_file_cache('store_class', true);
    ...
    {
        //导出excel连接地址
        $actionurl = getUrl('shop_manager/stat_store/newstore');
        $where = array();
        //所属医生分类
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            $excel_obj->addWorksheet($excel_obj->charset('新增医生统计', CHARSET));
            $excel_obj->generateXML($excel_obj->charset('新增医生统计', CHARSET) . date('Y-m-d-H', time()));
            exit();
        } else {
            //得到统计图数据
            $stat_arr['title'] = '新增医生统计';
            $stat_arr['yAxis'] = '新增医生数';
...
//            Tpl::showpage('stat.newstore');
            $this->view->pick('stat_store/stat_newstore');
        }
    }
...
//        Tpl::showpage('stat.store.hotrank');
        $this->view->pick('stat_store/stat_store_hotrank');
    }

    /**
     * 热卖排行列表
     */
    public function hotrank_listAction()
    {
        $datanum = 30;
        $model = Model('stat');
        switch ($_GET['type']) {
...
//        Tpl::showpage('stat.store.hotrank.list','null_layout');
        $this->view->pick('stat_store/stat_store_hotrank_list');
    }
    ...
    public function degreeAction()
    {
        ...
        if (!empty($storelist)) {
            foreach ($storelist as $k => $v) {
                $storelist[$k]['p_name'] = $v['grade_id'] > 0 ? $sd_list[$v['grade_id']] : '平台医生';
                $storelist[$k]['allnum'] = intval($v['allnum']);
                $statlist['headertitle'][] = $v['grade_id'] > 0 ? $sd_list[$v['grade_id']] : '平台医生';
...
//        Tpl::showpage('stat.storedegree');
        $this->view->pick('stat_store/stat_storedegree');
    ...
    public function showstoreAction()
    {
        ...
        if ($_GET['exporttype'] == 'excel') {
            ...
            $excel_obj->setStyle(array('id' => 's_title', 'Font' => array('FontName' => '宋体', 'Size' => '12', 'Bold' => '1')));
            //header
            $excel_data[0][] = array('styleid' => 's_title', 'data' => '医生名称');
            ...
            foreach ($store_list as $k => $v) {
                ...
                if (empty($search_grade_list[$v['grade_id']])) {
                    $excel_data[$k + 1][] = array('data' => '平台医生');
                } else {
            ...
            $excel_data = $excel_obj->charset($excel_data, CHARSET);
            $excel_obj->addArray($excel_data);
            $excel_obj->addWorksheet($excel_obj->charset('新增医生', CHARSET));
            $excel_obj->generateXML($excel_obj->charset('新增医生', CHARSET) . date('Y-m-d-H', time()));
            exit();
        }
...
//        Tpl::showpage('stat.info.storelist');
        $this->view->pick('stat_store/stat_info_store_list');
     ...
     * 输出医生列表XML数据
     */
    public function get_store_xmlAction()
    {
        $model = Model('stat');
     ...
     * 销售统计
     */
    public function storesalesAction()
    {
        if (!$this->search_arr['search_type']) {
            $this->search_arr['search_type'] = 'day';
        }
...
//        Tpl::showpage('stat.store.sales');
        $this->view->pick('stat_store/stat_store_sales');
    }

    /**
     * 医生销售统计列表
     */
    public function storesales_listAction()
    {
        $model = Model('stat');
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        ...
        //列表header
        $statheader = array();
        $statheader[] = array('text' => '医生名称', 'key' => 'store_name');
        ...
        if ($this->search_arr['exporttype'] == 'excel') {
            ...
            $excel_data = $excel_obj->charset($excel_data, CHARSET);
            $excel_obj->addArray($excel_data);
            $excel_obj->addWorksheet($excel_obj->charset('医生销售统计', CHARSET));
            $excel_obj->generateXML($excel_obj->charset('医生销售统计', CHARSET) . date('Y-m-d-H', time()));
            exit();
        }
    }
     ...
     * 输出医生销售统计XML数据
     */
    public function get_storesales_xmlAction()
    {
        $model = Model('stat');
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
...
//        Tpl::showpage('stat.store.salestrends','null_layout');
    ...
    public function storeareaAction()
    {
        ...
        foreach ((array)$province_array as $k => $v) {
            if ($statlist[$k]) {
                $stat_arr[] = array('cha' => $k, 'name' => $v, 'des' => "，医生量：{$statlist[$k]['storenum']}", 'level' => $statlist[$k]['level']);
            } else {
        ...
        Tpl::output('stat_json', $stat_json);
        Tpl::output('statlist', $statlist);
        $actionurl = getUrl('shop/stat_store/showstore',array('type'=>'storearea'));
        if (trim($_GET['search_time'])) {
...
//        Tpl::showpage('stat.storearea');
        $this->view->pick('stat_store/stat_storearea');
    }
}
