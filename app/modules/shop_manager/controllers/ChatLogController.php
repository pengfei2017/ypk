...
class ChatLogController extends ControllerBase
{
    public function initialize(){
        ...
        $this->translation = getTranslation('common,layout,member,predeposit');
    }
     ...
     * 聊天记录查询
     */
    public function chat_logAction() {

//        Tpl::setDirquna('shop');
//        Tpl::showpage('chat_log.list');
        $this->view->pick('chatlog/chat_log_list');
    }
    ...
    public function get_xmlAction() {
        ...
        foreach ($log_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn green' href='javascript:void(0);' onclick='ajax_form(\"login\",\"消息内容\",\"". getUrl('shop_manager/chat_log/chat_info', array('id' => $value['m_id'])) ."\",640)' class='url'><i class='fa fa-list-alt'></i>查看</a>";
...
//        Tpl::showpage('chat_msg_log.list');
        $this->view->pick('chatlog/chat_msg_log_list');
    }

    /**
     * 输出XML数据
     */
    public function get_msg_xmlAction() {
        $model_chat = Model('web_chat');
        $log_list = array();
        if (!empty($_GET['msg'])) {
            $add_time_from = strtotime($_GET['add_time_from']);
        ...
        foreach ($log_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn green' href='".getUrl('shop_manager/chat_log/chat_log',array('f_name'=>$value['f_name'],'t_name'=>$value['t_name'],'add_time_from'=>date('Y-m-d',$value['add_time']),'add_time_to'=>date('Y-m-d',$value['add_time'])))."' class='url'><i class='fa fa-list-alt'></i>详情</a>";
            $param['t_msg'] = $value['t_msg'];
            $param['add_time'] = date('Y-m-d H:i:s', $value['add_time']);
            $param['f_name'] = $value['f_name'];
            $param['f_id'] = $value['f_id'];
    ...
    public function chat_infoAction() {
        $id = $_GET['id'];
...
//        Tpl::showpage('chat_log.info', 'null_layout');
        $this->view->pick('chatlog/chat_log_info');
    }
}
