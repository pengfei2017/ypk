...
class ArticleClassController extends ControllerBase
{
    public function initialize(){
        parent::initialize();
        $this->translation = getTranslation('layout,article_class,common');
        $this->view->setVar('lang', $this->translation);
    }

    public function indexAction() {
        $this->article_classAction();
    }

    /**
     * 文章管理
     */
    public function article_classAction(){
...
//        Tpl::showpage('article_class.index');
        $this->view->pick('article_class/article_class_index');
    }

    /**
     * 文章分类 新增
     */
    public function article_class_addAction(){
        $lang   = getTranslation();
        $model_class = Model('article_class');
        if (chksubmit()){
            /**
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>getUrl('system_manager/article_class/article_class_add',array('ac_parent_id'=>intval($_POST['ac_parent_id']))),
                            'msg'=>$lang['article_class_add_class'],
                        ),
                        array(
                            'url'=>getUrl('system_manager/article_class/article_class'),
                            'msg'=>$lang['article_class_add_back_to_list'],
                        )
                    );
                    $this->log(getLang('nc_add,article_class_index_class').'['.$_POST['ac_name'].']',1);
...
//        Tpl::showpage('article_class.add');
        $this->view->pick('article_class/article_class_add');
    }

    /**
     * 文章分类编辑
     */
    public function article_class_editAction(){
        $lang   = getTranslation();
        $model_class = Model('article_class');
        if (chksubmit()){
            /**
...
//              $update_array['ac_parent_id'] = intval($_POST['ac_parent_id']);
                ...
                if ($result){
                    $url = array(
                        ...
                        array(
                            'url'=>getUrl('system_manager/article_class/article_class_edit',array('ac_id'=>intval($_POST['ac_id']))),
                            'msg'=>$lang['article_class_edit_again'],
                        ),
                    );
                    $this->log(getLang('nc_edit,article_class_index_class').'['.$_POST['ac_name'].']',1);
                    showMessage($lang['article_class_edit_succ'],getUrl('system_manager/article_class/article_class'));
                }else {
...
//        Tpl::showpage('article_class.edit');
        $this->view->pick('article_class/article_class_edit');
    }

    /**
     * 删除分类
     */
    public function article_class_delAction(){
        $lang   = getTranslation();
        $model_class = Model('article_class');
        if (intval($_GET['ac_id']) > 0){
            ...
            $this->log(getLang('nc_add,article_class_index_class').'[ID:'.intval($_GET['ac_id']).']',1);
            showMessage($lang['article_class_index_del_succ'],getUrl('system_manager/article_class/article_class'));
        }else {
            showMessage($lang['article_class_index_choose'],getUrl('system_manager/article_class/article_class'));
        }
    }
    /**
     * ajax操作
