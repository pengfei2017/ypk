...
class AdvController extends ControllerBase
{
    ...
    {
        parent::initialize();
        $this->translation = getTranslation('common,layout,adv');
        $this->view->setVar('lang', $this->translation);
    }

    public function indexAction()
    {
        $this->ap_manageAction();
    }
...
//            Tpl::showpage('adv.add');
            ...
            if ($error != '') {
                ...
                if ($result && $result2) {
                    $this->log(getLang('adv_add_succ') . '[' . $_POST["adv_name"] . ']', null);
                    showMessage($lang['adv_add_succ'], getUrl('system_manager/adv/adv', array('ap_id' => $_POST['ap_id'])));
                } else {
                    showMessage($lang['adv_add_fail'], getUrl('system_manager/adv/adv', array('ap_id' => $_POST['ap_id'])));
                }
            }
        }
    }
     ...
     * 管理广告位
     */
    public function ap_manageAction()
    {
//        Tpl::setDirquna('system');
//        Tpl::showpage('ap_manage');
        $this->view->pick('adv/ap_manage');
    }

    public function get_ap_xmlAction()
    {
        $lang = getTranslation();
        $adv = Model('adv');
        $condition = array();
        ...
        foreach ((array)$ap_list as $k => $ap_info) {
            ...
            $operation_detail = "<li><a href=" . getUrl('system_manager/adv/adv', array('ap_id' => $ap_info['ap_id'])) . "></i>管理广告</a></li>";
            $operation_detail .= "<li><a href='" . getUrl('system_manager/adv/ap_edit', array('ap_id' => $ap_info['ap_id'])) . "'>编辑属性</a></li>";
     ...
     * js代码调用
     */
    public function ap_copyAction()
    {
//        Tpl::setDirquna('system');
//        Tpl::showpage('ap_copy', 'null_layout');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->pick('adv/ap_copy');
    }
    ...
    public function adv_deleteAction()
    {
        if (preg_match('/^[\d,]+$/', $_GET['del_id'])) {
            $adv = Model('adv');
            $where = "where adv_id in (" . $_GET['del_id'] . ")";
...
//            Tpl::showpage('ap_edit');
            ...
            if ($error != '') {
                ...
                if ($result) {
                    $this->log(getLang('ap_change_succ') . '[' . $_POST["ap_name"] . ']', null);
     ...
     * 新增广告位
     */
    public function ap_addAction()
    {
        if ($_POST['form_submit'] != 'ok') {
//            Tpl::setDirquna('system');
//            Tpl::showpage('ap_add');
            $this->view->pick('adv/ap_add');
        } else {
            $lang = getTranslation();
            $adv = Model('adv');
            $upload = new UploadFile();
            ...
            if ($error != '') {
                ...
                if ($result) {
                    $this->log(getLang('ap_add_succ') . '[' . $_POST["ap_name"] . ']', null);
                    showMessage($lang['ap_add_succ'], getUrl('system_manager/adv/ap_manage'), 'html', 'succ', 1, 4000);
                } else {
...
//        Tpl::showpage('adv.index');
        $this->view->pick('adv/adv_index');
    }

    public function get_adv_xmlAction()
    {
        $lang = getTranslation();
        $adv = Model('adv');
        $condition = array();
        ...
        foreach ((array)$adv_list as $k => $adv_info) {
            $list = array();
            $operation_detail = '';
            $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$adv_info['adv_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='" . getUrl('system_manager/adv/adv_edit', array('adv_id' => $adv_info['adv_id'])) . "'><i class='fa fa-pencil-square-o'></i>编辑</a>";
     ...
     * 修改广告
     */
    public function adv_editAction()
    {
...
//            Tpl::showpage('adv.edit');
            ...
            if ($error != '') {
                ...
                if ($result) {
                    ...
                    $this->log(getLang('adv_change_succ') . '[' . $_POST["adv_name"] . ']', null);
