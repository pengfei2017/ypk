...
class DocumentController extends ControllerBase
{
    public function initialize(){
        parent::initialize();
        $this->translation = getTranslation('layout,common,document');
...
//        Tpl::showpage('document.index');
        $this->view->pick('document/document_index');
    }
        ...
        if(chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>getUrl('system_manager/document/document'),
                            'msg'=>$lang['document_edit_back_to_list']
                        ),
                        array(
                            'url'=>getUrl('system_manager/document/edit',array('doc_id'=>intval($_POST['doc_id']))),
                            'msg'=>$lang['document_edit_again']
                        ),
                    );
                    $this->log(getLang('nc_edit,document_index_document').'[ID:'.$_POST['doc_id'].']',1);
...
//        Tpl::showpage('document.edit');
        $this->view->pick('document/document_edit');
    }

    /**
     * 系统文章图片上传
     */
    public function document_pic_uploadAction(){
        /**
         * 上传图片
         */
