...
class LinkController extends ControllerBase
{
    public function initialize(){
        parent::initialize();
        $this->translation = getTranslation('common,layout,link');
...
//        Tpl::showpage('link.index');
        $this->view->pick('link/link_index');
    }
    ...
    public function link_delAction(){
        ...
        if (intval($_GET['link_id']) > 0){
            ...
            delete_file_cache('link',null);
            showMessage(删除成功,getUrl('system_manager/link/link'));
        }else {
            showMessage(删除失败,getUrl('system_manager/link/link'));
        }
    }
        ...
        if ($_POST['form_submit'] == 'ok'){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>getUrl('system_manager/link/link_add'),
                            'msg'=>再次发布,
                        ),
                        array(
                            'url'=>getUrl('system_manager/link/link'),
                            'msg'=>返回列表,
                        )
                    );
        ...
        if ($_POST['form_submit'] == 'ok'){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>getUrl('system_manager/link/link_edit',array('link_id'=>intval($_POST['link_id']))),
                            'msg'=>再次编辑
                        ),
                        array(
                            'url'=>getUrl('system_manager/link/link'),
