 ...
 * 推荐位
 *
 * Class RecPositionController
 * @package Ypk\Modules\SystemManager\Controllers
 */
class RecPositionController extends ControllerBase
{
    public function initialize()
    {
        parent::initialize();
        $this->translation = getTranslation('common,layout,rec_position');
...
//        Tpl::showpage('rec_position.index');
        $this->view->pick('rec_position/rec_position_index');
    }
    ...
    public function get_xmlAction()
    {
        ...
        foreach ($list as $k => $info) {
            ...
            $operation_detail = "<li><a href='" . getUrl('system_manager/rec_position/rec_edit', array('rec_id' => $info['rec_id'])) . "'>编辑内容</a></li>";
            $operation_detail .= "<li><a href=\"javascript:void(0);\" rec_id=\"{$info['rec_id']}\" nctype=\"jscode\"></i>调用代码</a></li>";
            $operation_detail .= "<li><a href='" . getUrl('system_manager/rec_position/rec_view', array('rec_id' => $info['rec_id'])) . "' target=\"_blank\">预览效果</a></li>";
            if ($operation_detail) {
     ...
     *
     */
    public function rec_addAction()
    {
//        Tpl::setDirquna('system');
//        Tpl::showpage('rec_position.add');
        $this->view->pick('rec_position/rec_position_add');
    }
...
//        Tpl::showpage('rec_position.edit');
        $this->view->pick('rec_position/rec_position_edit');
    }
            ...
            if (is_array($_GET['rec_id'])) {
                foreach ($_GET['rec_id'] as $rec_id) {
                    ...
                    if ($result) {
                        ...
                        delete_file_cache("rec_position/{$info['rec_id']}");
                        $this->log(getLang('nc_del,rec_position') . '[ID:' . $rec_id . ']', 1);
                    } else {
                        exit(json_encode(array('state' => false, 'msg' => '删除失败')));
                    }
                }
     ...
     *
     */
    public function rec_saveAction()
    {
        $array = array();
        $data = array();
            ...
            if (is_array($_FILES['pic']['tmp_name'])) {
                foreach ($_FILES['pic']['tmp_name'] as $k => $v) {
                    ...
                    if (in_array($ext, array('jpg', 'jpeg', 'gif', 'png'))) {
                        ...
                        if ($_FILES['pic']['error'][$k] != 0) showMessage(getLang('nc_common_op_fail'));
                        $c = count($array['body']);
                    ...
                    if (empty($array)) showMessage(getLang('param_error'));
                }
            }
        ...
        $model = Model('rec_position');
        $model->insert($data);
        $this->log(getLang('nc_add,rec_position') . '[' . $_POST['rtitle'] . ']', 1);
        showMessage(getLang('nc_common_save_succ'), getUrl('system_manager/rec_position/rec_list'));
    }
     ...
     *
     */
    public function rec_edit_saveAction()
    {
        if (!is_numeric($_POST['rec_id'])) showMessage(getLang('param_error'));
        $array = array();
        $data = array();
        $pattern = "/^http:\/\/[A-Za-z0-9]+[A-Za-z0-9.]+\.[A-Za-z0-9]+/i";
        //文字类型
            ...
            if (is_array($_FILES['pic']['tmp_name'])) {
                foreach ($_FILES['pic']['tmp_name'] as $k => $v) {
                    ...
                    if (in_array($ext, array('jpg', 'jpeg', 'gif', 'png', 'bmp'))) {
                        ...
                        if ($_FILES['pic']['error'][$k] != 0) showMessage(getLang('nc_common_save_fail'));
        ...
        if ($result) {
            ...
            delete_file_cache("rec_position/{$_POST['rec_id']}");
            showMessage(getLang('nc_common_save_succ'), getUrl('system_manager/rec_position/rec_list'));
        } else {
            showMessage(getLang('nc_common_save_fail'), getUrl('system_manager/rec_position/rec_list'));
        }
    }

    public function rec_codeAction()
    {
//        Tpl::setDirquna('system');
//        Tpl::showpage('rec_position.code','null_layout');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->pick('rec_position/rec_position_code');
    }

    public function rec_viewAction()
    {
        @header("Content-type: text/html; charset=" . CHARSET);
        echo rec_position(intval($_GET['rec_id']));
    }
}
