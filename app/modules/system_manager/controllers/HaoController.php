...
class HaoController extends ControllerBase
{
    private $links = array(
        array('url' => array('module' => 'system_manager', 'controller' => 'hao', 'action' => 'base'), 'lang' => 'hao_set'),
        array('url' => array('module' => 'system_manager', 'controller' => 'hao', 'action' => 'banner'), 'lang' => 'top_set'),
        array('url' => array('module' => 'system_manager', 'controller' => 'hao', 'action' => 'lc'), 'lang' => 'lc_set'),
        array('url' => array('module' => 'system_manager', 'controller' => 'hao', 'action' => 'sms'), 'lang' => 'sms_set'),
        array('url' => array('module' => 'system_manager', 'controller' => 'hao', 'action' => 'webchat'), 'lang' => 'webchat_set'),
       // array('url' => array('module' => 'ststem_manager', 'controller' => 'hao', 'action' => 'rc'), 'lang' => 'rc_set'),
    );
    public function initialize(){
        parent::initialize();
        $this->translation = getTranslation('layout,setting,common,hao');
    ...
    public function baseAction(){
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                $this->log(getLang('nc_edit,hao_set'),1);
                showMessage(getLang('nc_common_save_succ'));
            }else {
                $this->log(getLang('nc_edit,hao_set'),0);
    ...
    public function bannerAction(){
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                ...
                $this->log(getLang('nc_edit,top_set'),1);
                showMessage(getLang('nc_common_save_succ'));
            }else {
                $this->log(getLang('nc_edit,top_set'),0);
                showMessage(getLang('nc_common_save_fail'));
            }
        }
...
//        Tpl::setDirquna('system');
//        Tpl::showpage('hao.lc');
        $this->view->pick('hao/hao_lc');
    }

    /**
     * 楼层快速直达添加
     */
    public function lc_addAction() {
        $model_setting = Model('setting');
        $lc_info = $model_setting->getRowSetting('hao_lc');
        if ($lc_info !== false) {
            $lc_list = @unserialize($lc_info['value']);
        ...
        if (chksubmit()) {
            if (count($lc_list) >= 8) {
                showMessage('最多可设置8个楼层',getUrl('system_manager/hao/lc'));
            }
            ...
            if ($result){
                showMessage('保存成功',getUrl('system_manager/hao/lc'));
            }else {
                showMessage('保存失败');
            }
        }
...
//            Tpl::showpage('hao.lc_add');
            ...
            $result = $model_setting->updateSetting(array('hao_lc'=>serialize($lc_list)));
            if ($result){
                showMessage('编辑成功',getUrl('system_manager/hao/lc'));
            }
            showMessage('编辑失败');
        }
...
//        Tpl::showpage('hao.rc');
    ...
    public function rc_addAction() {
        ...
        if (chksubmit()) {
            if (count($rc_list) >= 8) {
                showMessage('最多可设置8个楼层',getUrl('system_manager/hao/rc'));
            }
            ...
            if ($result){
                showMessage('保存成功',getUrl('system_manager/hao/rc'));
            }else {
                showMessage('保存失败');
            }
        }
...
//        Tpl::showpage('hao.rc_add');
        $this->view->pick('hao/hao_rc_add');
    }

    /**
     * 删除
     */
    public function rc_delAction() {
        $model_setting = Model('setting');
        $rc_info = $model_setting->getRowSetting('hao_rc');
        if ($rc_info !== false) {
            $rc_list = @unserialize($rc_info['value']);
...
//            Tpl::showpage('hao.rc_add');
            ...
            $result = $model_setting->updateSetting(array('hao_rc'=>serialize($rc_list)));
            if ($result){
                showMessage('编辑成功',getUrl('system_manager/hao/rc'));
            }
            showMessage('编辑失败');
        }
    ...
    public function smsAction(){
        ...
        if (chksubmit()){
            ...
            $result = $model_setting->updateSetting($update_array);
            if ($result === true){
                $this->log(getLang('nc_edit,sms_set'),1);
                showMessage(getLang('nc_common_save_succ'));
            }else {
                $this->log(getLang('nc_edit,sms_set'),0);
                showMessage(getLang('nc_common_save_fail'));
            }
        }
        $list_setting = $model_setting->getListSetting();
...
//        Tpl::showpage('hao.webchat');
        $this->view->pick('hao/hao_webchat');
    }
}