...
class MemberController extends ControllerBase
{
    ...
    private $links = array(
        array('url' => array('module' => 'system_manager', 'controller' => 'member', 'action' => 'index'), 'lang' => 'member_index_manage'),
        //array('url' => array('module' => 'system_manager', 'controller' => 'member', 'action' => 'predeposit_add'), 'lang' => 'member_index_predeposit'),
    );

    public function initialize()
    {
        parent::initialize();
        getTranslation('member,layout,common');
    }
...
//        Tpl::showpage('member.index');
        $this->view->pick('member/member_index');
    }
        ...
        if (chksubmit()) {
            ...
            if ($error != '') {
                ...
                if ($result) {
                    $url = array(
                        array(
                            'url' => getUrl('system_manager/member/member'),
                            'msg' => $lang['member_edit_back_to_list'],
                        ),
                        array(
                            'url' => getUrl('system_manager/member/member_edit', array('member_id' => intval($_POST['member_id']))),
                            'msg' => $lang['member_edit_again'],
                        ),
                    );
                    $this->log(getLang('nc_edit,member_index_name') . '[ID:' . $_POST['member_id'] . ']', 1);
...
//        Tpl::showpage('member.edit');
        $this->view->pick('member/member_edit');
    }
            ...
            if ($error != '') {
                ...
                if ($result) {
                    $url = array(
                        ...
                        array(
                            'url' => getUrl('system_manager/member/member_add'),
                            'msg' => $lang['member_add_again'],
                        ),
                    );
                    $this->log(getLang('nc_add,member_index_name') . '[ ' . $_POST['member_name'] . ']', 1);
...
//        Tpl::showpage('member.add');
        $this->view->pick('member/member_add');
    }
...
//            }
    ...
    public function get_xmlAction()
    {
        ...
        foreach ($member_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn blue' href='" . getUrl('system_manager/member/member_edit', array('member_id' => $value['member_id'])) . "'><i class='fa fa-pencil-square-o'></i>编辑</a>
            <a class='btn blue' href='" . getUrl('system_manager/member/predeposit_add', array('member_id' => $value['member_id'], 'member_name' => $value['member_name'])) . "'><i class='fa fa-pencil-square-o'></i>存款</a>
    ...
    public function export_csvAction()
    {
        ...
        if (!is_numeric($_GET['curpage'])) {
            ...
            if ($count > self::EXPORT_SIZE) {   //显示下载链接
                ...
                Tpl::output('murl', getUrl('system_manager/member/index'));
//                Tpl::setDirquna('system');
//                Tpl::showpage('export.excel');
     ...
     * 添加余额控件
     */
    public function predeposit_addAction()
    {
        $model_member = Model('member');
        if (chksubmit()) {
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
                array("input" => $_POST["member_id"], "require" => "true", "message" => getLang('admin_points_member_error_again')),
                array("input" => $_POST["pointsnum"], "require" => "true", 'validator' => 'Compare', 'operator' => ' >= ', 'to' => 1, "message" => getLang('admin_points_points_min_error'))
            );
            $error = $obj_validate->validate();
            if ($error != '') {
            ...
            if (!is_array($member_info) || count($member_info) <= 0) {
                showMessage(getLang('admin_points_userrecord_error'), getUrl('system_manager/member/predeposit_add'), '', 'error');
            }
            ...
            if ($_POST['operatetype'] == 2 && $money > $available_predeposit) {
                showMessage(('预存款不足，会员当前预存款') . $available_predeposit, getUrl('system_manager/member/predeposit_add'), '', 'error');
            }
            if ($_POST['operatetype'] == 3 && $money > $available_predeposit) {
                showMessage(('可冻结预存款不足，会员当前预存款') . $available_predeposit, getUrl('system_manager/member/predeposit_add'), '', 'error');
            }
            if ($_POST['operatetype'] == 4 && $money > $freeze_predeposit) {
                showMessage(('可恢复冻结预存款不足，会员当前冻结预存款') . $freeze_predeposit, getUrl('system_manager/member/predeposit_add'), '', 'error');
            }
            ...
            switch ($_POST['operatetype']) {
                ...
                case 4:
                    ...
                    break;
                default:
                    showMessage('操作失败', getUrl('system_manager/member/predeposit_add'));
                    break;
            }
            try {
                ...
                if ($data['amount'] <= 999999) {
                    $model_pd->changePd($admin_act, $data); //增加或减少会员预存款
                    $model_pd->commit();
                    if ($admin_act == "sys_add_money") { //根据充值的预存款对会员进行升级
                    ...
                    $this->log($log_msg, 1);
                    showMessage('操作成功', getUrl('system_manager/member/index'));
                } else {
                    showMessage('添加金额不能大于999999', getUrl('system_manager/member/predeposit_add', array('member_name' => $_GET['member_name'], 'member_id' => $_GET['member_id'])));
                }
            } catch (\Exception $e) {
                $model_pd->rollback();
                $this->log($log_msg, 0);
                showMessage($e->getMessage(), getUrl('system_manager/member/predeposit_add', array('member_name' => $_GET['member_name'], 'member_id' => $_GET['member_id'])), 'html', 'error');
            }
        } else {
            //输出子菜单
            $condition['member_id'] = intval($_GET['member_id']);
...
//            Tpl::showpage('member.predeposit.add');
            $this->view->pick('member/member_predeposit_add');
        }
    }
