...
class NavigationController extends ControllerBase
{
    public function initialize(){
        parent::initialize();
        $this->translation = getTranslation('common,layout,navigation');
...
//        Tpl::showpage('navigation.index');
        $this->view->pick('navigation/navigation_index');
    }

    /**
     * 页面导航 添加
     */
    public function navigation_addAction(){
        $lang   = getTranslation();
        $model_navigation = Model('navigation');
        if (chksubmit()){
            /**
            ...
            if ($error != ''){
                ...
                if ($result){
                    delete_file_cache('nav');
                    $url = array(
                        array(
                            'url'=>getUrl('system_manager/navigation/navigation_add'),
                            'msg'=>$lang['navigation_add_again'],
                        ),
                        array(
                            'url'=>getUrl('system_manager/navigation/navigation'),
                            'msg'=>$lang['navigation_add_back_to_list'],
                        )
                    );
                    $this->log(getLang('navigation_add_succ').'['.$_POST['nav_title'].']',null);
...
//        Tpl::showpage('navigation.add');
        $this->view->pick('navigation/navigation_add');
    }

    /**
     * 页面导航 编辑
     */
    public function navigation_editAction(){
        $lang   = getTranslation();
        $model_navigation = Model('navigation');
        if (chksubmit()){
            /**
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>getUrl('system_manager/navigation/navigation_edit',array('nav_id'=>intval($_POST['nav_id']))),
                    ...
                    $this->log(getLang('navigation_edit_succ').'['.$_POST['nav_title'].']',null);
...
//        Tpl::showpage('navigation.edit');
        $this->view->pick('navigation/navigation_edit');
     ...
     * 删除页面导航
     */
    public function navigation_delAction(){
        $lang   = getTranslation();
        $model_navigation = Model('navigation');
        if (preg_match('/^[\d,]+$/', $_GET['nav_id'])) {
            ...
            delete_file_cache('nav');
            $this->log(getLang('article_index_del_succ').'[ID:'.implode(',',$_GET['nav_id']).']',null);
            showMessage($lang['navigation_index_del_succ'],getUrl('system_manager/navigation/navigation'));
        }else {
            showMessage($lang['navigation_index_choose_del'],getUrl('system_manager/navigation/navigation'));
        }
    }
