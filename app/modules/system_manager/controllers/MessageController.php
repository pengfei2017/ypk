...
//        array('url'=>'act=message&op=email_tpl','lang'=>'email_tpl'),
        array('url' => array('module' => 'system_manager', 'controller' => 'message', 'action' => 'email'), 'lang' => 'email_set'),
        array('url' => array('module' => 'system_manager', 'controller' => 'message', 'action' => 'email_tpl'), 'lang' => 'email_tpl'),
    );

    public function initialize()
    {
        parent::initialize();
        $this->translation = getTranslation('common,layout,message,setting');
    ...
    public function emailAction()
    {
        ...
        if (chksubmit()) {
            ...
            if ($result === true) {
                $this->log(getLang('nc_edit,email_set'), 1);
                showMessage(getLang('nc_common_save_succ'));
            } else {
                $this->log(getLang('nc_edit,email_set'), 0);
...
//        Tpl::showpage('message.email');
        $this->view->pick('message/message_email');
    }

    /**
     * 邮件模板列表
     */
    public function email_tplAction()
    {
...
//        Tpl::showpage('message.email_tpl');
        $this->view->pick('message/message_email_tpl');
    }

    /**
     * 编辑邮件模板
     */
    public function email_tpl_editAction()
    {
        $model_templates = Model('mail_templates');
        if (chksubmit()) {
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
                array("input" => $_POST["code"], "require" => "true", "message" => getLang('mailtemplates_edit_no_null')),
                array("input" => $_POST["title"], "require" => "true", "message" => getLang('mailtemplates_edit_title_null')),
                array("input" => $_POST["content"], "require" => "true", "message" => getLang('mailtemplates_edit_content_null')),
            );
            ...
            if ($error != '') {
                ...
                if ($result === true) {
                    $this->log(getLang('nc_edit,email_tpl'), 1);
                    showMessage(getLang('mailtemplates_edit_succ'), getUrl('system_manager/message/email_tpl'));
                } else {
                    $this->log(getLang('nc_edit,email_tpl'), 0);
                    showMessage(getLang('mailtemplates_edit_fail'));
                }
            }
        }
        if (empty($_GET['code'])) {
            showMessage(getLang('mailtemplates_edit_code_null'));
        }
...
//        Tpl::showpage('message.email_tpl.edit');
        $this->view->pick('message/message_email_tpl_edit');
    }
     ...
     * @return
     */
    public function email_testingAction()
    {
        /**
         * 读取语言包
         */
