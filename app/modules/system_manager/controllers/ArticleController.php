...
class ArticleController extends ControllerBase
{
    ...
    {
        parent::initialize();
        $this->translation = getTranslation('layout,common,article');
...
//        Tpl::showpage('article.index');
        $this->view->pick('article/article_index');
    }
    ...
    public function deleteAction()
    {
        ...
        if (preg_match('/^[\d,]+$/', $_GET['del_id'])) {
            ...
            $this->log(getLang('article_index_del_succ') . '[ID:' . implode(',', $_GET['del_id']) . ']', null);
    ...
    public function get_xmlAction()
    {
        ...
        if (is_array($article_list)) {
            ...
            foreach ($article_list as $k => $v) {
                $list = array();
                $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['article_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='" . getUrl('system_manager/article/article_edit', array('article_id' => $v['article_id'])) . "'><i class='fa fa-pencil-square-o'></i>编辑</a>";
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != '') {
                ...
                if ($result) {
                    ...
                    $url = array(
                        array(
                            'url' => getUrl('system_manager/article/article'),
                            'msg' => "{$lang['article_add_tolist']}",
                        ),
                        array(
                            'url' => getUrl('system_manager/article/article_add', array('ac_id' => intval($_POST['ac_id']))),
                            'msg' => "{$lang['article_add_continueadd']}",
                        ),
                    );
                    $this->log(getLang('article_add_ok') . '[' . $_POST['article_title'] . ']', null);
...
//        Tpl::showpage('article.add');
        $this->view->pick('article/article_add');
    }

    /**
     * 文章编辑
     */
    public function article_editAction()
    {
        $lang = getTranslation();
        $model_article = Model('article');
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != '') {
                ...
                if ($result) {
                    ...
                    $url = array(
                        ...
                        array(
                            'url' => getUrl('system_manager/article/article_edit', array('article_id' => intval($_POST['article_id']))),
                            'msg' => $lang['article_edit_edit_again'],
                        ),
                    );
                    $this->log(getLang('article_edit_succ') . '[' . $_POST['article_title'] . ']', null);
        ...
        $this->view->pick('article/article_edit');
...
//        Tpl::showpage('article.edit');
     ...
     * 文章图片上传
     */
    public function article_pic_uploadAction()
    {
        /**
         * 上传图片
         */
