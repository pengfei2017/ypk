...
use Ypk\Logic\SearchLogic;
use Ypk\QueueClient;
use Ypk\Tpl;

class SearchController extends BaseHomeController
{
    ...
    public function initialize()
    {
        ...
        parent::initialize();
        $this->translation = getTranslation('home_goods_class_index');
    ...
    public function indexAction()
    {
        ...
        if (intval($_GET['cate_id']) > 0) {
            ...
            setMyCookie('his_sh', implode('~', $his_sh_list), 2592000);
            //从TAG中查找分类
        ...
        if (!is_null($goods_list)) {
            ...
            if (!empty($_REQUEST['doctor_id'])) { //医生id
                $condition['doctor_id'] = $_REQUEST['doctor_id'];
            }
            //消费者保障服务
            ...
            if ($goods_class[$default_classid]['show_type'] == 1) {
                ...
                $goods_list = $model_goods->getGoodsOnlineList($condition, $fields, self::PAGESIZE, $order, 0, '', false, $count);
            }
        }
        ...
        if ($_GET['keyword'] == '') {
            ...
            if (is_numeric($_GET['cate_id']) && empty($_GET['keyword'])) {
                ...
                if (empty($seo_info[1])) {
                    $seo_info[1] = getConfig('site_name') . ' - ' . $seo_class_name;
                }
                Model('seo')->type($seo_info)->param(array('name' => $seo_class_name))->show();
            }
        } elseif ($_GET['keyword'] != '') {
            Tpl::output('html_title', (empty($_GET['keyword']) ? '' : $_GET['keyword'] . ' - ') . getConfig('site_name') . getLang('nc_common_search'));
        }
     ...
     * 获得推广商品
     */
    public function get_booth_goodsAction()
    {
        $gc_id = $_GET['cate_id'];
        if ($gc_id <= 0) {
            return false;
        ...
        $this->view->render('no_layout', 'goods_booth');
        $this->view->disable();
    }
     ...
     * 获得猜你喜欢
     */
    public function get_guesslikeAction()
    {
        $goodslist = Model('goods_browse')->getGuessLikeGoods(getSession('member_id'), 20);
        if (!empty($goodslist)) {
            Tpl::output('goodslist', $goodslist);
            //Tpl::showpage('goods_guesslike','null_layout');
        }
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('no_layout', 'goods_guesslike');
        $this->view->disable();
    }
     ...
     * 商品分类推荐
     */
    public function get_gc_goods_recommendAction()
    {
        ...
        if ($rec_gc_id > 0) {
            ...
            if (!empty($rec_list)) {
                ...
                if (!empty($goods_list)) {
                    ...
                    $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
                    $this->view->render('no_layout', 'goods_recommend');
                    $this->view->disable();
                }
            }
        }
