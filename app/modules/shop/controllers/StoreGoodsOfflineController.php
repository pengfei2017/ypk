...
class StoreGoodsOfflineController extends BaseSellerController
{
    public function initialize()
    {
        parent::initialize();
        getTranslation('member_store_goods_index,common,layout');
    }

    public function indexAction()
    {
        $this->goods_storageAction();
    }

    /**
     * 仓库中的商品列表
     */
    public function goods_storageAction()
    {
...
//                Tpl::showpage('store_goods_list.offline_lockup');
                $this->view->pick('store_goods_offline/store_goods_list_offline_lockup');
                break;
            // 等待审核或审核失败的商品
            case 'wait_verify':
                Tpl::output('verify', array('0' => '未通过', '10' => '等待审核'));
                //Tpl::showpage('store_goods_list.offline_waitverify');
                $this->view->pick('store_goods_offline/store_goods_list_offline_waitverify');
                break;
            // 仓库中的商品
            default:
                //Tpl::showpage('store_goods_list.offline');
                $this->view->pick('store_goods_offline/store_goods_list_offline');
                break;
        }
    }
     ...
     * 商品上架
     */
    public function goods_showAction()
    {
        ...
        if ($this->store_info['store_state'] != 1) {
            showDialog(getLang('store_goods_index_goods_show_fail') . '，店铺正在审核中或已经关闭', '', 'error');
        }
        ...
        if ($result['state']) {
            showDialog(getLang('store_goods_index_goods_show_success'), 'reload', 'succ');
        } else {
            showDialog(getLang('store_goods_index_goods_show_fail'), '', 'error');
        }
    }
    ...
    {
        $menu_array = array(
            array('menu_key' => 'goods_storage', 'menu_name' => getLang('nc_member_path_goods_storage'), 'menu_url' => getUrl('shop/store_goods_offline/index')),
            array('menu_key' => 'goods_lockup', 'menu_name' => getLang('nc_member_path_goods_state'), 'menu_url' => getUrl('shop/store_goods_offline/index', array('type' => 'lock_up'))),
            array('menu_key' => 'goods_verify', 'menu_name' => getLang('nc_member_path_goods_verify'), 'menu_url' => getUrl('shop/store_goods_offline/index', array('type' => 'wait_verify')))
        );
