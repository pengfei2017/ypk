...
class MemberSecurityController extends BaseMemberController
{
    ...
    public function initialize()
    {
        ...
        $this->view->setLayout("member_security");
    }
    ...
    public function indexAction()
    {
        ...
        $this->view->render('member_security','member_security_index');
        $this->view->disable();
    }
     ...
     * 绑定邮箱 - 发送邮件
     */
    public function send_bind_emailAction()
    {
        ...
        //发送频率验证
        $member_common_info = $model_member->getMemberCommonInfo(array('member_id' => getSession('member_id')));
        ...
        $condition['member_id'] = array('neq', getSession('member_id'));
        ...
        $update = $model_member->editMember(array('member_id' => getSession('member_id')), $data);
        if (!$update) {
        ...
        $update = $model_member->editMemberCommon($data, array('member_id' => getSession('member_id')));
        if (!$update) {
            showDialog('系统发生错误，如有疑问请与管理员联系');
        }
        $uid = base64_encode(encrypt(getSession('member_id') . ' ' . $_POST["email"]));
        $verify_url = getDomainName().getUrl('member/login/bind_email', array('uid' => $uid, 'hash' => md5($seed)));

        $model_tpl = Model('mail_templates');
        $tpl_info = $model_tpl->getTplInfo(array('code' => 'bind_email'));
        $param = array();
        $param['site_name'] = getConfig('site_name');
        $param['user_name'] = getSession('member_name');
        ...
        showDialog('验证邮件已经发送至您的邮箱，请于24小时内登录邮箱并完成验证！如果您始终未收到邮件，请于60秒后重新发送', getUrl('shop/member_security/index'), 'succ', '', 10);
    ...
    public function authAction()
    {
        ...
        if (chksubmit(false, true)) {
            if (!in_array($_POST['type'], array('modify_pwd', 'modify_mobile', 'modify_email', 'modify_paypwd', 'pd_cash'))) {
                redirect(getUrl('shop/member_security/index'));
            }
            $member_common_info = $model_member->getMemberCommonInfo(array('member_id' => getSession('member_id')));
            ...
            setMyCookie('seccode' . $_POST['nchash'], '', -3600);
            setSession('auth_' . $_POST['type'], TIMESTAMP);

            self::profile_menu($_POST['type'], $_POST['type']);
            if ($_POST['type'] == 'pd_cash') {
                //Tpl::showpage('member_pd_cash.add');
                $this->view->render('member_security','member_pd_cash_add');
            } else {
                //Tpl::showpage('member_security.' . $_POST['type']);
                $this->view->render('member_security','member_security_' . $_POST['type']);
            }
        }
        else {
            ...
            $member_info = $this->member_info;
            if (!$member_info) {
                $member_info = $model_member->getMemberInfo(array('member_id' => getSession('member_id')), 'member_email,member_email_bind,member_mobile,member_mobile_bind');
            }
            ...
            if (($_GET['type'] == 'modify_email' && $member_info['member_email_bind'] == '0') ||
                ...
                setSession('auth_' . $_GET['type'], TIMESTAMP);
                //Tpl::showpage('member_security.' . $_GET['type']);
                $this->view->render('member_security','member_security_' . $_GET['type']);
                exit;
            }
            ...
            if (in_array($_GET['type'], array('modify_pwd', 'modify_paypwd')) && $member_info['member_email_bind'] == '0' &&
                ...
                //showMessage('请先绑定邮箱或手机', getUrl('shop/member_security/index'), 'html', 'error');
                showMessage('请先绑定手机', getUrl('shop/member_security/index'), 'html', 'error');
            }

            Tpl::output('member_info', $member_info);

            //Tpl::showpage('member_security.auth');
            $this->view->render('member_security','member_security_auth');
        }
        $this->view->disable();
    }
     ...
     * 统一发送身份验证码
     */
    public function send_auth_codeAction()
    {
        if (!in_array($_GET['type'], array('email', 'mobile'))) exit();

        $model_member = Model('member');
        $member_info = $model_member->getMemberInfoByID(getSession('member_id'), 'member_email,member_mobile');
        ...
        if ($result) {
            ...
            $update = $model_member->editMemberCommon($update_data, array('member_id' => getSession('member_id')));
            if (!$update) {
     ...
     * 修改密码
     */
    public function modify_pwdAction()
    {
        $model_member = Model('member');

        //身份验证后，需要在30分钟内完成修改密码操作
        if (TIMESTAMP - getSession('auth_modify_pwd') > 1800) {
            showDialog('操作超时，请重新获得验证码', getUrl('shop/member_security/auth', array('type' => 'modify_pwd')), 'html', 'error');
        }
        ...
        $update = $model_member->editMember(array('member_id' => getSession('member_id')), array('member_passwd' => md5($_POST['password'])));
        $message = $update ? '密码修改成功' : '密码修改失败';
        $se = getSession('auth_modify_pwd');
        unset($se);
        showDialog($message, getUrl('shop/member_security/index'), $update ? 'succ' : 'error');
     ...
     * 设置支付密码
     */
    public function modify_paypwdAction()
    {
        $model_member = Model('member');

        //身份验证后，需要在30分钟内完成修改密码操作
        if (TIMESTAMP - getSession('auth_modify_paypwd') > 1800) {
            showMessage('操作超时，请重新获得验证码', getUrl('shop/member_security/auth', array('type' => 'modify_paypwd')), 'html', 'error');
        }
        ...
        $update = $model_member->editMember(array('member_id' => getSession('member_id')), array('member_paypwd' => md5($_POST['password'])));
        $message = $update ? '密码设置成功' : '密码设置失败';
        $se = getSession('auth_modify_paypwd');
        unset($se);
        showDialog($message, getUrl('shop/member_security/index'), $update ? 'succ' : 'error');
     ...
     * 绑定手机
     */
    public function modify_mobileAction()
    {
        $model_member = Model('member');
        $member_info = $model_member->getMemberInfoByID(getSession('member_id'), 'member_mobile_bind');
        if (chksubmit()) {
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
            ...
            $update = $model_member->editMember(array('member_id' => getSession('member_id')), array('member_mobile_bind' => 1));
            if (!$update) {
                showDialog('系统发生错误，如有疑问请与管理员联系');
            }
            showDialog('手机号绑定成功', getUrl('shop/member_security/index'), 'succ');
        }
    }
     ...
     * 修改手机号 - 发送验证码
     */
    public function send_modify_mobileAction()
    {
        $obj_validate = new Validate();
        $obj_validate->validateparam = array(
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array = array(
                    array('menu_key' => 'index', 'menu_name' => '账户安全', 'menu_url' => getUrl('shop/member_security/index')),
                );
                break;
            case 'modify_pwd':
                $menu_array = array(
                    array('menu_key' => 'index', 'menu_name' => '账户安全', 'menu_url' => getUrl('shop/member_security/index')),
                    array('menu_key' => 'modify_pwd', 'menu_name' => '修改登录密码', 'menu_url' => getUrl('shop/member_security/auth', array('type' => 'modify_pwd'))),
                );
                break;
            case 'modify_email':
                $menu_array = array(
                    array('menu_key' => 'index', 'menu_name' => '账户安全', 'menu_url' => getUrl('shop/member_security/index')),
                    array('menu_key' => 'modify_email', 'menu_name' => '邮箱验证', 'menu_url' => getUrl('shop/member_security/auth', array('type' => 'modify_email'))),
                    );
                break;
            case 'modify_mobile':
                $menu_array = array(
                    array('menu_key' => 'index', 'menu_name' => '账户安全', 'menu_url' => getUrl('shop/member_security/index')),
                    array('menu_key' => 'modify_mobile', 'menu_name' => '手机验证', 'menu_url' => getUrl('shop/member_security/auth', array('type' => 'modify_mobile'))),
                    );
                break;
            case 'modify_paypwd':
                $menu_array = array(
                    array('menu_key' => 'index', 'menu_name' => '账户安全', 'menu_url' => getUrl('shop/member_security/index')),
                    array('menu_key' => 'modify_paypwd', 'menu_name' => '设置支付密码', 'menu_url' => getUrl('shop/member_security/auth', array('type' => 'modify_paypwd'))),
                    );
                break;
            case 'pd_cash':
                $menu_array = array(
                    ...
                    array('menu_key' => 'pd_cash', 'menu_name' => '提现申请', 'menu_url' => getUrl('shop/member_security/auth', array('type' => 'pd_cash'))),
                    );
                break;
        }
