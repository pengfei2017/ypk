...
class MemberFavoriteGoodsController extends BaseMemberController
{

    public function initialize()
    {
        parent::initialize();
        getTranslation('member_layout,member_member_favorites');
    }
     ...
     * 增加商品收藏
     */
    public function favoritegoodsAction()
    {
        $fav_id = intval($_GET['fid']);
        if ($fav_id <= 0) {
            echo json_encode(array('done' => false, 'msg' => getLang('favorite_collect_fail', 'UTF-8')));
            die;
        }
        $favorites_model = Model('favorites');
        //判断是否已经收藏
        $favorites_info = $favorites_model->getOneFavorites(array('fav_id' => "$fav_id", 'fav_type' => 'goods', 'member_id' =>  getSession('member_id') ));
        if (!empty($favorites_info)) {
            echo json_encode(array('done' => false, 'msg' => getLang('favorite_already_favorite_goods', 'UTF-8')));
            die;
        }
        //判断商品是否为当前会员所有
        ...
        if ($goods_info['store_id'] == getSession('store_id')) {
            echo json_encode(array('done' => false, 'msg' => getLang('favorite_no_my_product', 'UTF-8')));
            die;
        }
        //添加收藏
        $insert_arr = array();
        $insert_arr['member_id'] = getSession('member_id');
        $insert_arr['member_name'] = getSession('member_name');
        ...
        if ($result) {
            ...
            echo json_encode(array('done' => true, 'msg' => getLang('favorite_collect_success', 'UTF-8')));
            die;
        } else {
            echo json_encode(array('done' => false, 'msg' => getLang('favorite_collect_fail', 'UTF-8')));
            die;
    ...
    public function fglistAction()
    {
        ...
        if (!empty($favorites_list) && is_array($favorites_list)) {
            ...
            $goods_model = Model('goods');
            $fields = 'goods.goods_id,goods.goods_name,goods.store_id,goods.goods_image,goods.goods_price,goods.goods_promotion_price,goods.goods_state,goods.goods_verify,goods.evaluation_count,goods.goods_salenum,goods.goods_collect,'
            ...
            $store_array = array();//医生编号
            if (!empty($goods_list) && is_array($goods_list)) {
                $store_goods_list = array();//医生为分组的商品
            ...
            $store_favorites = array();//医生收藏信息
            $voucher_template = array();
            if (!empty($store_array) && is_array($store_array)) {
                $store_list = $favorites_model->getStoreFavoritesList(array('member_id' => getSession('member_id'), 'fav_id' => array('in', $store_array)));
        ...
        self::profile_menu('favorites',($_GET['show']=='store') ? 'fav_goodsstore' : 'fav_goods');
        ...
        if ($show == 'pic' && !empty($_GET['curpage'])) {
            ...
            $this->view->render('no_layout', 'favorites_goods_picshowlist_item');
            $this->view->disable();
        } else {
            //Tpl::showpage($show_type);
            $this->view->render('member_favorite_goods', $show_type);
            $this->view->disable();
        }
    }
    ...
    public function delfavoritesAction()
    {
        if (!$_GET['fav_id'] || !$_GET['type']) {
            showDialog(getLang('member_favorite_del_fail'), '', 'error');
        }
        if (!preg_match_all('/^[0-9,]+$/', $_GET['fav_id'], $matches)) {
            showDialog(getLang('wrong_argument'), '', 'error');
        }
        ...
        if (!empty($fav_arr) && is_array($fav_arr)) {
            $favorites_list = $favorites_model->getFavoritesList(array('fav_id' => array('in', $fav_arr), 'fav_type' => "$type", 'member_id' => getSession('member_id')));
            if (!empty($favorites_list) && is_array($favorites_list)) {
                $fav_arr = array();
                foreach ($favorites_list as $k => $v) {
                    $fav_arr[] = $v['fav_id'];
                }
                $result = $favorites_model->delFavorites(array('fav_id' => array('in', $fav_arr), 'fav_type' => "$type", 'member_id' => getSession('member_id') ));
                if (!empty($fav_arr) && $result) {
                    //更新收藏数量
                    $goods_model = Model('goods');
                    $goods_model->editGoodsById(array('goods_collect' => array('exp', 'goods_collect - 1')), $fav_arr);
                    showDialog(getLang('favorite_del_success'), getUrl('shop/member_favorite_goods/fglist', array('show' => $_GET['show'])), 'succ');
                }
            } else {
                showDialog(getLang('favorite_del_fail'), '', 'error');
            }
        } else {
            showDialog(getLang('member_favorite_del_fail'), '', 'error');
        }
    ...
    public function log_msgAction()
    {
        ...
        if (chksubmit()) {
            if (empty($favorites)) {//检查状态,防止页面刷新不及时造成数据错误
                showDialog(getLang('wrong_argument'), 'reload', 'error', 'CUR_DIALOG.close();');
            }
            $fav_array = array();
            ...
            if ($state) {
                showDialog(getLang('nc_common_save_succ'), 'reload', 'succ', 'CUR_DIALOG.close();');
            } else {
                showDialog(getLang('nc_common_save_fail'), 'reload', 'error', 'CUR_DIALOG.close();');
            }
        }
        //Tpl::showpage('favorites_goods_msg','null_layout');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('no_layout', 'favorites_goods_msg');
        $this->view->disable();
    }
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'favorites':
                $menu_array = array(
                    array('menu_key' => 'fav_goods', 'menu_name' => getLang('nc_member_path_collect_list'), 'menu_url' => getUrl('shop/member_favorite_goods/fglist')),
                    array('menu_key' => 'fav_goodsstore', 'menu_name' => '同店商品', 'menu_url' => getUrl('shop/member_favorite_goods/fglist', array('show' => 'store'))),
                );
        }
