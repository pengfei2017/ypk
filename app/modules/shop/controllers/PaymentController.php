...
class PaymentController extends BaseHomeController
{

    public function initialize()
    {
        $lang = getTranslation('common,home_layout,home_cart_index');
     ...
     * 实物商品订单
     */
    public function real_orderAction()
    {
        $pay_sn = $_POST['pay_sn'];
        $payment_code = $_POST['payment_code'];
        $url = getUrl('shop/member_order');
        ...
        $order_pay_info = $logic_payment->getRealOrderInfo($pay_sn, getSession('member_id'));
        ...
        if (!empty($order_list)) {
            foreach ($order_list as $order_info) {
                if ($order_info['order_state'] == ORDER_STATE_NEW) { //获取已经提交但未支付的订单id集合
                    $api_pay_amount += $order_info['order_amount'] - $order_info['pd_amount'] - $order_info['rcb_amount'];
                    $pay_order_id_list[] = $order_info['order_id'];
                }
                $pay_amount += $order_info['order_amount'];//计算总金额
            }
        }
        if (empty($api_pay_amount)) {
            //跳转到支付成功页面
            redirect(getUrl('shop/buy/pay_ok', array('pay_sn' => $order_pay_info['data']['pay_sn'], 'pay_amount' => ncPriceFormat($pay_amount))));
        }
     ...
     * 虚拟商品进行支付
     */
    public function vr_orderAction()
    {
        $order_sn = $_POST['order_sn'];
        $payment_code = $_POST['payment_code']; //支付方式
        $url = getUrl('shop/member_vr_order');
        ...
        //计算所需支付金额等支付单信息
        $result = Logic('payment')->getVrOrderInfo($order_sn, getSession('member_id'));
        if (!$result['state']) {
            showMessage($result['msg'], $url, 'html', 'error');
        }
        ...
        if ($order_info['order_state'] == ORDER_STATE_PAY) { //表示站余额支付成功
            //发送兑换码到手机
     ...
     * 预存款充值
     */
    public function pd_orderAction()
    {
        $pdr_sn = $_POST['pdr_sn'];
        $payment_code = $_POST['payment_code'];
    ...
    private function _pd_pay($order_list, $post)
    {
        ...
        $buyer_info = $model_member->getMemberInfoByID(getSession('member_id'));
    ...
    private function _pd_vr_pay($order_info, $post)
    {
        ...
        if ($buyer_info['available_rc_balance'] == 0) { //判断充值卡有没有钱
            $post['rcb_pay'] = null;
        }
        if ($buyer_info['available_predeposit'] == 0) { //判断余额有没有钱
            $post['pd_pay'] = null;
        }
        ...
        try {
            ...
            //使用预存款支付，在pdPay函数内部产生兑换码并通过短信发送给客户
            if (!empty($post['pd_pay'])) {
    ...
    private function _api_pay($order_info, $payment_info)
    {
        ...
        if ($payment_info['payment_code'] == 'chinabank') {  //中国银联支付
            $payment_api->submit();
        } elseif ($payment_info['payment_code'] == 'wxpay') { //微信扫码支付
            ...
            if (array_key_exists('order_list', $order_info)) {
                ...
                Tpl::output('args', 'buyer_id=' . getSession('member_id') . '&pay_id=' . $order_info['pay_id']);
                $this->view->setVar('pay_sn', $order_info['pay_sn']);
            } elseif (array_key_exists('pdr_id', $order_info)) {
                //预存款充值
                Tpl::output('order_list', array($order_info));
                Tpl::output('args', 'buyer_id=' . getSession('member_id') . '&pdr_sn=' . $order_info['pdr_sn']);
            } else {
                //虚拟订单
                Tpl::output('order_list', array($order_info));
                Tpl::output('args', 'buyer_id=' . getSession('member_id') . '&order_id=' . $order_info['order_id']);
                $this->view->setVar('order_id', $order_info['order_id']);
                $this->view->setVar('order_sn', $order_info['order_sn']);
            }
            ...
            //微信扫码支付模式1  扫码支付模式一必须在设置【return_url】需要在微信公众平台的【微信支付】->【开发配置】->【扫码支付】->【支付回调URL】设置为同步通知页面url【http://180.76.141.156/shop/payment/wxpay_return】
            //Tpl::output('pay_url', base64_encode(encrypt($payment_api->get_payurls(), MD5_KEY)));
            $this->view->setVar('nav_list', read_file_cache('nav', true));
            $this->view->setLayoutsDir(SHOP_LAYOUT_DIR);
            $this->view->setLayout("buy_layout");
            $this->view->setVar('pay_amount', $order_info['api_pay_amount']);
            $this->view->render('buy', 'payment_wxpay'); //跳转到微信扫码支付页面
            $this->view->disable();
        } else {
    ...
    {
        Log::record('notifyAction：----post------11111111' . json_encode($_POST));
        Log::record('notifyAction：-----get-----11111111' . json_encode($_GET));
        if (empty($_POST)) {
            //支付宝是用$_POST方式进行的异步通知，财付通是用$_GET方式进行的异步通知
            unset($_GET['_url']); //去掉phalcon自动添加的$_GET['_url']参数，要不验证支付宝签名不成功
            $_POST = $_GET;
            //Log::record('notifyAction22：----------11111111' . json_encode($_POST));
        }

        $extra_common_param = $_POST['extra_common_param'];
        if (isset($_POST['attach'])) {
            //如果是财付通支付，进行重新赋值，方便后面合并使用支付宝验证方法
            $extra_common_param = $_POST['attach'];
        }
        $extra_common_param = json_decode($extra_common_param, true);
        //Log::record('notifyAction：----------22222222' . json_encode($extra_common_param));
        switch ($extra_common_param['payment_code']) {
            case 'alipay':
                $success = 'success';
                $fail = 'fail';
                break;
        ...
        $order_type = $extra_common_param['order_type'];
        ...
        //取得支付方式
        $result = $logic_payment->getPaymentInfo($extra_common_param['payment_code']);
        if (!$result['state']) {
            exit($fail);
        }
        $payment_info = $result['data'];
        ...
        if ($result['state']) {
            ...
            if ($order_type == 'real_order') {
                ...
                $log_buyer_id = $order_pay_info['pdr_member_id'];
                $log_buyer_name = $order_pay_info['pdr_member_name'];
    ...
    public function returnAction()
    {
        ...
        //Log::record('returnAction：----get------11111111' . json_encode($_GET));
        $extra_common_param = $_GET['extra_common_param'];
        if (isset($_GET['attach'])) {
            //如果是财付通支付，进行重新赋值，方便后面合并使用支付宝验证方法
            $extra_common_param = $_GET['attach'];
        }
        $extra_common_param = json_decode($extra_common_param, true);
        //Log::record('returnAction：----------22222222' . json_encode($extra_common_param));
        $order_type = $extra_common_param['order_type'];
        if ($order_type == 'real_order') {
            $act = 'member_order';
        } elseif ($order_type == 'vr_order') {
        ...
        //支付成功后跳转
        if ($order_type == 'real_order') {
            $pay_ok_url = getUrl('shop/buy/pay_ok') . '?pay_sn=' . $out_trade_no . '&pay_amount=' . ncPriceFormat($api_pay_amount);
        } elseif ($order_type == 'vr_order') {
            $pay_ok_url = getUrl('shop/buy_virtual/pay_ok') . 'order_sn=' . $out_trade_no . '&order_id=' . $order_pay_info['order_id'] . '&order_amount=' . ncPriceFormat($api_pay_amount);
        } elseif ($order_type == 'pd_order') {
            $pay_ok_url = getUrl('shop/predeposit');
        }
    ...
    public function qrcodeAction()
    {
        ...
        $data = decrypt($data, MD5_KEY);
        require_once RESOURCE_SITE_URL . '/phpqrcode/phpqrcode.php';
        \QRcode::png($data);
    }
     ...
     */
    public function wxpay_returnAction()
    {
        ...
        //警告：native_notify.php不是扫码支付成功的通知页面，而是扫码支付模式一的扫码回调之后进行统一下单的回调页面，native_notify.php是在接到回调通知之后，用户进行统一下单支付，并返回支付信息以完成支付的页面，并不是支付成功的通知页面（见：native_notify.php）
        require_once BASE_API_PATH . '/payment/wxpay/native_notify.php';
    }

    /**
     * 支付成功，异步通知，更新订单状态
     */
    public function wxpay_notifyAction()
    {
        $result = Logic('payment')->getPaymentInfo('wxpay');
        if (!$result['state']) {
            Log::record('wxpay not found', 'RUN');
        ...
        new \wxpay($result['data'], array());
        require_once BASE_API_PATH . '/payment/wxpay/notify.php';
    }

    public function query_stateAction()
    {
        ...
        } elseif (!empty($_GET['pdr_sn'])) {
            $logic_payment = Logic('payment');
            $info = $logic_payment->getPdOrderInfo($_GET['pdr_sn'], intval($_GET['buyer_id']));
            exit(json_encode(array('state' => ($info['data']['pdr_payment_state'] == '1'), 'pay_sn' => $info['data']['pdr_sn'], 'type' => 'pd')));
        }
    }
}