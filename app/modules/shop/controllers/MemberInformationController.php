...
class MemberInformationController extends BaseMemberController
{
    ...
    {
        $lang = getTranslation('member_home_member');
        ...
        if (chksubmit()) {
            ...
            $update = $model_member->editMember(array('member_id' => getSession('member_id')), $member_array);
        ...
        Tpl::output('menu_sign_url', getUrl('shop/member_information/member'));
        Tpl::output('menu_sign1', 'baseinfo');
        //Tpl::showpage('member_profile');
        $this->view->pick('member_information/member_profile');
    }
    ...
    public function moreAction()
    {
        ...
        if (chksubmit()) {
            $model->table('sns_mtagmember')->where(array('member_id' => getSession('member_id')))->delete();
            if (!empty($_POST['mid'])) {
                $insert_array = array();
                foreach ($_POST['mid'] as $val) {
                    $insert_array[] = array('mtag_id' => $val, 'member_id' => getSession('member_id'), 'recommend' => 0);
                }
                $model->table('sns_mtagmember')->pk(array('mtag_id', 'member_id'))->insertAll($insert_array, true);
            }
            showDialog(getLang('nc_common_op_succ'), '', 'succ');
        }
        ...
        // 用户已添加标签列表。
        $mtm_array = $model->table('sns_mtagmember')->where(array('member_id' => getSession('member_id')))->select();
        $mtag_list = array();
        $mtm_list = array();
        ...
        $this->view->render('member_information', 'member_profile_more');
        $this->view->disable();
    }
    ...
    {
        if (!chksubmit()) {
            redirect(getUrl('shop/member_information/avatar'));
        }
        ...
        $this->view->render('member_information', 'member_profile_avatar');
        $this->view->disable();
    }
    ...
    public function cutAction()
    {
        if (chksubmit()) {
            ...
            $res = strpos($_POST['newfile'], "avatar_" . getSession('member_id') . "_new.");
            if ($res > 0) {
                redirect(getUrl('shop/member_information/avatar'));
            }
            $src = BASE_UPLOAD_PATH . DS . ATTACH_AVATAR . DS . $_POST['newfile'];
            $avatarfile = BASE_UPLOAD_PATH . DS . ATTACH_AVATAR . DS . "avatar_" . getSession('member_id') . ".jpg";

            //import('function.thumb');
            $cropped = resize_thumb($avatarfile, $src, $w, $h, $x1, $y1, $scale);
            @unlink($src);
            Model('member')->editMember(array('member_id' => getSession('member_id')), array('member_avatar' => 'avatar_' . getSession('member_id') . '.jpg'));
            setSession('avatar', 'avatar_' . getSession('member_id') . '.jpg');
            redirect(getUrl('shop/member_information/avatar'));
        }
    }
    ...
    {
        $model_member = Model('member');
        $member_info = $model_member->getMemberInfoByID(getSession('member_id'), 'member_avatar');
        // 重新设置头衔COOKIE
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'member':
                $menu_array = array(
                    1 => array('menu_key' => 'member', 'menu_name' => getLang('home_member_base_infomation'), 'menu_url' => getUrl('shop/member_information/member')),
                    2 => array('menu_key' => 'more', 'menu_name' => getLang('home_member_more'), 'menu_url' => getUrl('shop/member_information/more')),
                    5 => array('menu_key' => 'avatar', 'menu_name' => getLang('home_member_modify_avatar'), 'menu_url' => getUrl('shop/member_information/avatar')));
                break;
        }
