...
class StoreTransportController extends BaseSellerController
{
    ...
    public function listAction(){
        ...
        $this->view->pick('store_transport/store_transport_list');
    }
    ...
    public function addAction(){
        ...
        $this->view->pick('store_transport/store_transport_add');
    }
    ...
    public function deleteAction(){
        ...
        if ($transport['store_id'] != $_SESSION['store_id']){
            showMessage(getLang('transport_op_fail'),getReferer(),'html','error');
        }
        //查看是否正在被使用
        if ($model_transport->isUsing($id)){
            showMessage(getLang('transport_op_using'),getReferer(),'html','error');
        }
    ...
    public function cloneAction(){
        ...
        $transport = $model_transport->getTransportInfo(array('id'=>$id));
        unset($transport['id']);
        $transport['title'] .= getLang('transport_clone_name');
        ...
        try {
            ...
            if (!$insert) throw new \Exception(getLang('transport_op_fail'));
    ...
    public function saveAction(){
        ...
        if (is_numeric($_POST['transport_id'])){
            ...
            if (empty($_POST['special'])) showMessage(getLang('transport_op_fail'),getReferer(),'html','error');
            //新增
        ...
        if ($result){
            header('location: '.getUrl('shop/store_transport').'?type='.$_POST['type']);exit;
        }else{
            showMessage(getLang('transport_op_fail'),getReferer(),'html','error');
        }
    }
     ...
     *
     */
    public function offpay_areaAction()
    {
        ...
        $this->view->pick('store_transport/store_transport_offpay_area');
    }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            ...
            case 'offpay_area':
                $menu_array = array(
                    1=>array('menu_key'=>'transport',   'menu_name'=>getLang('nc_member_path_postage'),       'menu_url'=>getUrl('shop/store_transport/index')),
                );
                if (checkPlatformStore()) {
                    $menu_array[] = array(
                        'menu_key' => 'offpay_area',
                        'menu_name' => '货到付款地区',
                        'menu_url' => getUrl('shop/store_transport/offpay_area')
                    );
                }
                break;
        }
