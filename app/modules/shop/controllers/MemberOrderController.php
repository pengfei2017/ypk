...
class MemberOrderController extends BaseMemberController
{
    public function initialize()
    {
        parent::initialize();
        $lang = getTranslation('member_member_index');
    }
    ...
    public function indexAction()
    {
            ...
            if (is_array($order_info['extend_order_goods'])) {
                foreach ($order_info['extend_order_goods'] as $value) {
                    ...
                    if ($value['goods_type'] == 5) {
                        ...
                        $gc_id_1 = Goods::findFirst(array('conditions' => 'goods_id=' . $value['goods_id'], 'columns' => 'gc_id_1'));
                        if ($gc_id_1 !== false && !empty($gc_id_1['gc_id_1'])) {
                            $value['gc_id_1'] = $gc_id_1['gc_id_1'];
                        }
        ...
        $this->view->setVar('order_group_list', $order_group_list);
        $this->view->setVar('show_page', $model_order->showpage());
    ...
    public function search_deliverAction()
    {
        ...
        $this->view->render('member_order', 'member_order_deliver_detail');
        $this->view->disable();
    }
    ...
    public function show_orderAction()
    {
        ...
        $result = $logic_order->getMemberOrderInfo($_GET['order_id'], getSession('member_id'));
        if (!$result['state']) {
        ...
        $this->view->render('member_order', 'member_order_show');
        $this->view->disable();
    }
    ...
    private function _order_cancel($order_info, $post)
    {
        if (!chksubmit()) {
            ...
            $this->view->render('no_layout', 'member_order_cancel');
            exit();
        } else {
            $model_order = Model('order');
            $logic_order = Logic('order');
            ...
            if ($order_info['order_type'] != 2) {
                ...
                $result = $logic_order->changeOrderStateCancel($order_info, 'buyer', getSession('member_name'), $msg, true, $cancel_condition);
            } else {
                //取消预定订单
                $result = Logic('order_book')->changeOrderStateCancel($order_info, 'buyer', getSession('member_name'), $msg);
            }
            return $result;
        }
    }
    ...
    private function _order_receive($order_info, $post)
    {
        if (!chksubmit()) {
            ...
            $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
            $this->view->render('no_layout', 'member_order_receive'); //弹出是否收货的弹出框
            //$this->view->disable();
            exit();
        } else {
            $model_order = Model('order');
            ...
            if (!$if_allow) {
                return queue_callback(false, '正在确认，请稍候...');
            }
            return $logic_order->changeOrderStateReceive($order_info, 'buyer', getSession('member_name'), '签收了货物');
        }
    }
    ...
    private function _order_recycle($order_info, $get)
    {
        ...
        if (!$if_allow) {
            return queue_callback(false, '正在添加到回收站，请稍候...');
        }
    ...
    {
        $lang = getTranslation('member_layout');
        $menu_array = array(
            array('menu_key' => 'member_order', 'menu_name' => getLang('nc_member_path_order_list'), 'menu_url' => getUrl('shop/member_order')),
            array('menu_key' => 'member_order_recycle', 'menu_name' => '回收站', 'menu_url' => getUrl('shop/member_order', array('recycle' => 1))),
        );
