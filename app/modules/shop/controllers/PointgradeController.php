...
class PointgradeController extends BasePointShopController
{
    public function initialize()
    {
        parent::initialize();
        //验证是否登录
        if (getSession('is_login') != '1') {
            showDialog(getLang('no_login'), getUrl('member/login'), 'error');
        }
    }
    ...
    public function indexAction()
    {
        ...
        //分类导航
        $nav_link = array(
            0 => array('title' => getLang('homepage'), 'link' => SHOP_SITE_URL),
            1 => array('title' => getLang('nc_pointprod'), 'link' => getUrl('shop/pointshop/index')),
     ...
     * 经验明细列表
     */
    public function exppointlogAction()
    {
        //查询会员及其附属信息
        ...
        $where = array();
        $where['exp_memberid'] = getSession('member_id');
        ...
        $this->view->render('pointgrade', 'point_exppointslog');
    }
}