...
class StoreAlbumController extends BaseSellerController
{
    public function initialize()
    {
        parent::initialize();
        getTranslation('member_store_album,common,member_store_index');
    }
    ...
    public function album_cateAction()
    {
         ...
         * 验证是否存在默认相册
         */
        $return = $model_album->checkAlbum(array('album_aclass.store_id' => getSession('store_id'), 'is_default' => '1'));
        if (!$return) {
            $album_arr = array();
            $album_arr['aclass_name'] = getLang('album_default_album');
            $album_arr['store_id'] = getSession('store_id');
         ...
         */
        $param = array();
        $param['album_aclass.store_id'] = getSession('store_id');
...
//        Tpl::output('show_page', $page->show());
        ...
        $this->view->render('store_album', 'store_album_list');
        $this->view->disable();
    }
    ...
    public function album_addAction()
    {
         ...
         */
        $model_album = Model('album');
        $class_count = $model_album->countClass(getSession('store_id'));
        Tpl::output('class_count', $class_count['count']);
        //Tpl::showpage('store_album.class_add', 'null_layout');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('store_album', 'store_album_class_add');
        $this->view->disable();
    }
    ...
    public function album_add_saveAction()
    {
        if (chksubmit()) {
            ...
            if ($class_count['count'] >= 20) {
                showDialog(getLang('album_class_save_max_20'), getUrl('shop/store_album'), 'error', empty($_GET['inajax']) ? '' : 'CUR_DIALOG.close();');
            }
            /**
             * 实例化相册模型
             */
            ...
            if ($return) {
                showDialog(getLang('album_class_save_succeed'), getUrl('shop/store_album'), 'succ', empty($_GET['inajax']) ? '' : 'CUR_DIALOG.close();');
            }
        }
    ...
    public function album_edit_saveAction()
    {
         ...
         * 验证
         */
        $return = $model_album->checkAlbum(array('album_aclass.store_id' => getSession('store_id'), 'album_aclass.aclass_id' => intval($_POST['id'])));
        if ($return) {
            /**
             * 更新
             */
            ...
            if ($re) {
                showDialog(getLang('album_class_edit_succeed'), getUrl('shop/store_album'), 'succ', empty($_GET['inajax']) ? '' : 'CUR_DIALOG.close();');
            }
        } else {
    ...
    public function album_delAction()
    {
         ...
         * 验证
         */
        $return = $model_album->checkAlbum(array('album_aclass.store_id' => getSession('store_id'), 'album_aclass.aclass_id' => intval($_GET['id']), 'is_default' => '0'));
        if (!$return) {
        ...
        $param['value'] = array('1', getSession('store_id'));
        $class_info = $model_album->getOneClass($param);
        $param = array();
        $param['aclass_id'] = $class_info['aclass_id'];
        $return = $model_album->updatePic($param, array('aclass_id' => intval($_GET['id'])));
        if ($return) {
            showDialog(getLang('album_class_file_del_succeed'), getUrl('shop/store_album'), 'succ');
        } else {
            showDialog(getLang('album_class_file_del_lose'));
        }
    }
    ...
    public function album_pic_listAction()
    {
        ...
        $param['album_pic.store_id'] = getSession('store_id');
        if ($_GET['sort'] != '') {
            switch ($_GET['sort']) {
                case '0':
                    $param['order'] = 'upload_time desc';
        ...
        $this->view->render('store_album', 'store_album_pic_list');
        $this->view->disable();
    }
    ...
    public function pic_listAction()
    {
        ...
        if (!empty($_GET) && $_GET['id'] != '0') {
            ...
            $cparam['value'] = array(intval($_GET['id']), getSession('store_id'));
        ...
        switch ($_GET['item']) {
            case 'goods':
                ...
                $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
                $this->view->render('no_layout', 'store_goods_add_step2_master_image');
                $this->view->disable();
                break;
            case 'des':
                //Tpl::showpage('store_goods_add.step2_desc_image','null_layout');
                $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
                $this->view->render('no_layout', 'store_goods_add_step2_desc_image');
                $this->view->disable();
                break;
            case 'groupbuy':
                //Tpl::showpage('store_groupbuy.album','null_layout');
                $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
                $this->view->render('no_layout', 'store_groupbuy_album');
                $this->view->disable();
                break;
            case 'store_sns_normal':
                //Tpl::showpage('store_sns_add.album', 'null_layout');
                $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
                $this->view->render('no_layout', 'store_sns_add_album');
                $this->view->disable();
                break;
            case 'goods_image':
                Tpl::output('color_id', $_GET['color_id']);
                $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
                $this->view->render('no_layout', 'store_goods_add_step3_goods_image');
                $this->view->disable();
            ...
            case 'mobile':
                ...
                $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
                $this->view->render('no_layout', 'store_goods_add_step2_mobile_image');
                $this->view->disable();
    ...
    public function change_album_coverAction()
    {
        ...
        $return = $model_album->checkAlbum(array('album_aclass.store_id' => getSession('store_id'), 'album_aclass.aclass_id' => $pic_info['aclass_id']));
        if ($return) {
     ...
     * ajax修改图名称
     */
    public function change_pic_nameAction()
    {
    ...
    public function album_pic_infoAction()
    {
         ...
         * 验证
         */
        $return = $model_album->checkAlbum(array('album_pic.store_id' => getSession('store_id'), 'album_pic.apic_id' => intval($_GET['id'])));
        if (!$return) {
            showMessage(getLang('album_parameter_error'), '', 'html', 'error');
        }
        ...
        $param = array();
        $param['field'] = array('aclass_id', 'store_id');
        $param['value'] = array(intval($_GET['class_id']), getSession('store_id'));
        $class_info = $model_album->getOneClass($param);
        Tpl::output('class_info', $class_info);
        ...
        $this->view->render('store_album', 'store_album_pic_info');
        $this->view->disable();
    }
     ...
     * 图片 ajax
     */
    public function album_ad_ajaxAction()
    {
        if (empty($_GET['class_id']) && empty($_GET['id'])) {
            exit();
        }
        ...
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('store_album', 'store_album_pic_scroll_ajax');
        $this->view->disable();
    }
    ...
    public function album_pic_delAction()
    {
        ...
        $return = $model_album->checkAlbum(array('album_pic.store_id' => getSession('store_id'), 'in_apic_id' => $id));
        if (!$return) {
            showDialog(getLang('album_class_pic_del_lose'));
        }

        //删除图片
        $return = $model_album->delPic($id, getSession('store_id'));
        if ($return) {
     ...
     * 移动相册
     */
    public function album_pic_moveAction()
    {
        $model_album = Model('album');
        if (chksubmit()) {
            ...
            if ($return) {
                showDialog(getLang('album_class_pic_move_succeed'), 'reload', 'succ');
            } else {
                showDialog(getLang('album_class_pic_move_lose'));
            }
        }
        $param = array();
        ...
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('store_album', 'store_album_move');
        $this->view->disable();
    }
     ...
     * 替换图片
     */
    public function replace_image_uploadAction()
    {
        $file = $_GET['id'];
        ...
        $upload->set('default_dir', ATTACH_GOODS . DS . getSession('store_id') . DS . $pic_cover);
...
//            }
         ...
         */
        // 取得图像大小
        if (!getConfig('oss.open')) {
            list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH . DS . ATTACH_GOODS . DS . getSession('store_id') . DS . $apic_info['apic_cover']);
        } else {
            list($width, $height, $type, $attr) = getimagesize(getConfig('oss.img_url') . '/' . ATTACH_GOODS . '/' . getSession('store_id') . DS . $apic_info['apic_cover']);
        }
        /**
         * 更新图片分类
         */
     ...
     * 添加水印
     */
    public function album_pic_watermarkAction()
    {
        if (empty($_POST['id']) && !is_array($_POST['id'])) {
            showDialog(getLang('album_parameter_error'));
        }
        ...
        $store_wm_info = $model_store_wm->getOneStoreWMByStoreId(getSession('store_id'));
        if ($store_wm_info['wm_image_name'] == '' && $store_wm_info['wm_text'] == '') {
            showDialog(getLang('album_class_setting_wm'), getUrl('shop/store_album/store_watermark'), 'error', 'CUR_DIALOG.close();');//"请先设置水印"
        }
        ...
        foreach ($wm_list as $v) {
            $gd_image->create(BASE_UPLOAD_PATH . DS . ATTACH_GOODS . DS . getSession('store_id') . DS . str_ireplace('.', '_1280.', $v['apic_cover']));//生成有水印的大图
        }
        showDialog(getLang('album_pic_plus_wm_succeed'), 'reload', 'succ');
    }

    /**
     * 水印管理
     */
    public function store_watermarkAction()
    {
        $model_store_wm = Model('store_watermark');
        /**
         * 获取会员水印设置
        ...
        if (chksubmit()) {
            ...
            if ($result) {
                showDialog(getLang('store_watermark_congfig_success'), 'reload', 'succ');
            } else {
                showDialog(getLang('store_watermark_congfig_fail'));
            }
        }
        /**
         * 获取水印字体
...
//            }
        ...
        $this->view->render('store_album', 'store_watermark_form');
        $this->view->disable();
    }
...
//            }
        ...
        if ($album_limit > 0) {
            ...
            if ($album_count >= $album_limit) {
                // 目前并不出该提示，而是提示上传0张图片
                $error = getLang('store_goods_album_climit');
//                if (strtoupper ( CHARSET ) == 'GBK') {
//                    $error = getLang($error);
//                }
                $data['state'] = 'false';
...
//            }
        ...
        if (!getConfig('oss.open')) {
            ...
            list($width, $height, $type, $attr) = getimagesize(getConfig('oss.img_url') . '/' . ATTACH_GOODS . '/' . $store_id . DS . $pic);
        }
        $image = explode('.', $_FILES["file"]["name"]);
//        if (strtoupper(CHARSET) == 'GBK') {
//            $image['0'] = getLang($image['0']);
//        }
        $insert_array = array();
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'album':
                $menu_array = array(
                    1 => array('menu_key' => 'album', 'menu_name' => getLang('nc_member_path_my_album'), 'menu_url' => getUrl('shop/store_album')),
                    2 => array('menu_key' => 'watermark', 'menu_name' => getLang('nc_member_path_watermark'), 'menu_url' => getUrl('shop/store_album/store_watermark'))
                );
                break;
            case 'album_pic':
                $menu_array = array(
                    1 => array('menu_key' => 'album', 'menu_name' => getLang('nc_member_path_my_album'), 'menu_url' => getUrl('shop/store_album')),
                    3 => array('menu_key' => 'pic_list', 'menu_name' => getLang('nc_member_path_album_pic_list'), 'menu_url' => getUrl('shop/store_album/album_pic_list',array('id'=>intval($_GET['id'])))),
                    2 => array('menu_key' => 'watermark', 'menu_name' => getLang('nc_member_path_watermark'), 'menu_url' => getUrl('shop/store_album/store_watermark'))
                );
                break;
            case 'album_pic_info':
                $menu_array = array(
                    1 => array('menu_key' => 'album', 'menu_name' => getLang('nc_member_path_my_album'), 'menu_url' => getUrl('shop/store_album')),
                    3 => array('menu_key' => 'pic_info', 'menu_name' => getLang('nc_member_path_album_pic_info'), 'menu_url' => getUrl('shop/store_album/album_pic_info',array('id'=>intval($_GET['id']),'class_id'=>intval($_GET['class_id'])))),
                    2 => array('menu_key' => 'watermark', 'menu_name' => getLang('nc_member_path_watermark'), 'menu_url' =>getUrl('shop/store_album/store_watermark'))
                );
                break;
        }
    ...
    public function ajax_change_imgmessageAction()
    {
         ...
         * 小图尺寸
         */
        list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH . DS . ATTACH_GOODS . DS . getSession('store_id') . DS . $pic_info['0']['apic_cover']);
//        if(strtoupper(CHARSET) == 'GBK'){
//            $pic_info['0']['apic_name'] = getLang($pic_info['0']['apic_name']);
//        }
    ...
    public function ajax_check_class_nameAction()
    {
        ...
        $param['value'] = array($ac_name, getSession('store_id'));
        $class_info = $model_album->getOneClass($param);
        if (!empty($class_info)) {
            echo 'false';
            die;
