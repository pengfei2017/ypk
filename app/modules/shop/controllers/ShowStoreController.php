...
class ShowStoreController extends BaseStoreController
{
    ...
    public function indexAction()
    {
        if (!$this->store_decoration_only) {
            ...
            $new_goods_list = $model_goods->getGoodsOnlineList($condition, $fieldstr, 12, 'goods_id desc', 0, '', false, $count);

            $condition['goods_commend'] = 1;
            //得到12个推荐商品列表
            $count = $model_goods->getGoodsOnlineCount($condition, "distinct goods_commonid");
            $recommended_goods_list = $model_goods->getGoodsOnlineList($condition, $fieldstr, 12, 'goods_id desc', 0, '', false, $count);
        ...
        //显示医生的个人信息
        $store_join_model = StoreJoinin::findFirst('member_id=' . $this->store_info['member_id']);
        if ($store_join_model !== false) {
            Tpl::output('mail_content', $store_join_model->getMailContent());
        }
    }
    ...
    public function show_articleAction()
    {
        ...
        if (!empty($store_navigation_info) && is_array($store_navigation_info)) {
            ...
            $this->view->pick('show_store', 'article');
        }
    }
     ...
     * 全部商品
     */
    public function goods_allAction()
    {
    ...
    function cou_goodsAction()
    {
        ...
        $this->view->render('show_store', 'goods_list_cou');
    }

    /**
     * 满即送活动列表
     */
    function mansong_goodsAction()
    {
        ...
        $this->view->render('show_store', 'goods_list_mansong');
        $this->view->disable();
    }
     ...
     * ajax获取动态数量
     */
    function ajax_store_trend_countAction()
    {
     ...
     * ajax 医生流量统计入库
     */
    public function ajax_flowstat_recordAction()
    {
        ...
        //确定统计分表名称
        $last_num = $store_id % 10; //获取医生ID的末位数字
        $tablenum = ($t = intval(getConfig('flowstat_tablenum'))) > 1 ? $t : 1; //处理流量统计记录表数量
