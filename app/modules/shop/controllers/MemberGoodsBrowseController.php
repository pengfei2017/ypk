...
class MemberGoodsBrowseController extends BaseMemberController
{
    ...
    public function listAction()
    {
        ...
        if ($goodsid_arr) {
            $goods_list_tmp = Model('goods')->getGoodsList(array('goods_id' => array('in', $goodsid_arr)), 'goods_id, goods_name, goods_price,goods_promotion_price,goods_promotion_type, goods_marketprice, goods_image, store_id, gc_id, gc_id_1, gc_id_2, gc_id_3');
            $goods_list = array();
        ...
        //查询浏览记录商品分类
        $browseclass_list = $model->getGoodsbrowseList(array('member_id' => getSession('member_id')), 'gc_id, min(gc_id_1) as gc_id_1, min(gc_id_2) as gc_id_2, min(gc_id_3) as gc_id_3', 0, 0, 'gc_id desc', 'gc_id');
        ...
        $this->view->render('member_goodsbrowse', 'index');
        $this->view->disable();
    ...
    public function delAction()
    {
        ...
        if (trim($_GET['goods_id']) == 'all') {
            if ($model->delGoodsbrowse(array('member_id' => getSession('member_id')))) {
            ...
            if ($model->delGoodsbrowse(array('member_id' => getSession('member_id'), 'goods_id' => $goods_id))) {
                $return_arr = array('done' => true);
            } else {
                $return_arr = array('done' => false, 'msg' => '删除失败');
            }
