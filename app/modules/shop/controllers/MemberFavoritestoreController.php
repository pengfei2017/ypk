...
class MemberFavoritestoreController extends BaseMemberController
{
     ...
     * 增加医生收藏
     */
    public function favoritestoreAction()
    {
        ...
        //判断是否已经收藏
        $favorites_info = $favorites_model->getOneFavorites(array('fav_id' => "$fav_id", 'fav_type' => 'store', 'member_id' =>  getSession('member_id') ));
        if (!empty($favorites_info)) {
            echo json_encode(array('done' => false, 'msg' => getLang('favorite_already_favorite_store')));
            die;
        }
        //判断医生是否为当前会员所有
        if ($fav_id == getSession('store_id')) {
            echo json_encode(array('done' => false, 'msg' => getLang('favorite_no_my_store', 'UTF-8')));
            die;
        }
        //添加收藏
        $insert_arr = array();
    ...
    {
        $favorites_model = Model('favorites');
        $favorites_list = $favorites_model->getStoreFavoritesList(array('member_id' => getSession('member_id')), '*', 10);
        ...
        $this->view->render('member_favoritestore', 'favorites_store_index');
        $this->view->disable();
    }
    ...
    public function moreAction()
    {
        ...
        if (empty($_GET['curpage'])) {
            $store_info = array();
            $favorites_model = Model('favorites');
            $favorites_list = $favorites_model->getStoreFavoritesList(array('member_id' => getSession('member_id')), '*');
            if (!empty($favorites_list) && is_array($favorites_list)) {
                $favorites_id = array();//收藏的医生编号
                foreach ($favorites_list as $key => $favorites) {
                    $fav_id = $favorites['fav_id'];
        ...
        if (!empty($_GET['curpage'])) {
            ...
            $this->view->render('no_layout','favorites_store_index_more_item');
        } else {

            self::profile_menu('favorites', 'more');
            //Tpl::showpage('favorites_store_index.more');
            $this->view->render('member_favoritestore', 'favorites_store_index_more');
        }
        $this->view->disable();
    }
        ...
        if (!empty($fav_arr) && is_array($fav_arr)) {
            ...
            if (!empty($favorites_list) && is_array($favorites_list)) {
                ...
                if (!empty($fav_arr) && $result) {
                    ...
                    showDialog(getLang('favorite_del_success'), getUrl('shop/member_favorite_store/fslist'), 'succ');
                }
            } else {
    ...
    {
        $menu_array = array(
            array('menu_key' => 'fav_store', 'menu_name' => getLang('nc_member_path_collect_store'), 'menu_url' => getUrl('shop/member_favorite_store/fslist'))
        );
        if ($menu_key == 'more') {
            $menu_array[] = array('menu_key' => 'more', 'menu_name' => '收藏医生详细');
        }
