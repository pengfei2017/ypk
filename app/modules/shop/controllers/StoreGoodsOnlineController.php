...
class StoreGoodsOnlineController extends BaseSellerController
{
    public function initialize()
    {
        parent::initialize();
        getTranslation('member_store_goods_index');
    }

    public function indexAction()
    {
        $this->goods_listAction();
    }
    ...
    public function goods_listAction()
    {
        ...
        if (!getSession('seller_gc_limits') && getSession('seller_group_id')) {
            $gc_list = Model('seller_group_bclass')->getSellerGroupBclasList(array('group_id' => getSession('seller_group_id')), '', '', 'gc_id', 'gc_id');
        ...
        $this->view->render('store_goods_online', 'store_goods_list_online');
        $this->view->disable();
    }
     ...
     * 编辑商品页面
     */
    public function edit_goodsAction()
    {
        ...
        //获取商品类别id
        $gc_id = $goodscommon_info['gc_id_1'];
        if (intval($gc_id) == 1073) { //表示修改服务
            redirect(getUrl('shop/store_goods_add/serviceAdd', array('type' => 'edit', 'commonid' => $common_id)));
        }
        if (intval($gc_id) == 1076) { //表示修改聊天卡
            redirect(getUrl('shop/store_goods_add/addChatCard', array('type' => 'edit', 'commonid' => $common_id)));
        }
        ...
        $where = array('goods_commonid' => $common_id, 'store_id' => getSession('store_id'));
        ...
        // 获取类型相关数据
        $typeinfo = $model_type->getAttr($goods_class['type_id'], getSession('store_id'), $goodscommon_info['gc_id']);
        ...
        // 是否能使用编辑器
        if (checkPlatformStore()) { // 平台医生可以使用编辑器
        ...
        $this->view->render('store_goods_online', 'store_goods_add_step2');
        $this->view->disable();
    }
     ...
     * 编辑商品保存
     */
    public function edit_save_goodsAction()
    {
            ...
            if (empty($bind_info)) {
                ...
                if (empty($bind_info)) {
                    ...
                    if (empty($bind_info)) {
                        ...
                        $bind_info = $model_bind_class->getStoreBindClassInfo($where);
                        if (empty($bind_info)) {
                            showDialog(getLang('store_goods_index_again_choose_category2'));
                        }
                    }
        ...
        if ($result['state']) {
            //提交事务
            showDialog(getLang('nc_common_op_succ'), $_POST['ref_url'], 'succ');
        } else {
            //回滚事务
            showDialog($result['msg'], getUrl('shop/store_goods_online/index'));
        }
    }
     ...
     * 编辑图片
     */
    public function edit_imageAction()
    {
        $common_id = intval($_GET['commonid']);
        if ($common_id <= 0) {
            showMessage(getLang('wrong_argument'), getUrl('shop/seller_center'), 'html', 'error');
        }
        $model_goods = Model('goods');
        $common_list = $model_goods->getGoodsCommonInfoByID($common_id, 'store_id,goods_lock,spec_value,is_virtual,is_fcode,is_presell');
        if ($common_list['store_id'] != getSession('store_id') || $common_list['goods_lock'] == 1) {
            showMessage(getLang('wrong_argument'), getUrl('shop/seller_center'), 'html', 'error');
        }
        ...
        $this->view->render('store_goods_online', 'store_goods_add_step3');
        $this->view->disable();
    }
     ...
     * 保存商品图片
     */
    public function edit_save_imageAction()
    {
        if (chksubmit()) {
            $common_id = intval($_POST['commonid']);
            $rs = Logic('goods')->editSaveImage($_POST['img'], $common_id, getSession('store_id'), getSession('seller_id'), getSession('seller_name'));
            if ($rs['state']) {
                showDialog(getLang('nc_common_op_succ'), $_POST['ref_url'], 'succ');
            } else {
                showDialog(getLang('nc_common_save_fail'), getUrl('shop/store_goods_online/index'));
            }
        }
    }
     ...
     * 编辑分类
     */
    public function edit_classAction()
    {
        // 实例化商品分类模型
        $model_goodsclass = Model('goods_class');
        // 商品分类
        $goods_class = $model_goodsclass->getGoodsClass(getSession('store_id'));
        ...
        $this->view->render('store_goods_online', 'store_goods_add_step1');
        $this->view->disable();
    }
    ...
    public function drop_goodsAction()
    {
        ...
        $result = Logic('goods')->goodsDrop($commonid_array, getSession('store_id'), getSession('seller_id'), getSession('seller_name'));
        if ($result['state']) {
            // 添加操作日志
            $this->recordSellerLog('删除商品，SPU：' . $common_id);
            showDialog(getLang('store_goods_index_goods_del_success'), 'reload', 'succ');
        } else {
            showDialog(getLang('store_goods_index_goods_del_fail'), '', 'error');
        }
    }
     ...
     * 商品下架
     */
    public function goods_unshowAction()
    {
        $common_id = $this->checkRequestCommonId($_GET['commonid']);
        $commonid_array = explode(',', $common_id);
        $result = Logic('goods')->goodsUnShow($commonid_array, $this->store_info['store_id'], getSession('seller_id'), getSession('seller_name'));
        if ($result['state']) {
            showDialog(getLang('store_goods_index_goods_unshow_success'), getReferer() ? getReferer() : getUrl('shop/store_goods_online/goods_list'), 'succ', '', 2);
        } else {
            showDialog(getLang('store_goods_index_goods_unshow_fail'), '', 'error');
        }
    }
     ...
     * 设置广告词
     */
    public function edit_jingleAction()
    {
        if (chksubmit()) {
            $common_id = $this->checkRequestCommonId($_POST['commonid']);
            $commonid_array = explode(',', $common_id);
            $where = array('goods_commonid' => array('in', $commonid_array), 'store_id' => getSession('store_id'));
        ...
        $this->view->render('store_goods_online', 'store_goods_list_edit_jingle');
        $this->view->disable();
    }
     ...
     * 设置关联版式
     */
    public function edit_plateAction()
    {
        if (chksubmit()) {
            $common_id = $this->checkRequestCommonId($_POST['commonid']);
            $commonid_array = explode(',', $common_id);
        ...
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('store_goods_online', 'store_goods_list_edit_plate');
        $this->view->disable();
    }
     ...
     * 添加赠品
     */
    public function add_giftAction()
    {
        $common_id = intval($_GET['commonid']);
        if ($common_id <= 0) {
            showMessage(getLang('wrong_argument'), getUrl('shop/seller_center'), 'html', 'error');
        ...
        if (empty($goodscommon_info) || $goodscommon_info['store_id'] != getSession('store_id')) {
            showMessage(getLang('wrong_argument'), getUrl('shop/seller_center'), 'html', 'error');
        }
        ...
        $this->view->render('store_goods_online', 'store_goods_edit_add_gift');
        $this->view->disable();
    }
     ...
     * 保存赠品
     */
    public function save_giftAction()
    {
        if (!chksubmit()) {
        ...
        // 验证商品是否存在
        $model_gift = Model('goods_gift');
        $goods_list = $model_gift->getAllowGiftGoodsList(array('goods_commonid' => $commonid, 'store_id' => getSession('store_id')), 'goods_id');
        if (empty($goods_list)) {
            showDialog(getLang('wrong_argument'));
        }
        // 删除该商品原有赠品
        ...
        foreach ($data as $key => $val) {
            ...
            foreach ($val as $k => $v) {
                ...
                if ($gift_info['store_id'] != getSession('store_id') || $is_general == false) {
                    continue;
                }
        ...
        showDialog(getLang('nc_common_save_succ'), $_POST['ref_url'], 'succ');
    }

    /**
     * 设置商品佣金
     */
    public function edit_invite_priceAction()
    {
        $model_goods = Model('goods');
        if (chksubmit()) {
            $where = array();
        ...
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('store_goods_online', 'store_goods_list_edit_invite_price');
        $this->view->disable();
    }
    ...
    public function search_goodsAction()
    {
        ...
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('store_goods_online', 'store_goods_edit_search_goods');
        $this->view->disable();
    }
     ...
     * ajax获取商品列表
     */
    public function get_goods_list_ajaxAction()
    {
        $common_id = $_GET['commonid'];
        if ($common_id <= 0) {
            echo 'false';
        ...
        if (empty($goodscommon_list) || $goodscommon_list['store_id'] != getSession('store_id')) {
            echo 'false';
            exit();
        }
        $goods_list = $model_goods->getGoodsList(array('store_id' => getSession('store_id'), 'goods_commonid' => $common_id), 'goods_id,goods_spec,store_id,goods_price,goods_serial,goods_storage_alarm,goods_storage,goods_image,invite_rate');
        if (empty($goods_list)) {
            echo 'false';
            exit();
        }
        ...
        foreach ($goods_list as $key => $val) {
            ...
            foreach ($goods_spec as $k => $v) {
                $spec_array[] = '<div class="goods-spec">' . $spec_name[$k] . getLang('nc_colon') . '<em title="' . $v . '">' . $v . '</em>' . '</div>';
            }
            ...
            $goods_list[$key]['url'] = getUrl('shop/goods/index', array('goods_id' => $val['goods_id']));
        }
        echo json_encode($goods_list);
        exit;
    }

    public function edit_iframe_ajaxAction()
    {
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('store_goods_online', 'store_goods_list_edit_body');
        $this->view->disable();
        //Tpl::showpage('store_goods_list.edit_iframe', 'null_layout');
    }

    public function edit_body_ajaxAction()
    {
        $model_goods = Model('goods');
        if (chksubmit()) {
            $where = array();
            ...
            if ($result) {
                showDialog(getLang('nc_common_op_succ'), 'reload', 'succ', 'setTimeout(function () {$(".dialog_close_button", parent.document).click();}, 2000)');
            } else {
                showDialog(getLang('store_goods_index_goods_edit_fail'), 'reload', 'error', 'setTimeout(function () {$(".dialog_close_button", parent.document).click();}, 2000)');
            }
        }
        $goods_info = $model_goods->getGoodsInfoByID($_GET['goods_id']);
     ...
     * @param array|bool $allow_promotion
     */
    private function profile_menu($menu_type, $menu_key, $allow_promotion = array())
    {
        ...
        switch ($menu_type) {
            case 'goods_list':
                $menu_array = array(
                    array('menu_key' => 'goods_list', 'menu_name' => '出售中的商品', 'menu_url' => getUrl('shop/store_goods_online/index'))
                );
                break;
            case 'edit_detail':
                if ($allow_promotion['lock'] === false) {
                    $menu_array = array(
                        array('menu_key' => 'edit_detail', 'menu_name' => '编辑商品', 'menu_url' => getUrl('shop/store_goods_online/edit_goods', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                        array('menu_key' => 'edit_image', 'menu_name' => '编辑图片', 'menu_url' => getUrl('shop/store_goods_online/edit_image', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer())))),
                    );
                }
//                if ($allow_promotion['gift'] == true) {
//                    $menu_array[] = array('menu_key' => 'add_gift', 'menu_name' => '赠送赠品', 'menu_url' => getUrl('shop/store_goods_online/add_gift', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer()))));
//                }
                break;
            case 'edit_class':
                $menu_array = array(
                    array('menu_key' => 'edit_class', 'menu_name' => '选择分类', 'menu_url' => getUrl('shop/store_goods_online/edit_class', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                    array('menu_key' => 'edit_detail', 'menu_name' => '编辑商品', 'menu_url' => getUrl('shop/store_goods_online/edit_goods', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                    array('menu_key' => 'edit_image', 'menu_name' => '编辑图片', 'menu_url' => getUrl('shop/store_goods_online/edit_image', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer())))),
                );
                break;
     ...
     * @internal param string $menu_key 当前导航的menu_key
     * @internal param bool $allow_promotion
     */
    public function maker_qrcodeAction()
    {
        $store_id = getSession('store_id');
        // 生成商店二维码
        require_once(RESOURCE_SITE_URL . DS . 'phpqrcode' . DS . 'index.php');
        $PhpQRCode = new \PhpQRCode();
        $PhpQRCode->set('pngTempDir', BASE_UPLOAD_PATH . DS . ATTACH_STORE . DS . getSession('store_id') . DS);
        //print_r($PhpQRCode);
        $model_goods = Model('goods');
        $where = array();
        ...
        foreach ($lst as $k => $v) {
            $goods_id = $v['goods_id'];
            //生成二维码
            $qrcode_url = WAP_SITE_URL . '/js_template/product_detail.html?goods_id=' . $goods_id;
        ...
        //生成医生二维码
        $qrcode_url = WAP_SITE_URL . '/js_template/store.html?store_id=' . $store_id;
        $PhpQRCode->set('date', $qrcode_url);
