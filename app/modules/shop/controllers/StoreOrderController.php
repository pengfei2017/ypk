...
class StoreOrderController extends BaseSellerController
{
    public function initialize()
    {
        parent::initialize();
        getTranslation('member_store_index,member_layout,member_member_index,member_printorder');
    }
    ...
    public function indexAction()
    {
        ...
        $order_list = $model_order->getStoreOrderList(getSession('store_id'), $_GET['order_sn'], $_GET['buyer_name'], $_GET['state_type'], $_GET['query_start_date'], $_GET['query_end_date'], $_GET['skip_off'], '*', array('order_goods', 'order_common', 'member'));
        ...
        $this->view->render('store_order','store_order_index');
        $this->view->disable();
    }
    ...
    public function show_orderAction()
    {
        ...
        if (empty($order_info)) {
            showMessage(getLang('store_order_none_exist'), '', 'html', 'error');
        }
        ...
        $this->view->render('store_order','store_order_show');
        $this->view->disable();
    }
    ...
    private function _order_cancel($order_info, $post)
    {
        ...
        if (!chksubmit()) {
            ...
            $this->view->render('store_order','store_order_cancel');
            exit();
        } else {
            ...
            if ($order_info['order_type'] == 2) {
                //预定订单
                return Logic('order_book')->changeOrderStateCancel($order_info, 'seller', getSession('seller_name'), $msg);
            } else {
                $cancel_condition = array();
                if ($order_info['payment_code'] != 'offline') {
                    $cancel_condition['order_state'] = ORDER_STATE_NEW;
                }
                return $logic_order->changeOrderStateCancel($order_info, 'seller', getSession('seller_name'), $msg, true, $cancel_condition);
            }
        }
    }
    ...
    private function _order_ship_price($order_info, $post)
    {
        ...
        if (!chksubmit()) {
            ...
            $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
            $this->view->render('store_order','store_order_edit_price');
            exit();
        } else {
            ...
            return $logic_order->changeOrderShipPrice($order_info, 'seller', getSession('seller_name'), $post['shipping_fee']);
        }
    }
    ...
    private function _order_spay_price($order_info, $post)
    {
        ...
        if (!chksubmit()) {
            ...
            $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
            $this->view->render('store_order','store_order_edit_spay_price');
            exit();
        } else {
            ...
            return $logic_order->changeOrderSpayPrice($order_info, 'seller', getSession('member_name'), $post['goods_amount']);
        }
    }
     ...
     * 打印发货单
     */
    public function order_printAction()
    {
        $order_id = intval($_GET['order_id']);
        if ($order_id <= 0) {
            showMessage(getLang('wrong_argument'), '', 'html', 'error');
        ...
        if (empty($order_info)) {
            showMessage(getLang('member_printorder_ordererror'), '', 'html', 'error');
        }
        Tpl::output('order_info', $order_info);
        ...
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('store_order','store_order_print');
        $this->view->disable();
    }
    ...
    private function profile_menu($menu_type = '', $menu_key = '')
    {
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                    array('menu_key' => 'store_order', 'menu_name' => getLang('nc_member_path_all_order'), 'menu_url' => getUrl('shop/store_order')),
                    array('menu_key' => 'state_new', 'menu_name' => getLang('nc_member_path_wait_pay'), 'menu_url' => getUrl('shop/store_order/index', array('state_type' => 'state_new'))),
                    array('menu_key' => 'state_pay', 'menu_name' => getLang('nc_member_path_wait_send'), 'menu_url' => getUrl('shop/store_order/index', array('state_type' => 'state_pay'))),
                    //array('menu_key' => 'state_notakes', 'menu_name' => '待自提', 'menu_url' => getUrl('shop/store_order/store_order', array('state_type' => 'state_notakes'))),
                    array('menu_key' => 'state_send', 'menu_name' => getLang('nc_member_path_sent'), 'menu_url' => getUrl('shop/store_order/index', array('state_type' => 'state_send'))),
                    array('menu_key' => 'state_success', 'menu_name' => getLang('nc_member_path_finished'), 'menu_url' => getUrl('shop/store_order/index', array('state_type' => 'state_success'))),
                    array('menu_key' => 'state_cancel', 'menu_name' => getLang('nc_member_path_canceled'), 'menu_url' => getUrl('shop/store_order/index', array('state_type' => 'state_cancel'))),
                );
                break;
        }
