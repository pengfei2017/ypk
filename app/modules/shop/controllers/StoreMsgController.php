...
class StoreMsgController extends BaseSellerController
{
    ...
    public function msg_listAction() {
        ...
        $this->view->pick('store_msg/store_msg_list');
    }

    /**
     * 系统公告列表
     */
    public function sys_listAction() {
        ...
        $this->view->pick('store_msg/store_msg_sys_list');
    }

    /**
     * 消息详细
     */
    public function msg_infoAction() {
        $sm_id = intval($_GET['sm_id']);
        if ($sm_id <= 0) {
            showMessage(getLang('wrong_argument'), '', '', 'succ');
        }
        $model_storemsg = Model('store_msg');
        $where = array();
        $where['sm_id'] = $sm_id;
        ...
        $this->view->pick('store_msg/store_msg_info');
    }
     ...
     * 消息接收设置
     */
    public function msg_settingAction() {
        // 验证是否为管理员
        if (!$this->checkIsAdmin()) {
            showDialog(getLang('para_error'), '', 'error');
        }
        ...
        $this->view->pick('store_msg/store_msg_setting');
    }

    /**
     * 编辑店铺消息接收设置
     */
    public function edit_msg_settingAction() {
        // 验证是否为管理员
        if (!$this->checkIsAdmin()) {
            showDialog(getLang('para_error'), '', 'error');
        }
        ...
        $this->view->pick('store_msg/store_msg_setting_edit');
    }

    /**
     * 保存店铺接收设置
     */
    public function save_msg_settingAction() {
        // 验证是否为管理员
        if (!$this->checkIsAdmin()) {
            showDialog(getLang('para_error'), '', 'error');
        }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array();
        $menu_array = array(
            array('menu_key'=>'msg_list',   'menu_name'=>'消息列表',    'menu_url'=>getUrl('shop/store_msg/index')),
            array('menu_key'=>'sys_list',   'menu_name'=>'系统公告',    'menu_url'=>getUrl('shop/store_msg/sys_list')),
            array('menu_key'=>'msg_setting','menu_name'=>'消息接收设置',  'menu_url'=>getUrl('shop/store_msg/msg_setting')),
        );
        if (!$this->checkIsAdmin()) {
            unset($menu_array[2]);
        }
