...
class StoreBillController extends BaseSellerController
{
    /**
     * 每次导出多少条记录
     * @var unknown
    ...
    public function indexAction() {
        ...
        $this->view->pick('store_bill/store_bill_index');
    }
     ...
     *
     */
    public function show_billAction(){
        if (!preg_match('/^\d+$/',$_GET['ob_id'])) {
        ...
        if ($_GET['type'] =='refund'){
            ...
            Tpl::output('order_list',$order_list);
            Tpl::output('show_page',$model_order->showpage());
            $sub_tpl_name = 'store_bill_show_order_list';
       ...
       // Tpl::showpage('store_bill.show');
        $this->view->pick('store_bill/store_bill_show');
    }
     ...
     *
     */
    public function bill_printAction(){
        if (!preg_match('/^\d+$/',$_GET['ob_id'])) {
            showMessage('参数错误','','html','error');
        }
        $model_bill = Model('bill');
       ...
       // Tpl::showpage('store_bill.print','null_layout');
        $this->view->pick('store_bill/store_bill_print');
    }
     ...
     *
     */
    public function confirm_billAction(){
        if (!preg_match('/^\d+$/',$_GET['ob_id'])) {
        ...
        if ($update){
            ...
            showDialog(getLang('nc_common_op_fail'),'reload','error');
        }
    }
    ...
    public function export_orderAction(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('murl',getUrl('shop/store_bill/show_bill',array('ob_id'=>$_GET['ob_id'])));
               // Tpl::showpage('store_export.excel');
                $this->view->pick('store_bill/store_export_excel');
                exit();
            }else{
                //如果数量小，直接下载
     ...
     *
     */
    public function export_bookAction(){
        if (!preg_match('/^\d+$/',$_GET['ob_id'])) {
            showMessage('参数错误','','html','error');
        }
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('list',$array);
                Tpl::output('murl',getUrl('shop/store_bill/ob_id',array('ob_id'=>$_GET['ob_id'])));
               // Tpl::showpage('store_export.excel');
                $this->view->pick('store_bill/store_export_excel');
                exit();
            }
     ...
     *
     */
    public function export_refund_orderAction(){
        if (!preg_match('/^\d+$/',$_GET['ob_id'])) {
            showMessage('参数错误','','html','error');
        }
        $model_bill = Model('bill');
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl',getUrl('shop/store_bill/show_bill',array('query_type'=>'refund','ob_id'=>$_GET['ob_id'])));
                //Tpl::showpage('store_export.excel');
                $this->view->pick('store_bill/store_export_excel');
                exit();
            }else{
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                    1=>array('menu_key'=>'list','menu_name'=>'实物订单结算', 'menu_url'=>getUrl('shop_manager/bill/list')),
                );
                break;
            case 'show':
                $menu_array = array(
                    array('menu_key'=>'order_list','menu_name'=>'订单列表', 'menu_url'=>getUrl('shop/store_bill/show_bill',array('ob_id'=>$_GET['ob_id']))),
                    array('menu_key'=>'refund_list','menu_name'=>'退款订单','menu_url'=>getUrl('shop/store_bill/show_bill',array('type'=>'refund','ob_id'=>$_GET['ob_id']))),
                    array('menu_key'=>'cost_list','menu_name'=>'促销费用','menu_url'=>getUrl('shop/store_bill/show_bill',array('type'=>'cost','ob_id'=>$_GET['ob_id'])))
                );
                if (floatval($this->_bill_info['ob_order_book_totals']) > 0) {
                    array_push($menu_array,array('menu_key'=>'book_list','menu_name'=>'未退定金', 'menu_url'=>getUrl('shop/store_bill/show_bill',array('type'=>'book','ob_id'=>$_GET['ob_id']))));
                }
                break;
        }
