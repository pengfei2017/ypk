...
//                die;
                showDialog('登录凭证已失效，请先登录', getUrl('shop/seller_login/show_login', array('type' => 'login')));
            }
    ...
    {
        $current_menu = $seller_function_list[$this->dispatcher->getControllerName()];
    ...
    private function _getMenuList()
    {
        ...
        $menu_list = array(
            'goods' => array('name' => "{$nav_name}", 'child' => array(
                ...
                array('name' => "发布聊天卡", 'act' => 'store_goods_add', 'op' => 'addChatCard'),
//                array('name' => '淘宝CSV导入', 'act' => 'taobao_import', 'op' => 'index'),
                array('name' => "出售中的{$nav_name}和聊天卡", 'act' => 'store_goods_online', 'op' => 'index'),
