...
class MemberRefundController extends BaseMemberController
{
    ...
    public function indexAction()
    {
        ...
        $this->view->render('member_refund', 'member_refund');
        $this->view->disable();
    }
    ...
    public function add_refundAction()
    {
        ...
        if ($order_id < 1 || $goods_id < 1) {//参数验证
            showDialog(getLang('wrong_argument'), getUrl('shop/member_order/index'), 'error');
        }
        $condition = array();
        $condition['buyer_id'] = getSession('member_id');
        ...
        if (chksubmit() && $goods_id > 0) {
            ...
            $show_url = getUrl('shop/member_return/index');
            ...
            if ($refund_array['refund_type'] != '2') {
                ...
                $show_url = getUrl('shop/member_refund/index');
            }
            ...
            if ($state) {
                ...
                showDialog(getLang('nc_common_save_succ'), $show_url, 'succ');
            } else {
                showDialog(getLang('nc_common_save_fail'), 'reload', 'error');
            }
        }
        //Tpl::showpage('member_refund_add');
        $this->view->render('member_refund', 'member_refund_add');
        $this->view->disable();
    }
     ...
     *
     */
    public function add_refund_allAction()
    {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                $model_refund->editOrderLock($order_id);
                showDialog(getLang('nc_common_save_succ'), getUrl('shop/member_refund/index'), 'succ');
            } else {
                showDialog(getLang('nc_common_save_fail'), 'reload', 'error');
            }
        }
        //Tpl::showpage('member_refund_all');
        $this->view->render('member_refund', 'member_refund_all');
        $this->view->disable();
    }
    ...
    public function viewAction()
    {
        ...
        $this->view->render('member_refund', 'member_refund_view');
        $this->view->disable();
    }
