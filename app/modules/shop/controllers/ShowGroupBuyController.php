 ...
 * Time: 18:27
 *
 * 采用的布局页：home_layout（即和首页采用同样的布局页面）
 */
...
class ShowGroupBuyController extends BaseHomeController
{
    ...
    public function initialize()
    {
        ...
        //读取语言包
        getTranslation('member_groupbuy,home_cart_index');

        //检查抢购功能是否开启
        if (intval(getConfig('groupbuy_allow')) !== 1) {
            showMessage(getLang('groupbuy_unavailable'), getUrl('shop/index'), '', 'error');
        }

        //分类导航
        $nav_link = array(
            0 => array(
                'title' => getLang('homepage'),
                'link' => SHOP_SITE_URL,
            ),
            1 => array(
                'title' => getLang('nc_groupbuy')
            )
        );
        ...
        if ($_GET['op'] != 'groupbuy_detail') {
            // 抢购价格区间
            $this->groupbuy_price = read_file_cache('groupbuy_price', true);
     ...
     * 选择城市
     */
    public function select_cityAction()
    {
        $city_id = intval($_GET['city_id']); //获取城市id
        ...
        redirect(getUrl('shop/show_groupbuy', $_GET['back_op']));
    }
    ...
    public function indexAction()
    {
        ...
        foreach (range(1, 4) as $i) {
            $a = getConfig('live_pic' . $i);
            if ($a) {
                $picArr[] = array($a, getConfig('live_color' . $i), getConfig('live_link' . $i));
            }
        }
     ...
     * 进行中的虚拟抢购
     */
    public function vr_groupbuy_listAction()
    {
        Tpl::output('current', 'online');
        Tpl::output('buy_button', getLang('groupbuy_buy'));
     ...
     * 即将开始的虚拟抢购
     */
    public function vr_groupbuy_soonAction()
    {
     ...
     * 往期虚拟抢购
     */
    public function vr_groupbuy_historyAction()
    {
    ...
    private function _show_vr_groupbuy_list($function_name)
    {
        ...
        Tpl::output('html_title', getLang('text_groupbuy_list'));
        ...
        $this->view->render('show_groupbuy', 'groupbuy_vr_list');
        $this->view->disable();
    }
     ...
     * 即将开始的抢购
     **/
    public function groupbuy_soonAction()
    {
        Tpl::output('current', 'soon');
        Tpl::output('buy_button', '未开始');
     ...
     * 往期抢购
     **/
    public function groupbuy_historyAction()
    {
        Tpl::output('current', 'history');
        Tpl::output('buy_button', '已结束');
    ...
    private function _show_groupbuy_list($function_name)
    {
        ...
        $this->view->render('show_groupbuy', 'groupbuy_list');
        $this->view->disable();
    }
     ...
     * 抢购详细信息
     **/
    public function groupbuy_detailAction()
    {
        ...
        if (empty($groupbuy_info)) {
            showMessage(getLang('param_error'), getUrl('shop/show_groupbuy/ndex'), '', 'error');
        }
     ...
     * 购买记录
     */
    public function groupbuy_orderAction()
    {
        $group_id = intval($_GET['group_id']);
        if ($group_id > 0) {
            if (!$_GET['is_vr']) {
        ...
        $this->view->render('no_layout', 'groupbuy_order');
        $this->view->disable();
    }
     ...
     * 商品评价
     */
    public function groupbuy_evaluateAction()
    {
        ...
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('no_layout', 'groupbuy_evaluate');
        $this->view->disable();
    }
