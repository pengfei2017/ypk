...
class StoreDeliverSetController extends BaseSellerController
{
    public function initialize() {
        parent::initialize();
        getTranslation('member_store_index,deliver,common,layout');
    }

    /**
     * 发货地址列表
     */
    public function daddress_listAction() {
        getTranslation('member_member_index');
        ...
        $this->view->pick('store_deliver_set/store_deliver_set_daddress_list');
    }

    /**
     * 新增/编辑发货地址
     */
    public function daddress_addAction() {
        getTranslation('member_member_index');
        $lang   = getTranslation();
        $model_daddress = Model('daddress');
        if (chksubmit()) {
            ...
            if (empty($address_info) && !is_array($address_info)){
                showMessage($lang['store_daddress_wrong_argument'],getUrl('shop/store_deliver_set/daddress_list'),'html','error');
            }
            Tpl::output('address_info',$address_info);
        }
       //Tpl::showpage('store_deliver_set.daddress_add','null_layout');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->pick('store_deliver_set/store_deliver_set_daddress_add');
    }

    /**
     * 删除发货地址
     */
    public function daddress_delAction() {
        $address_id = intval($_GET['address_id']);
        if ($address_id <=  0) {
            showDialog(getLang('store_daddress_del_fail'),'','error');
        }
        $condition = array();
        $condition['address_id'] = $address_id;
        $condition['store_id'] = $_SESSION['store_id'];
        ...
        if ($delete){
            showDialog(getLang('store_daddress_del_succ'),getUrl('shop/store_deliver_set/daddress_list'),'succ');
        }else {
            showDialog(getLang('store_daddress_del_fail'),'','error');
        }
    }
     ...
     * 设置默认发货地址
     */
    public function daddress_default_setAction() {
        $address_id = intval($_GET['address_id']);
    ...
    public function expressAction() {
        ...
        $this->view->pick('store_deliver_set/store_deliver_express');
    }

    /**
     * 免运费额度设置
     */
    public function free_freightAction(){
        ...
        $this->view->pick('store_deliver_set/store_free_freight_index');
    }

    /**
     * 默认配送区域设置
     */
    public function deliver_regionAction(){
        if (chksubmit()) {
        ...
        $this->view->pick('store_deliver_set/store_default_deliver_region_index');
    }

    /**
     * 发货单打印设置
     */
    public function print_setAction(){
        $model = Model();
        $store_info = $model->table('store')->where(array('store_id'=>$_SESSION['store_id']))->find();
        if(empty($store_info)){
            showDialog(getLang('store_storeinfo_error'),getUrl('shop/store_printsetup/index'),'error');
        }
        if(chksubmit()){
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
                array("input"=>$_POST['store_printdesc'], "require"=>"true","validator"=>"Length","min"=>1,"max"=>200,"message"=>getLang('store_printsetup_desc_error'))
            );
            $error = $obj_validate->validate();
            if ($error != ''){
                showDialog($error);
            ...
            $this->view->pick('store_deliver_set/store_printsetup');
        }
    }
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        getTranslation('member_layout');
        switch ($menu_type) {
            case 'daddress':
                $menu_array = array(
                    array('menu_key'=>'daddress',   'menu_name'=>getLang('store_deliver_daddress_list'),  'menu_url'=>getUrl('shop/store_deliver_set/daddress_list')),
                    array('menu_key'=>'express',        'menu_name'=>getLang('store_deliver_default_express'),    'menu_url'=>getUrl('shop/store_deliver_set/express')),
                    array('menu_key'=>'free_freight',   'menu_name'=>'免运费额度',      'menu_url'=>getUrl('shop/store_deliver_set/free_freight')),
                    array('menu_key'=>'deliver_region',   'menu_name'=>'默认配送地区',      'menu_url'=>getUrl('shop/store_deliver_set/deliver_region')),
                    array('menu_key'=>'print_set', 'menu_name'=>'发货单打印设置', 'menu_url'=>getUrl('shop/store_deliver_set/print_set'))
                );
                break;
        }
