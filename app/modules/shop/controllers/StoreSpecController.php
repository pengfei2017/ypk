...
class StoreSpecController extends BaseSellerController
{
...
//        $list = GoodsClass::find("gc_id=1076");
//        if (count($list) > 0) {
//            $list = $list->toArray();
//        }
...
//        Tpl::showpage('store_spec.index');
        $this->view->pick('store_spec/store_spec_index');
    }

    /**
     * 添加规格值
     */
    public function add_specAction()
    {
        $sp_id = intval($_GET['spid']);
        $gc_id = intval($_GET['gcid']);
        // 验证参数
        if ($sp_id <= 0) {
            showMessage(getLang('wrong_argument'));
        }
        // 分类信息
...
//        Tpl::showpage('store_spec.add', 'null_layout');
        ...
        $this->view->render('store_spec', 'store_spec_add');
        $this->view->disable();
    }
     ...
     * 保存规格值
     */
    public function save_specAction()
    {
     ...
     * ajax删除规格值
     */
    public function ajax_delspecAction()
    {
     ...
     * AJAX获取商品分类
     */
    public function ajax_classAction()
    {
        $id = intval($_GET['id']);
    ...
    private function _profile_menu($menu_type, $menu_key)
    {
        ...
        switch ($menu_type) {
            case 'spec':
                $menu_array = array(
                    array('menu_key' => 'spec', 'menu_name' => getLang('nc_member_path_store_spec'), 'menu_url' => getUrl('shop/store_spec/index'))
                );
                break;
        }
