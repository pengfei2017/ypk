...
class StoreDeliverController extends BaseSellerController
{
    public function initialize()
    {
        parent::initialize();
        getTranslation('member_store_index,deliver,member_member_index,member_layout');
    }
    ...
    public function indexAction()
    {
        ...
        $condition = array();
        $condition['store_id'] = getSession('store_id');
        ...
        foreach ($order_list as $key => $order_info) {
            foreach ($order_info['extend_order_goods'] as $value) {
                ...
                $value['goods_url'] = getUrl('shop/goods/index', array('goods_id' => $value['goods_id']));
        ...
        $this->view->render('store_deliver', 'store_order_deliver');
        $this->view->disable();
    }
    ...
    public function sendAction()
    {
        ...
        if (chksubmit()) {
            ...
            $result = $logic_order->changeOrderSend($order_info, 'seller', getSession('seller_name'), $_POST);
            if (!$result['state']) {
        ...
        if ($order_info['extend_order_common']['daddress_id'] > 0) {
            ...
            //取默认地址
            $daddress_info = $model_daddress->getAddressList(array('store_id' => getSession('store_id')), '*', 'is_default desc', 1);
        ...
        if ($order_info['extend_order_common']['reciver_info']['dlyp'] != '') {
            ...
            //快递公司
            $my_express_list = Model()->table('store_extend')->getfby_store_id(getSession('store_id'), 'express');
        ...
        $this->view->render('store_deliver', 'store_deliver_send');
        $this->view->disable();
    }
     ...
     * @return boolean
     */
    public function buyer_address_editAction()
    {
        $order_id = intval($_GET['order_id']);
        ...
        $this->view->render('store_deliver', 'store_deliver_buyer_address_edit');
        $this->view->disable();
    }
     ...
     * 收货地址保存
     */
    public function buyer_address_saveAction()
    {
        $model_order = Model('order');
        $data = array();
     ...
     * @return boolean
     */
    public function send_address_selectAction()
    {
        $address_list = Model('daddress')->getAddressList(array('store_id' => getSession('store_id')));
        Tpl::output('address_list', $address_list);
        Tpl::output('order_id', $_GET['order_id']);
        //Tpl::showpage('store_deliver.daddress.select', 'null_layout');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('store_deliver', 'store_deliver_daddress_select');
        $this->view->disable();
    }
     ...
     * 保存发货地址修改
     */
    public function send_address_saveAction()
    {
     ...
     * 物流跟踪
     */
    public function search_deliverAction()
    {
        $order_sn = $_GET['order_sn'];
        if (!preg_match('/^\d{10,20}$/', $_GET['order_sn'])) showMessage(getLang('wrong_argument'), '', 'html', 'error');
        $model_order = Model('order');
        ...
        //取得配送公司代码
        $express = read_file_cache('express', true);
        ...
        $this->view->render('store_deliver', 'store_deliver_detail');
        $this->view->disable();
    }
     ...
     * 延迟收货
     */
    public function delay_receiveAction()
    {
        $order_id = intval($_GET['order_id']);
        ...
        if (chksubmit()) {
            ...
            if (!in_array($delay_date, array(5, 10, 15))) {
                showDialog(getLang('wrong_argument'), '', 'error', empty($_GET['inajax']) ? '' : 'CUR_DIALOG.close();');
            }
            ...
            $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
            $this->view->render('store_deliver', 'store_deliver_delay_receive');
            $this->view->disable();
            exit();
        }
    }
     ...
     * 运单打印
     */
    public function waybill_printAction()
    {
        $order_id = intval($_GET['order_id']);
        if ($order_id <= 0) {
        ...
        if (empty($store_waybill_info)) {
            showMessage('请首先绑定打印模板', getUrl('shop/store_waybill/waybill_manage'), '', 'error');
        }
        ...
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->render('store_deliver', 'waybill_print');
        $this->view->disable();
    }
    ...
    private function profile_menu($menu_type, $menu_key = '')
    {
        ...
        switch ($menu_type) {
            case 'deliver':
                $menu_array = array(
                    array('menu_key' => 'deliverno', 'menu_name' => getLang('nc_member_path_deliverno'), 'menu_url' => getUrl('shop/store_deliver/index',array('state'=>'deliverno'))),
                    array('menu_key' => 'delivering', 'menu_name' => getLang('nc_member_path_delivering'), 'menu_url' => getUrl('shop/store_deliver/index',array('state'=>'delivering'))),
                    array('menu_key' => 'delivered', 'menu_name' => getLang('nc_member_path_delivered'), 'menu_url' => getUrl('shop/store_deliver/index',array('state'=>'delivered'))),
                );
                break;
            case 'search':
                $menu_array = array(
                    1 => array('menu_key' => 'nodeliver', 'menu_name' => getLang('nc_member_path_deliverno'), 'menu_url' =>getUrl('shop/store_deliver/index',array('state'=>'nodeliver')) ),
                    2 => array('menu_key' => 'delivering', 'menu_name' => getLang('nc_member_path_delivering'), 'menu_url' => getUrl('shop/store_deliver/index',array('state'=>'delivering'))),
                    3 => array('menu_key' => 'delivered', 'menu_name' => getLang('nc_member_path_delivered'), 'menu_url' => getUrl('shop/store_deliver/index',array('state'=>'delivered'))),
                    4 => array('menu_key' => 'search', 'menu_name' => getLang('nc_member_path_deliver_info'), 'menu_url' => '###'),
                );
                break;
        }
