...
class StoreController extends MobileHomeController
{
     ...
     * 医生信息
     */
    public function store_infoAction()
    {
...
//            : UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.getConfig('default_store_avatar');
        $store_info['store_avatar'] = getStoreHeaderLogo($store_online_info['member_id']);
        ...
        if ($store_info['is_own_shop']) {
            $store_info['store_credit_text'] = '官方医生';
        } else {
     ...
     * 医生商品分类
     */
    public function store_goods_classAction()
    {
        $store_id = (int)$_REQUEST['store_id'];
        if ($store_id <= 0) {
            output_error('参数错误');
     ...
     * 商品评价
     */
    public function store_creditAction()
    {
     ...
     * 医生商品排行
     */
    public function store_goods_rankAction()
    {
        $store_id = (int)$_REQUEST['store_id'];
        if ($store_id <= 0) {
            output_data(array());
     ...
     * 医生商品上新
     */
    public function store_new_goodsAction()
    {
        $store_id = (int)$_REQUEST['store_id'];
        if ($store_id <= 0) {
     ...
     * 医生简介
     */
    public function store_introAction()
    {
        $store_id = (int)$_REQUEST['store_id'];
        if ($store_id <= 0) {
            output_error('参数错误');
        ...
        //$store_info['store_avatar'] = $store_online_info['store_avatar']
            //? UPLOAD_SITE_URL . '/' . ATTACH_STORE . '/' . $store_online_info['store_avatar']
            //: UPLOAD_SITE_URL . '/' . ATTACH_COMMON . DS . getConfig('default_store_avatar');
		$store_info['store_avatar']=getMemberAvatarForID($store_info['member_id']);
        ...
        //显示医生的个人信息
        $store_join_model = StoreJoinin::findFirst('member_id=' . $store_info['member_id']);
        if ($store_join_model !== false) {
            $store_info['mail_content'] = $store_join_model->getMailContent();
        }

		//判断医生所在地
        if(empty($store_info['area_info']) || strlen(trim($store_info['area_info']))<=0){
            $member_id=$this->getMemberIdIfExists();
            if(!empty($member_id)){
                $store_join_info=StoreJoinin::findFirst("member_id=".$store_info['member_id']);
                if($store_join_info!==false){
                    $store_info['area_info']=$store_join_info->getCompanyAddress();
                }
            }
        }

        if(empty($store_info['area_info']) || strlen(trim($store_info['area_info']))<=0){
            $store_info['area_info']="所在地保密";
        }
     ...
     * 医生促销活动
     */
    public function store_promotionAction()
    {
        $param = $_REQUEST;
        $store_id = (int)$param['store_id'];
        if ($store_id <= 0) {
