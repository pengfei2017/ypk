...
class MemberOrderController extends MobileMemberController
{
    ...
    public function order_listAction()
    {
        ...
        $new_order_payed_array = array(); //用于记录已经付款的订单
        foreach ($order_list_array as $value) {
            if ($value['order_state'] == ORDER_STATE_PAY) {
                $new_order_payed_array[] = $value;
            }
        ...
        //处理已支付过的服务订单和聊天卡订单的状态
//        if (count($new_order_payed_array) > 0) {
//            foreach ($new_order_payed_array as $order_info) {
//                $order_goods_list = OrderGoods::find("order_id=" . $order_info['order_id']);
...
//                        if ($goods_info !== false) {
//                            if ($goods_info->getGcId1() == 1073) { //表示购买的是服务
//
//
//                                break;
//                            }
...
//        }
     ...
     * 订单确认收货
     */
    public function order_receiveAction()
    {
        $model_order = Model('order');
        $logic_order = Logic('order');
        $order_id = intval($_POST['order_id']);
        ...
        if (!$result['state']) {
            ...
            //todo 调用四大计算
            $order_info['order_type']='real_order';
     ...
     * 取得当前的物流最新信息
     */
    public function get_current_deliverAction()
    {
        $order_id = intval($_POST['order_id']);
        if ($order_id <= 0) {
            output_error('订单不存在');
