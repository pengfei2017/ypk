...
class ShopController extends MobileHomeController
{
    ...
    private function _get_Own_Store_List()
    {
        ...
        if($_GET['type']=='doctor'){
            $condition['member_id']=array('not in',1);
        }else{
            $condition['member_id']=array('like',1);
        }
        //所需字段
        $fields = "*";
        //排序方式
        ...
        foreach ($own_store_list as $key => $value) {
            ...
            //$simply_store_list[$key]['store_avatar_url'] = UPLOAD_SITE_URL . '/' . ATTACH_COMMON . DS . getConfig('default_store_avatar');
            $simply_store_list[$key]['store_avatar_url'] = getStoreHeaderLogo($value['member_id']);
