<?php
/**
 * 支付完成后，回调此文件中的各个方法
 */

namespace Ypk\Modules\Mobile\Controllers;


use Phalcon\Mvc\View;
use Ypk\Log;
use Ypk\QueueClient;
use Ypk\Tpl;

class PaymentController extends MobileHomeController
{

    private $payment_code;

    public function initialize()
    {
        parent::initialize();
    }

    /**
     * 支付回调(只有支付宝手机网站支付的回调在用)
     */
    public function returnAction()
    {
        unset($_GET['_url']);

        //Log::record('returnAction------------' . json_encode($_GET));

        $temp_out_trade_no = explode('-', $_GET['out_trade_no']);
        $_GET['out_trade_no'] = $temp_out_trade_no[0];
        $this->payment_code = $_GET['payment_code'] = $temp_out_trade_no[1];

        $payment_api = $this->_get_payment_api();

        $payment_config = $this->_get_payment_config();

        $callback_info = $payment_api->getReturnInfo($payment_config);

        if ($callback_info) {
            $order_type = explode('_', $_GET['out_trade_no'])[1];
            Tpl::output('order_type', $order_type);
            //验证成功
            $result = $this->_update_order($callback_info['out_trade_no'], $callback_info['trade_no']);
            if ($result['state']) {
                Tpl::output('result', 'success');
                Tpl::output('message', '支付成功');
            } else {
                Tpl::output('result', 'fail');
                Tpl::output('message', '支付失败');
            }
        } else {
            //验证失败
            Tpl::output('result', 'fail');
            Tpl::output('message', '支付失败');
        }

        // Tpl::showpage('payment_message');
        $this->view->setRenderLevel(View::LEVEL_ACTION_VIEW);
        $this->view->pick('common/payment_message');
    }

    /**
     * 支付宝异步回调提醒
     */
    public function notifyAction()
    {
        Log::record('notifyAction-----get-------' . json_encode($_GET));
        Log::record('notifyAction-----post-------' . json_encode($_POST));
        if (empty($_POST)) {
            //支付宝是用$_POST方式进行的异步通知，财付通是用$_GET方式进行的异步通知
            unset($_GET['_url']); //去掉phalcon自动添加的$_GET['_url']参数，要不验证支付宝签名不成功
            $_POST = $_GET;
            //Log::record('notifyAction22：----------11111111' . json_encode($_POST));
        }
        $temp_out_trade_no = explode('-', $_POST['out_trade_no']);
        $_POST['out_trade_no'] = $temp_out_trade_no[0];
        $this->payment_code = $_POST['payment_code'] = $temp_out_trade_no[1];

        $payment_api = $this->_get_payment_api();

        $payment_config = $this->_get_payment_config();

        $callback_info = $payment_api->getNotifyInfo($payment_config);

        if ($callback_info) {
            //验证成功
            $result = $this->_update_order($callback_info['out_trade_no'], $callback_info['trade_no']);
            if ($result['state']) {
                echo 'success';
                die;
            }
        }

        //验证失败
        echo "fail";
        die;
    }

    /**
     * 微信jsapi异步回调提醒
     */
    public function wxpay_jsapi_notifyAction()
    {
        $this->payment_code = 'wxpay_jsapi';
        // wxpay_jsapi
        $inc_file = BASE_PATH . DS . 'api' . DS . 'payment' . DS . $this->payment_code . DS . $this->payment_code . '.php';
        if (is_file($inc_file)) {
            require($inc_file);
        }

        $payment_info = $this->_get_payment_config();
        $api = new $this->payment_code($payment_info);
        $result = $api->notify();

        if ($result) {
            $internalSn = $result['out_trade_no'] . '_' . $result['attach'];
            $externalSn = $result['transaction_id'];
            $this->_update_order($internalSn, $externalSn);
        }

        exit;
    }

    /**
     * 支付宝移动支付
     */
    public function notify_alipay_nativeAction()
    {
        $this->payment_code = 'alipay_native';
        $inc_file = BASE_PATH . DS . 'api' . DS . 'payment' . DS . $this->payment_code . DS . $this->payment_code . '.php';

        if (is_file($inc_file)) {
            require($inc_file);
        }
        $payment_config = $this->_get_payment_config();
        $payment_api = new $this->payment_code();
        $payment_api->payment_config = $payment_config;
        $payment_api->alipay_config['partner'] = $payment_config['alipay_partner'];

        //商户订单号
        $out_trade_no = $_POST['out_trade_no'];
        if ($payment_api->verify_notify()) {
            //支付宝交易号
            $trade_no = $_POST['trade_no'];

            //交易状态
            $trade_status = $_POST['trade_status'];

            if ($_POST['trade_status'] == 'TRADE_FINISHED' || $_POST['trade_status'] == 'TRADE_SUCCESS') {
                $result = $this->_update_order($out_trade_no, $trade_no);
                if (!$result['state']) {
                    logResult("订单状态更新失败" . $out_trade_no);
                }
            }
            exit("success");
        } else {
            logResult("verifyNotify验证失败" . $out_trade_no);
            exit("fail");
        }
    }

    /**
     * 获取支付接口实例
     */
    private function _get_payment_api()
    {
        $inc_file = BASE_PATH . DS . 'api' . DS . 'payment' . DS . $this->payment_code . DS . $this->payment_code . '.php';

        if (is_file($inc_file)) {
            require($inc_file);
        }

        $payment_api = new $this->payment_code();

        return $payment_api;
    }

    /**
     * 获取支付接口信息
     */
    private function _get_payment_config()
    {
        $model_mb_payment = Model('mb_payment');

        //读取接口配置信息
        $condition = array();
        if ($this->payment_code == 'wxpay3') {
            $condition['payment_code'] = 'wxpay';
        } else {
            $condition['payment_code'] = $this->payment_code;
        }
        $payment_info = $model_mb_payment->getMbPaymentOpenInfo($condition);

        return $payment_info['payment_config'];
    }

    /**
     * 更新订单状态
     */
    private function _update_order($out_trade_no, $trade_no)
    {
        $model_order = Model('order');
        $logic_payment = Logic('payment');

        $tmp = explode('_', $out_trade_no);
        $out_trade_no = $tmp[0];
        if (!empty($tmp[1])) {
            $order_type = $tmp[1];
        } else {
            $order_pay_info = Model('order')->getOrderPayInfo(array('pay_sn' => $out_trade_no)); //订单实体信息
            if (empty($order_pay_info)) {
                $result = $logic_payment->getPdOrderInfo($out_trade_no);
                if (!$result['state']) {
                    $order_type = 'v'; //虚拟订单
                } else {
                    $order_type = 'pd'; //在线充值订单
                }
            } else {
                $order_type = 'r'; //实物订单
            }
        }

        // wxpay_jsapi
        $paymentCode = $this->payment_code;
        if ($paymentCode == 'wxpay_jsapi') {
            $paymentCode = 'wx_jsapi';
        } elseif ($paymentCode == 'wxpay3') {
            $paymentCode = 'wxpay';
        } elseif ($paymentCode == 'alipay_native') {
            $paymentCode = 'ali_native';
        }

        //表示是实物订单
        if ($order_type == 'r') {
            $result = $logic_payment->getRealOrderInfo($out_trade_no);
            if (intval($result['data']['api_pay_state'])) {
                return array('state' => true);
            }
            $order_list = $result['data']['order_list'];
            $result = $logic_payment->updateRealOrder($out_trade_no, $paymentCode, $order_list, $trade_no);

            $api_pay_amount = 0;
            if (!empty($order_list)) {
                foreach ($order_list as $order_info) {
                    $api_pay_amount += $order_info['order_amount'] - $order_info['pd_amount'] - $order_info['rcb_amount'];

                    //更改服务订单和聊天卡订单的订单状态
                    //change_order_state($order_info);
                }
            }
            $log_buyer_id = $order_list[0]['buyer_id'];
            $log_buyer_name = $order_list[0]['buyer_name'];
            $log_desc = '实物订单使用' . orderPaymentName($paymentCode) . '成功支付，支付单号：' . $out_trade_no;

        } elseif ($order_type == 'v') { //表示是虚拟订单
            $result = $logic_payment->getVrOrderInfo($out_trade_no);
            $order_info = $result['data'];
            if (!in_array($result['data']['order_state'], array(ORDER_STATE_NEW, ORDER_STATE_CANCEL))) {
                return array('state' => true);
            }
            $result = $logic_payment->updateVrOrder($out_trade_no, $paymentCode, $result['data'], $trade_no);

            $api_pay_amount = $order_info['order_amount'] - $order_info['pd_amount'] - $order_info['rcb_amount'];
            $log_buyer_id = $order_info['buyer_id'];
            $log_buyer_name = $order_info['buyer_name'];
            $log_desc = '虚拟订单使用' . orderPaymentName($paymentCode) . '成功支付，支付单号：' . $out_trade_no;

            //更改服务订单和聊天卡订单的订单状态
            change_vr_order_state($order_info);
        } elseif ($order_type == 'pd') {
            $result = $logic_payment->getPdOrderInfo($out_trade_no);
            if ($result['data']['pdr_payment_state'] == 1) {
                return array('state' => true);
            }

            $api_pay_amount = $result['data']['pdr_amount'];
            $log_buyer_id = $result['data']['pdr_member_id'];
            $log_buyer_name = $result['data']['pdr_member_name'];
            $log_desc = '在线充值使用' . orderPaymentName($paymentCode) . '成功支付，支付单号：' . $out_trade_no;

            $payment_info = $logic_payment->getPaymentInfo($paymentCode);
            $payment_info = $payment_info['data'];
            $result = $logic_payment->updatePdOrder($out_trade_no, $trade_no, $payment_info, $result['data']);
        }

        if ($result['state']) {
            //记录消费日志
            QueueClient::push('addConsume', array('member_id' => $log_buyer_id, 'member_name' => $log_buyer_name,
                'consume_amount' => ncPriceFormat($api_pay_amount), 'consume_time' => TIMESTAMP, 'consume_remark' => $log_desc));
        }

        return $result;
    }

}
