...
class GoodsController extends MobileHomeController
{
    ...
    public function goods_listAction()
    {
        ...
        // ==== 暂时不显示定金预售商品，手机端未做。  ====
        $condition['is_book'] = 0;
        if (!empty($_GET['doctor_id']) && intval($_GET['doctor_id']) > 0) {
            $condition['doctor_id'] = $_GET['doctor_id'];
        }
    ...
    public function goods_detailAction()
    {
        ...
        if (!empty($goods_detail['goods_info'])) {
            ...
            if (!empty($goods_detail['goods_info'])) {
                ...
                //获取时长
                if ($goods_detail['goods_info']['gc_id_1'] == 1076) { //表示是聊天卡
                    $str = "(时长:" . ($goods_detail['goods_info']['time_long'] / 60) . "分钟，赠送积分：".$goods_detail['goods_info']['goods_points'].")";
                } else {
                    $str = "(赠送积分:" . $goods_detail['goods_info']['goods_points'] . ")";
                }
                $goods_detail['goods_info']['self_info'] = $str;

                //获取有效期限
                if(empty($goods_detail['goods_info']['doctor_service_start_time']) && empty($goods_detail['goods_info']['doctor_service_end_time'])){
                    $goods_detail['goods_info']['time_continue']='不限';
                }else{
                    $goods_detail['goods_info']['time_continue'] = date('Y/m/d H:i', $goods_detail['goods_info']['doctor_service_start_time']) . '-' . date('Y/m/d H:i', $goods_detail['goods_info']['doctor_service_end_time']);
                }
            }
        }
    ...
    private function _goods_detail_extend($goods_detail)
    {
        ...
        //商品链接
        $goods_detail['goods_info']['goods_url'] = getUrl('mobile/goods/index', array('goods_id' => $goods_detail['goods_info']['goods_id']));
     ...
     * 商品详细页
     */
    public function goods_bodyAction()
    {
...
//        Tpl::showpage('goods_body');
        ...
        $this->view->pick('common/goods_body');
    }

    public function goods_evaluateAction()
    {
        $goods_id = intval($_GET['goods_id']);
            ...
            if ($freight_total > 0) {
                if ($store_info['store_free_price'] > 0) {
                    if ($freight_total >= $store_info['store_free_price']) {
                        ...
                        $freight_total = '运费：' . $freight_total . ' 元，医生满 ' . $store_info['store_free_price'] . ' 元 免运费';
                    }
                } else {
     ...
     * 取得医生分店地址
     */
    public function store_o2o_addrAction()
    {
    ...
    public function auto_completeAction()
    {
