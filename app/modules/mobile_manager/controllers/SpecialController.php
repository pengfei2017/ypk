...
//        Tpl::showpage('mb_special.list');
        $this->view->pick('special/special_list');
    }

    /**
     * 输出专题列表XML数据
     */
    public function get_special_xmlAction()
    {
        ...
        if (!empty($list) && is_array($list)) {
            ...
            foreach ($list as $k => $v) {
                ...
                $operation .= '<a class="btn blue" href="' . getUrl('mobile_manager/special/special_edit', array('special_id' => $v['special_id'])) . '"><i class="fa fa-pencil-square-o"></i>编辑</a>';
     ...
     * 保存专题
     */
    public function special_saveAction()
    {
        $model_mb_special = Model('mb_special');
        ...
        if ($result) {
            ...
            showMessage(getLang('nc_common_save_succ'), getUrl('mobile_manager/special/special_list'));
        } else {
            $this->log('添加手机专题' . '[ID:' . $result . ']', 0);
            showMessage(getLang('nc_common_save_fail'), getUrl('mobile_manager/special/special_list'));
        }
    }
     ...
     * 编辑专题描述
     */
    public function update_special_descAction()
    {
        $model_mb_special = Model('mb_special');
     ...
     * 删除专题
     */
    public function special_delAction()
    {
        $model_mb_special = Model('mb_special');
        ...
        if ($result) {
            ...
            showMessage(getLang('nc_common_del_succ'), getUrl('mobile_manager/special/special_list'));
        } else {
            $this->log('删除手机专题' . '[ID:' . $_POST['special_id'] . ']', 0);
            showMessage(getLang('nc_common_del_fail'), getUrl('mobile_manager/special/special_list'));
        }
    }
...
//Tpl::showpage('mb_special_item.list');
        $this->view->pick('special/special_item_list');
    }

    /**
     * 编辑专题
     */
    public function special_editAction()
    {
        $model_mb_special = Model('mb_special');
...
//Tpl::showpage('mb_special_item.list');
     ...
     * 专题项目添加
     */
    public function special_item_addAction()
    {
        $model_mb_special = Model('mb_special');
        ...
        if ($param['item_type'] == 'adv_list') {
            $result = $model_mb_special->isMbSpecialItemExist($param); //判断广告是否已经存在
            if ($result) {
        ...
        if ($param['item_type'] == 'goods1') {
            ...
            if ($result) {
                echo json_encode(array('error' => '限时折扣板块只能添加一个'));
                die;
            }
        }
        //抢购板块只能添加一个
        if ($param['item_type'] == 'goods2') {
            $result = $model_mb_special->isMbSpecialItemExist($param);
            if ($result) {
                echo json_encode(array('error' => '抢购板块只能添加一个'));
                die;
            }
        }
     ...
     * 专题项目删除
     */
    public function special_item_delAction()
    {
        $model_mb_special = Model('mb_special');
     ...
     * 专题项目编辑
     */
    public function special_item_editAction()
    {
        $model_mb_special = Model('mb_special');
...
//        Tpl::showpage('mb_special_item.edit');
        $this->view->pick('special/special_item_edit');
    }

    /**
     * 专题项目保存
     */
    public function special_item_saveAction()
    {
        $model_mb_special = Model('mb_special');
        ...
        if ($result) {
            if ($_POST['special_id'] == $model_mb_special::INDEX_SPECIAL_ID) {
                showMessage(getLang('nc_common_save_succ'), getUrl('mobile_manager/special/index_edit'));
            } else {
                showMessage(getLang('nc_common_save_succ'), getUrl('mobile_manager/special/special_edit', array('special_id' => $_POST['special_id'])));
            }
        } else {
     ...
     * 图片上传
     */
    public function special_image_uploadAction()
    {
        $data = array();
...
//        Tpl::showpage('mb_special_widget.goods', 'null_layout');
        $this->view->pick('special/special_widget_goods');
     ...
     * 限时折扣商品列表
     */
    public function goods_xianshi_listAction()
    {
        $model_goods = Model('goods');
        $condition = array();
...
//        Tpl::showpage('mb_special_widget.goods', 'null_layout');
     ...
     * 抢购商品列表
     */
    public function goods_groupbuy_listAction()
    {
        $model_goods = Model('goods');
        $condition = array();
        $condition['goods_name'] = array('like', '%' . $_GET['keyword'] . '%');
...
//        Tpl::showpage('mb_special_widget.goods', 'null_layout');
     ...
     * 更新项目排序
     */
    public function update_item_sortAction()
    {
     ...
     * 更新项目启用状态
     */
    public function update_item_usableAction()
    {
        $model_mb_special = Model('mb_special');
    ...
    private function show_menu($menu_key = '')
    {
        ...
        if ($menu_key == 'index_edit') {
            ...
            $menu_array[] = array('menu_key' => 'special_list', 'menu_name' => '专题', 'menu_url' => getUrl('mobile_manager/special/special_list'));
        } else {
            $menu_array[] = array('menu_key' => 'index_edit', 'menu_name' => '首页', 'menu_url' => getUrl('mobile_manager/special/index'));
