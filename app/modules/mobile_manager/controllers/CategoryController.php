...
class CategoryController extends ControllerBase {
    ...
    public function indexAction() {
        $this->mb_category_listAction();
    }
    /**
     *
     */
    public function mb_category_listAction(){
        $lang   = getTranslation();
            ...
            if (is_array($_POST['del_id']) && !empty($_POST['del_id'])){
                foreach ($_POST['del_id'] as $k => $v){
                    ...
                    if (!empty($tmp['gc_thumb'])){
                        @unlink(UPLOAD_SITE_URL.DS.ATTACH_MOBILE.'/category/'.$tmp['gc_thumb']);
                    }
                    unset($tmp);
                    $model_link->del($v);
                }
...
//Tpl::showpage('mb_category.list');
        $this->view->pick('category/category_list');
    }

    /**
     * 删除
     */
    public function mb_category_delAction(){
        $lang   = getTranslation();
        if (intval($_GET['gc_id']) > 0){
            ...
            showMessage($lang['link_index_del_succ'],getUrl('mobile_manager/category/mb_category_list'));
        }else {
            showMessage($lang['link_index_choose_del'],getUrl('mobile_manager/category/mb_category_list'));
        }
    }
     ...
     * 添加
     */
    public function mb_category_addAction(){
        ...
        if ($_POST['form_submit'] == 'ok'){
            ...
            if ($result){
                $url = array(
                    array(
                        'url'=>getUrl('mobile_manager/category/mb_category_add'),
                        'msg'=>$lang['link_add_again'],
                    ),
                    array(
                        'url'=>getUrl('mobile_manager/category/mb_category_list'),
...
//Tpl::showpage('mb_category.add');
        $this->view->pick('category/category_add');
    }

    /**
     * 编辑
     */
    public function mb_category_editAction(){
        $lang   = getTranslation();
        $model_link = Model('mb_category');
        if ($_POST['form_submit'] == 'ok'){
            ...
            if ($result){
                ...
                if (!empty($_POST['gc_thumb']) && !empty($link_array['gc_thumb'])){
                    @unlink(UPLOAD_SITE_URL.DS.ATTACH_MOBILE.'/category/'.$link_array['gc_thumb']);
                }
                $url = array(
                    array(
                        'url'=>getUrl('mobile_manager/category/mb_category_edit',array('gc_id'=>intval($_POST['gc_id']))),
...
//Tpl::showpage('mb_category.edit');
        $this->view->pick('category/category_edit');
    }
    /**
     * ajax操作
     */
