...
class CommonController extends ControllerBase
{
    ...
    {
        $this->translation = getTranslation('admin_common,layout');
        ...
        $this->view->setVar('height', get_height($save_file));
        $this->view->setVar('width', get_width($save_file));
     ...
     * 设置常用菜单
     */
    public function common_operationsAction()
    {
        $type = $_GET['type'];
        $value = $_GET['value'];
     ...
     * 代办事项
     */
    public function pending_mattersAction()
    {
        $statistics = $this->get_pending_matters();
        Tpl::output('statistics', $statistics);
//
//        Tpl::showpage('common.pending_matters', 'null_layout');
        $this->view->pick('common/common_pending_matters');
    }

    /**
     * 代办事项ajax数据
     */
    public function ajax_pending_mattersAction()
    {
        $statistics = $this->get_pending_matters();
        $count = 0;
