...
class AreaLogic extends Model
{
    ...
    private function _getAllArea()
    {
        ...
        $temp = Area::find();
        if (count($temp) > 0) {
            $area_all_array = $temp->toArray();
        } else {
            $area_all_array = array();
        }
        //$area_all_array = $this->limit(false)->select();

        if (!empty($area_all_array)) {
    ...
    {
        $info_parent = Area::findFirst(array("conditions" => "area_id=" . $area_id, "columns" => "area_name,area_parent_id"));
        if ($info_parent) {
            $info_parent = $info_parent->toArray();
