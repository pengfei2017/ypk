 ...
 * @package Ypk\Logic
 */
class VoucherLogic extends Model
{
    ...
    public function __construct()
    {
        parent::__construct();
        //申请记录状态
        $this->applystate_arr = array('new' => array(1, getLang('voucher_applystate_new')), 'verify' => array(2, getLang('voucher_applystate_verify')), 'cancel' => array(3, getLang('voucher_applystate_cancel')));
        //套餐状态
        $this->quotastate_arr = array('activity' => array(1, getLang('voucher_quotastate_activity')), 'cancel' => array(2, getLang('voucher_quotastate_cancel')), 'expire' => array(3, getLang('voucher_quotastate_expire')));
        //代金券模板状态
    ...
    {
        return array('unused' => array(1, getLang('voucher_voucher_state_unused')), 'used' => array(2, getLang('voucher_voucher_state_used')), 'expire' => array(3, getLang('voucher_voucher_state_expire')));
    }
    ...
    {
        $info = read_file_cache($member_id, false, null, 'm_voucher/');
        if (empty($info['voucher_count']) && $info['voucher_count'] !== 0) {
            ...
            write_file_cache($member_id, array('voucher_count' => $voucher_count), null, 'm_voucher/');
        } else {
    ...
    public function getCanChangeTemplateInfo($vid, $member_id, $store_id = 0)
    {
        ...
        if ($store_id > 0 && $store_id == $template_info['voucher_t_store_id']) {
            return array('state' => false, 'msg' => '不可以兑换自己医生的代金券');
        }
        ...
        if (!empty($voucher_list)) {
            ...
            //买家最多只能拥有同一个医生尚未消费抵用的医生代金券最大数量的验证
            if ($voucher_count >= intval(getConfig('promotion_voucher_buyertimes_limit'))) {
                $message = sprintf('您的可用代金券已有%s张,不可再兑换了', getConfig('promotion_voucher_buyertimes_limit'));
    ...
    public function editVoucher($data, $condition, $member_id = 0)
    {
        ...
        if ($result && $member_id > 0) {
            write_file_cache($member_id, array('voucher_count' => null), null, 'm_voucher/');
        }
        return $result;
    }
    ...
    public function getVoucherTemplateList($where, $field = '*', $limit = 0, $page = 0, $order = '', $group = '')
    {
        ...
        //查询医生分类
        $store_class = read_file_cache('store_class', true);
        //会员级别
    ...
    public function exchangeVoucher($template_info, $member_id, $member_name = '')
    {
        ...
        if ($template_info['voucher_t_points'] > 0 && $template_info['voucher_t_gettype'] == $this->voucher_gettype_array['points']['sign']) {
            ...
            $points_arr['pl_desc'] = getLang('home_voucher') . $insert_arr['voucher_code'] . getLang('points_pointorderdesc');
        ...
        if ($result) {
            ...
            write_file_cache($member_id, array('voucher_count' => array('exp', 'voucher_count+1')), null, 'm_voucher/');
