 ...
 * @package Ypk\Logic
 */
class GoodsClassLogic extends Model
{
    /**
     * 缓存数据
     */
    ...
    public function getCache()
    {
        ...
        $data = read_file_cache('gc_class');
        if (!$data) {
            $data = array();
            foreach ($this->getGoodsClassList(array()) as $v) {
                $id = $v['gc_id']; //类别id
                $pid = $v['gc_parent_id']; //父类别id
                $data['data'][$id] = $v;
                $data['parent'][$id] = $pid;
    ...
    public function getGoodsClassForCacheModel()
    {
        ...
        foreach ($r as $k => & $v) {
            ...
            } else if (isset($p[$p[$p[$k]]]) && (string)$p[$p[$p[$k]]] == '0') {
                $v['depth'] = 3;
            }
        }
     ...
     * 取得医生绑定的分类
     *
     * @param   number $store_id 医生id
     * @param int|number $pid 父级分类id
     * @param int|number $deep 深度
     * @param null $group_id
     * @param int $gc_limits
     * @param   number $is_all_class 是否自营医生并绑定全部商品分类(接口使用)
     * @return array 二维数组
     */
    public function getGoodsClass($store_id, $pid = 0, $deep = 1, $group_id = null, $gc_limits = 1, $is_all_class = null)
    {
        ...
        if (!$is_all_class) {
            ...
            $model_storebindclass = new Store_bind_classLogic();
        ...
        if (!$gc_limits && $group_id) {
            $gc_list_group = (new SellerGroupBclassLogic())->getSellerGroupBclasList(array('group_id' => $group_id), '', '', 'distinct class_' . $deep, 'class_' . $deep);
            $temp = array();
    ...
    public function delGoodsClass($condition)
    {
        // 删除缓存
        $this->dropCache();
        return deleteAll('Ypk\Models\GoodsClass', $condition);
    }
    ...
    public function delGoodsClassByGcIdString($gcids)
    {
        ...
        // 删除商品分类
        if ($this->delGoodsClass(array('gc_id', array('in', $gcid_array))) == false) {
            return false;
        }

        // 删除常用商品分类
        (new GoodsClassStapleLogic())->delStaple(array('gc_id_1|gc_id_2|gc_id_3' => array('in', $gcid_array)));
        // 删除分类的tag表
        (new GoodsClassTagLogic())->delGoodsClassTag(array('gc_id_1|gc_id_2|gc_id_3' => array('in', $gcid_array)));
        // 删除医生绑定分类
        (new Store_bind_classLogic())->delStoreBindClass(array('class_1|class_2|class_3' => array('in', $gcid_array)));
        //删除商家权限组绑定的分类
        (new SellerGroupBclassLogic())->delSellerGroupBclass(array('class_1|class_2|class_3' => array('in', $gcid_array)));
        // 商品下架
        //todo 商品下架还没有做
        (new GoodsLogic())->editProducesLockUp(array('goods_stateremark' => '商品分类被删除，需要重新选择分类'), array('gc_id' => array('in', $gcid_array)));
        return true;
    }
     ...
     * 前台头部的商品分类
     *
     * @param int|number $update_all 是否更新（1更新，0不更新）
     * @return array 数组
     */
     ...
     * @return array
     */
    public function getAllGoodsClassNav()
    {
        $resArr = array();
        $cacheArr = read_file_cache('allGoodsClassList');
        if (isset($cacheArr) && !empty($cacheArr)) {
            $resArr = $cacheArr;
        } else {
            $Level1List = GoodsClass::find('gc_parent_id=0'); //获取所有的一级分类
            if (count($Level1List) > 0) {
                $resArr['level1'] = $Level1List->toArray();
                foreach ($resArr['level1'] as $val) {
                    $Level2List = GoodsClass::find('gc_parent_id=' . $val['gc_id']); //根据一级类别id获取二级分类集合
                    if (count($Level2List) > 0) {
                        $resArr['level2'][$val['gc_id']] = $Level2List->toArray();
                        foreach ($resArr['level2'][$val['gc_id']] as $val2) {
                            $Level3List = GoodsClass::find('gc_parent_id=' . $val2['gc_id']); //根据二级分类id获取三级分类集合
                            if (count($Level3List) > 0) {
                                $resArr['level3'][$val2['gc_id']] = $Level3List->toArray();
                            }
                        }
                    }
                }
                ...
                //获取一级类别下推荐分类集合
                foreach ($resArr['level1'] as $val) {
                    $cn_classids = GoodsClassNav::findFirst(array("conditions" => "gc_id='" . $val['gc_id'] . "'", "columns" => 'cn_classids')); //获取一级分类下的推荐分类id集合
                    if ($cn_classids) {
                        $cn_classids = $cn_classids->toArray();
                        if (strlen($cn_classids['cn_classids']) > 0) {
                            $recommendList = GoodsClass::find("gc_id in (" . $cn_classids['cn_classids'] . ")"); //获取推荐的类别集合
                            if (count($recommendList) > 0) {
                                $resArr['recommendList'][$val['gc_id']] = $recommendList->toArray();
                            }
                        }
                    }
                }
                ...
                //获取一级类别下的品牌集合
                foreach ($resArr['level1'] as $val) {
                    $cn_brandids = GoodsClassNav::findFirst(array("conditions" => "gc_id='" . $val['gc_id'] . "'", "columns" => 'cn_brandids'));
                    if ($cn_brandids) {
                        $cn_brandids = $cn_brandids->toArray();
                        if (strlen($cn_brandids['cn_brandids'] > 0)) {
                            $brandlist = Brand::find("brand_id in (" . $cn_brandids['cn_brandids'] . ")"); //获取品牌集合
                            if (count($brandlist) > 0) {
                                $resArr['brandList'][$val['gc_id']] = $brandlist->toArray();
                            }
                        }
                    }
                }
                ...
                //获取一级类别下的广告1和广告2
                foreach ($resArr['level1'] as $val) {
                    $adv1 = GoodsClassNav::findFirst(array("conditions" => "gc_id=" . $val['gc_id'], "columns" => "cn_adv1"));
                    if ($adv1) {
                        $adv1 = $adv1->toArray();
                        var_dump($adv1);
                        $resArr['cn_adv1'][$val['gc_id']] = $adv1['cn_adv1'];
                    }
                    $adv2 = GoodsClassNav::findFirst(array("conditions" => "gc_id=" . $val['gc_id'], "columns" => "cn_adv2"));
                    if ($adv2) {
                        $adv2 = $adv2->toArray();
                        var_dump($adv2);
                        $resArr['cn_adv2'][$val['gc_id']] = $adv2['cn_adv2'];
                    }
                }
            }
            write_file_cache('allGoodsClassList', $resArr);
        }
        return $resArr;
    }
     ...
     * 取分类列表，最多为三级
     *
     * @param int|string $show_deep 显示深度
    ...
    public function getGoodsClassNav($id = 0, $sign = 1)
    {
        if (intval($id) > 0) {
            ...
            if ($sign == 1) {
                ...
                $nav_link [] = array(
                    'title' => $data[$id]['gc_name'],
                    'link' => getUrl('shop/search/index', array('cate_id' => $data[$id]['gc_id']))
                );
            }
            ...
            // 加上 首页 商品分类导航
            $nav_link[] = array('title' => getLang('goods_class_index_search_results'));
        }
        // 首页导航
        $nav_link[] = array('title' => getLang('homepage'), 'link' => SHOP_SITE_URL);
    ...
    public function getKeyWords($gc_id = null)
    {
        ...
        $keywrods = read_file_cache('goods_class_seo', true);
