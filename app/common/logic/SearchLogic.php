...
class SearchLogic extends Model
{
     ...
     * @param int|number $pagesize 每页显示商品数
     * @return array|bool
     */
    public function getIndexerList($condition = array(), $order = array(), $pagesize = 24)
    {
        try {
            //全文搜索初始化
            $this->_createXS($pagesize, getConfig('fullindexer.appname'));
    ...
    public function getTagCategory($keyword = '')
    {
        if ($keyword != '') {
            ...
            $tag_list = read_file_cache('class_tag', true);
    ...
    {
        if (!getConfig('fullindexer.open')) return array(null, null);
    ...
    public function autoComplete($get)
    {
        ...
        if (!getConfig('fullindexer.open')) return array();
        try {
            require(BASE_PATH . '/api/xs/lib/XS.php');
            $obj_doc = new \XSDocument();
            $obj_xs = new \XS(getConfig('fullindexer.appname'));
            ...
            //             Log::record('search\auto_complete'.$e->getMessage(),Log::ERR);
        }
        return array();
    }
}