...
class AlbumLogic extends Model
{
     ...
     * @return array 二维数组
     */
    public function getPicList($condition = array())
    {
        $where = $this->getCondition($condition);
        $order = $condition['order'] ? $condition['order'] : 'apic_id desc';
        $result = AlbumPic::find(array('conditions' => $where, 'order' => $order));
        if ($result) {
    ...
    public function delAlbum($id)
    {
        ...
        if (is_array($id)) {
            $where = $id['conditions'];
            $result = AlbumClass::findFirst(array('conditions' => $where));
            if ($result) {
                $result->delete();
            } else {
                ...
                $where = $id['conditions'];
                $model = AlbumPic::findFirst(array('conditions' => $where));
                if ($model) {
                    $model->delete();
                } else {
                    return false;
