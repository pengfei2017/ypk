...
class GoodsLogic extends Model
{
    ...
    public function getGoodsList($condition, $field = '*', $group = '', $order = '', $limit = 0, $page = 0, $count = 0)
    {
        ...
        return Model('goods')->field($field)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->select();
    }
    ...
    public function calculateStorage($goods_list)
    {
        ...
        if (!empty($goods_list)) {
            ...
            $query_goods_common = Goods::query();
            $query_goods_common->inWhere('goods_commonid', $goodscommonid_array)->columns(array('goods_storage', 'goods_commonid', 'goods_id', 'goods_storage_alarm'));
            if (isset($_GET['cate_id']) && intval($_GET['cate_id']) > 0) {
                $gc_id_arr = $this->getRecursiveClass(intval($_GET['cate_id']));
                if (!empty($gc_id_arr)) {
                    $query_goods_common->inWhere('gc_id', $gc_id_arr);
                }
            }
            $goods_storage = $query_goods_common->execute(); //没有限制limit的查询
            $storage_array = array();
            foreach ($goods_storage->toArray() as $val) {
    ...
    public function editProduces($condition, $update1, $update2 = array(), $updateXS = false)
    {
        ...
        $goods_array = $this->getGoodsCommonList(parseWhere($condition), 'goods_commonid', 0);
    ...
    {
        $goodscommon = GoodsCommon::findFirst(array(
            "conditions" => getConditionsByParamArray($condition),
            "bind" => $condition,
            "columns" => $field
        ));

        if ($goodscommon == false) {
            $goodscommon = array();
        } else {
            $goodscommon = $goodscommon->toArray();
        }
    ...
    {
        return GoodsCommon::count($condition);
    }
     ...
     * @return array
     */
    public function getRecursiveClass($gc_id_param)
    {
        $gc_id_arr = array();
        if (isset($gc_id_param) && !is_array($gc_id_param)) {
            $logic_goods_class = new GoodsClassLogic();
            $gc_list = $logic_goods_class->getGoodsClassForCacheModel();
            if (!empty($gc_list[$gc_id_param])) {
                $gc_id[] = $gc_id_param;
                $gcchild_id = empty($gc_list[$gc_id_param]['child']) ? array() : explode(',', $gc_list[$gc_id_param]['child']);
                $gcchildchild_id = empty($gc_list[$gc_id_param]['childchild']) ? array() : explode(',', $gc_list[$gc_id_param]['childchild']);
                $gc_id_arr = array_merge($gc_id, $gcchild_id, $gcchildchild_id);
            }
        }
        return $gc_id_arr;
    }
