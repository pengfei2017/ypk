...
class goodsModel extends Model
{
    ...
    public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods_id asc', $page = 0, $limit = 0)
    {
        ...
        $goods_list = $this->getGoodsOnlineList($condition, $_field . ' nc_distinct,' . $field, $page, $order, $limit, '', false, $count);
        return $goods_list;
    }
    ...
    {
        $condition['goods_state'] = self::STATE1; //商品状态必须正常1
        $condition['goods_verify'] = self::VERIFY1; //商品必须通过审核1
    ...
    public function getGoodsCommendList($store_id, $limit = 5)
    {
        ...
        $goods_id_list = $this->getGoodsOnlineList(array('store_id' => $store_id, 'goods_commend' => 1), 'goods_id,goods_commonid', 0, 'rand()', $limit, 'goods_commonid,goods_id');
        if (!empty($goods_id_list)) {
            $tmp = array();
            foreach ($goods_id_list as $v) {
                $tmp[] = $v['goods_id'];
            }
            return $this->getGoodsOnlineList(array('goods_id' => array('in', $tmp)), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price', 0, 'rand()', $limit);
        }
        return array();
    }
    ...
    public function getGoodsInfoAndPromotionById($goods_id)
    {
        ...
        if (getConfig('promotion_allow') && APP_ID == 'mobile') {
            //$model = Model('p_sole');
            $model = new p_soleModel();
            $goods_info['sole_info'] = $model->getSoleGoodsInfoOpenByGoodsID($goods_info['goods_id']);
        }
    ...
    {
        return read_db_cache($goods_id, 'goods', $fields);
    }
    ...
    {
        return write_db_cache($goods_id, $goods_info, 'goods');
    }
    ...
    {
        return delete_db_cache($goods_id, 'goods');
    }
    ...
    {
        return read_db_cache($goods_commonid, 'goods_common', $fields);
    }
    ...
    {
        return write_db_cache($goods_commonid, $common_info, 'goods_common');
    }
    ...
    {
        return delete_db_cache($goods_commonid, 'goods_common');
    }
    ...
    {
        return read_db_cache($goods_commonid, 'goods_spec');
    }
    ...
    {
        return write_db_cache($goods_commonid, $spec_list, 'goods_spec');
    }
    ...
    {
        return delete_db_cache($goods_commonid, 'goods_spec');
    }
    ...
    {
        return read_db_cache($key, 'goods_image');
    }
    ...
    {
        return write_db_cache($key, $image_list, 'goods_image');
    }
    ...
    {
        return delete_db_cache($key, 'goods_image');
    }
    ...
    public function getGoodsDetail($goods_id)
    {
        ...
        $goods_info['time_long'] = $goods_info['spec_name']; //获取时长
        ...
        if (getConfig('cache_open')) {
            ...
            write_db_cache('updateRedisDate', array($goods_id => $goods_info['goods_click']), 'goodsClick');
        } else {
