...
class redpacketModel extends Model {
    ...
    public function getCurrentAvailableRedpacketCount($member_id) {
        $info = read_db_cache($member_id, 'm_redpacket', 'redpacket_count');
        if (empty($info['redpacket_count']) && $info['redpacket_count'] !== 0) {
            ...
            write_db_cache($member_id, array('redpacket_count' => $redpacket_count), 'm_redpacket');
        } else {
    ...
    public function updateRedpacketExpire($member_id){
        ...
        //清空缓存
        delete_db_cache($member_id, 'm_redpacket');
    }
    ...
    public function editRedpacket($where, $data, $member_id = 0) {
        ...
        if ($result && $member_id > 0){
            write_db_cache($member_id, array('redpacket_count' => null), 'm_redpacket');
        }
        return $result;
    }
    ...
    public function exchangeRedpacket($template_info, $member_id, $member_name = ''){
        ...
        if ($result){
            ...
            write_db_cache($member_id, array('redpacket_count' => array('exp','redpacket_count+1')), 'm_redpacket');
