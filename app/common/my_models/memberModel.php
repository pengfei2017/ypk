...
class memberModel extends Model
{
    ...
    {
        $member_info = read_db_cache($member_id, 'member', $fields);
        if (empty($member_info)) {
            $member_info = $this->getMemberInfo(array('member_id' => $member_id), '*', true);
            write_db_cache($member_id, $member_info, 'member');
        }
        return $member_info;
    }
    ...
    public function editMember($condition, $data)
    {
        ...
        if ($update && $condition['member_id']) {
            delete_db_cache($condition['member_id'], 'member');
        }
        return $update;
    }
    ...
    public function createSession($member_info = array(), $reg = false)
    {
        ...
        $_SESSION['is_login'] = '1'; //是否登录标识
        $_SESSION['member_id'] = $member_info['member_id']; //会员id
        $_SESSION['member_name'] = $member_info['member_name']; //会员名字
        $_SESSION['member_email'] = $member_info['member_email']; //会员电子邮箱
        $_SESSION['is_buy'] = isset($member_info['is_buy']) ? $member_info['is_buy'] : 1; //是否有购买权限
        $_SESSION['avatar'] = $member_info['member_avatar']; //会员头像
        $_SESSION['member_type_id'] = $member_info['member_type_id']; //会员身份id
...
//        }
        ...
        $member_info['member_type_id'] = $register_info['member_type_id'];
        $member_info['member_state'] = $register_info['member_state'];
        //添加邀请人(推荐人)
    ...
    public function addMember($param)
    {
        ...
        try {
            ...
            $member_info['member_state'] = is_null($param['member_state']) ? 1 : $param['member_state'];
            ...
            if (empty($param['inviter_id'])) {
                $member_info['inviter_id'] = 1; //如果邀请人不存在，则默认为平台推荐，值为1
            } else {
...
//            }
            ...
            $this->commit();
            //添加节点树
            create_member_tree_info($insert_id);
            return $insert_id;
        } catch (\Exception $e) {
            $this->rollback();
            return false;
    ...
    public function checkloginMember()
    {
        if (intval(getSession('is_login')) == 1) {
            @header("Location: " . getUrl('shop/index/index'));
            exit();
        }
    }
