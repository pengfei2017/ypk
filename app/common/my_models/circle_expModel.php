...
class circle_expModel extends Model {
    ...
    private function relOperation($param){
        $exp = 0;
        $today = date('Y-m-d');
        $dexp = $param['type'] == 'release' ? intval(getConfig('circle_exprelease')) : intval(getConfig('circle_expreply'));
        if(intval(getConfig('circle_expreleasemax')) != 0){
            ...
            if(!empty($exp_info) && $exp_info['em_time'] == $today){        // there are data
                $exp = ( $dexp > (intval(getConfig('circle_expreleasemax'))-intval($exp_info['em_exp'])) ) ? (intval(getConfig('circle_expreleasemax'))-intval($exp_info['em_exp'])) : $dexp;
            }else{                                                          // There is no data
                $exp = ( $dexp > intval(getConfig('circle_expreleasemax')) ) ? intval(getConfig('circle_expreleasemax')) : $dexp ;
            }
            $insert = array();
    ...
    private function repdOperation($param){
        ...
        if(intval(getConfig('circle_exprepliedmax')) != 0){
            ...
            if(!empty($exp_info) && $exp_info['et_time'] == $today){            // there are data
                $exp = ( intval(getConfig('circle_expreplied')) > (intval(getConfig('circle_exprepliedmax'))-intval($exp_info['et_exp'])) ) ? (intval(getConfig('circle_exprepliedmax'))-intval($exp_info['et_exp'])) : intval(getConfig('circle_expreplied'));
            }else{                                                              // There is no data
                $exp = ( intval(getConfig('circle_expreplied')) > (intval(getConfig('circle_exprepliedmax'))) ) ? intval(getConfig('circle_exprepliedmax')) : intval(getConfig('circle_expreplied'));
            }
            $insert = array();
            ...
            $exp = intval(getConfig('circle_expelease'));
        }
        if($exp != 0){
            $param['exp'] = $exp;
            return $this->expOperation(1, $param);
    ...
    private function expOperation($type, $param){
        ...
        if($type){
            ...
            if( intval($cm_info['cm_nextexp']) != 0 && (intval($cm_info['cm_exp'])+$param['exp']) >= intval($cm_info['cm_nextexp']) && intval($cm_info['cm_level']) != 16 ){
                $data = read_file_cache('circle_level', true);
    ...
    private function recordsLog($type, $param){
        ...
        switch ($param['type']){
            case 'release':
                ...
                $insert['el_type']  = 2;
                $insert['el_desc']  = getLang('circle_exp_release_theme');
                break;
            case 'reply':
                $update = array();
                ...
                $insert['el_type']  = 3;
                $insert['el_desc']  = getLang('circle_exp_reply');
                break;
            case 'replied':
                $update = array();
                $update['theme_exp']= array('exp', 'theme_exp+'.$param['exp']);
                ...
                $insert['el_type']  = 4;
                $insert['el_desc']  = getLang('circle_exp_theme_replied');
                break;
            case 'delRelease':
                $insert['el_type']  = 5;
                $insert['el_desc']  = getLang('circle_exp_theme_delete');
                break;
            case 'delReplied':
                $insert['el_type']  = 6;
                $insert['el_desc']  = getLang('circle_exp_reply_delete');
                break;
            case 'master':
                $insert['el_type']  = 1;
