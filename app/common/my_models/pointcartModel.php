...
class pointcartModel extends Model {
    ...
    public function addPointCart($param) {
        ...
        if($result) {
            //清除相关缓存
            write_db_cache($param['pmember_id'], array('pointcart_count' => null), 'm_pointcart');
            return $result;
        } else {
            return false;
        }
    ...
    public function countPointCart($member_id) {
        $info = read_db_cache($member_id, 'm_pointcart', 'pointcart_count');
        if (empty($info['pointcart_count']) && $info['pointcart_count'] !== 0) {
            ...
            write_db_cache($member_id, array('pointcart_count' => $pointcart_count), 'm_pointcart');
        } else {
    ...
    public function delPointCartById($pc_id,$member_id = 0){
        ...
        if ($result && $member_id > 0){
            write_db_cache($member_id, array('pointcart_count' => null), 'm_pointcart');
        }
        return $result;
    }
    /**
