...
class apiseccodeModel extends Model {
    ...
    /*private function addApiSeccodeToCache($key,$seccode) {
        ...
        write_db_cache($key,array('sec_val'=>encrypt(strtoupper($seccode),MD5_KEY),'sec_addtime'=>time()),'apiseccode');
        return true;
    }*/
    /**
     * 验证验证码
    ...
    /*private function checkApiSeccodeByCache($key,$value,$is_runout=true){
        $info = read_db_cache($key, 'apiseccode');
        if (!$info) {
            return false;
        }
        //超时失效
        if (time() - $info['sec_addtime'] > 3600) {
            delete_db_cache($key, 'apiseccode');
            return false;
        }
        //验证码是否正确
        $checkvalue = decrypt($info['sec_val'],MD5_KEY);
        ...
        if ($is_runout) {//无论成功与否都失效
            ...
            if (!$return) delete_db_cache($key, 'apiseccode');
        }
        return $return;
    }*/
    /**
