...
class pointorderModel extends Model{
    ...
    public function addPointOrder($param) {
        ...
        if ($result && $param['point_buyerid'] > 0){
            write_db_cache($param['point_buyerid'], array('pointordercount' => null), 'm_pointorder');
        }
        return $result;
    }
    /**
    ...
    public function cancelPointOrder($point_orderid, $member_id = 0) {
        ...
        if ($result){
            ...
            //更新会员缓存
            write_db_cache($order_info['point_buyerid'], array('pointordercount' => null), 'm_pointorder');
            ...
            $insert_arr['pl_desc']          = getLang('member_pointorder_cancel_tip1').$order_info['point_ordersn'].getLang('member_pointorder_cancel_tip2');
    ...
    public function getMemberPointsOrderGoodsCount($member_id){
        $info = read_db_cache($member_id, 'm_pointorder', 'pointordercount');
        if (empty($info['pointordercount']) && $info['pointordercount'] !== 0) {
            ...
            write_db_cache($member_id, array('pointordercount' => $pointordercount), 'm_pointorder');
        } else {
