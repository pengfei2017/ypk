...
class orderModel extends Model
{
    ...
    {
        if (!getConfig('cache_open')) return array();
        $type = 'ordercount' . $type;
        //$ins = Cache::getInstance('cacheredis');
        //$ins = read_db_cache('cacheredis');
        //$order_info = $ins->hget($id, $type, $key);
        delete_db_cache($id, $type);
        $order_info = read_db_cache($id, $type, $key);
    ...
    {
        if (!getConfig('cache_open') || empty($type) || !intval($id) || !is_array($data)) return;
        //$ins = Cache::getInstance('cacheredis');
        $type = 'ordercount' . $type;
        //$ins->hset($id, $type, $data);
        write_db_cache($id, $data, $type);
    }
    ...
    {
        if (!getConfig('cache_open')) return true;
        //$ins = Cache::getInstance('cacheredis');
        $type = 'ordercount' . $type;

        return delete_db_cache($id,$type);
        ...
        switch ($operate) {
            ...
            case 'complain':
                $state = in_array($order_info['order_state'], array(ORDER_STATE_PAY, ORDER_STATE_SEND)) ||
                    intval($order_info['finnshed_time']) > (TIMESTAMP - getConfig('complain_time_limit'));
                break;
