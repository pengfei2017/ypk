...
class goods_browseModel extends Model
{
    ...
    public function getViewedGoodsList($member_id = 0, $shownum = 0)
    {
        ...
        if ($member_id > 0) {
            if (!getConfig('cache_open')) {//查询数据库
                ...
                $hash_key = $member_id;
                //先查找$hash_key缓存
                $browse_goodsid = read_db_cache($hash_key, 'goodsbrowse', 'goodsid');
                ...
                if ($goodsid_arr) {
                    ...
                    //获得缓存的浏览商品信息
                    $browselist_tmp = read_db_cache($hash_key, 'goodsbrowse', implode(',', $goodsid_arr));
     ...
     * @param int $member_id 会员ID（一般传递$_SESSION['member_id']）
     * @param int $store_id 该会员的医生ID（一般传递$_SESSION['store_id']）
     * @return array
     */
    public function saveViewedGoods($goods_id, $member_id = 0, $store_id = 0)
    {
        ...
        if ((!$goods_info) || $store_id == $goods_info['store_id']) {//医生浏览自己的商品不加入浏览历史
            return false;
        }
        if (GoodsBrowse::count("goods_id=" . $goods_id . " and member_id=" . $member_id) < 0) {
    ...
    public function addViewedGoodsToCache($member_id)
    {
        ...
        $hash_key = $member_id;
        //处理浏览历史cache中商品ID
        $_cache = read_db_cache($hash_key, 'goodsbrowse', 'goodsid');
        //查询浏览历史
        ...
        //缓存商品信息
        write_db_cache($hash_key, $_cache, 'goodsbrowse');
        return true;
    }
