...
class web_chatModel extends Model
{
    ...
    public function addMsg($msg)
    {
        ...
        $msg['t_msg'] = autolink($msg['t_msg']); //正则匹配聊天内容中的url地址和关键字，自动给它们添加<a>链接
        $msg['add_time'] = time();
        ...
        if ($m_id > 0) {
            ...
            $goods_info = array();
            /**
             * //匹配类似  #http://developer.site/shop/goods/index?goods_id=100012$# 这样的消息中的goods_id的值
             * //聊天消息中如果发送了商品详情页路径，商品消息会更美观的显示
             * //消息包含路径的格式是路径必须放在#和$#之间，否则当作普通url文本处理，不匹配，有些应用场景需要发原始路径
             * //判断字符串以#开头并且以$#结尾
             */
            //判断字符串以#开头并且以$#结尾
            //if (substr($t_msg, 0, strlen('#')) === '#' && substr($t_msg, -strlen('$#')) === '$#') {

            //判断是否发送消息中含有本网站的商品，别的网站上的商品无法进行图文消息显示
            //只要返回的消息json字符串中含有商品$msg['goods_id']字段，该条消息的商品就会自动在消息窗口中显示成图文样式
            if (strpos($t_msg, FIRST_LEVEL_DOMAIN_NAME) !== false) {
                //检查消息中是否包含goods_id参数
                if (strpos($t_msg, 'goods_id=') !== false) {
                    $temp_arr = explode('goods_id=', $t_msg);
                    if (count($temp_arr) > 1) {
                        $temp = $temp_arr[1];
    ...
    public function getMemberRecentList($condition = array(), $page = '', $limit = '')
    {
        ...
        if ($msg['count'] > 0) {
            ...
            $list = $this->table('chat_msg')->field($field)->group('t_id,t_name')->where($condition)->limit($limit)->page($page, $count)->order('addtime desc')->select();
        }
        return $list;
    }
    ...
    public function getMemberFromList($condition = array(), $page = '', $limit = '')
    {
        ...
        if ($msg['count'] > 0) {
            ...
            $list = $this->table('chat_msg')->field($field)->group('f_id,f_name')->where($condition)->limit($limit)->page($page, $count)->order('addtime desc')->select();
        }
        return $list;
    }
    ...
    public function getGoodsInfo($goods_id)
    {
        ...
        if (is_array($goods) && !empty($goods)) {
            $goods['url'] = getUrl('goods/index', array('goods_id' => $goods['goods_id']));
