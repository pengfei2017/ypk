...
class sns_bindingModel extends Model{
    ...
    public function getUsableApp($member_id){
        ...
        foreach ($app_arr as $k=>$v){
            if (getConfig('share_'.$k.'_isuse') == 1){
                $v['isbind'] = false;
                $app_arrnew[$k] = $v;
            }
        }
    ...
    public function getApps(){
        $app_arr = array();
        $app_arr['qqweibo'] = array('name'=>getLang('nc_shareset_qqweibo'),'url'=>"http://t.qq.com",'applyurl'=>'http://dev.t.qq.com');
        $app_arr['sinaweibo'] = array('name'=>getLang('nc_shareset_sinaweibo'),'url'=>"http://www.weibo.com",'applyurl'=>'http://open.weibo.com/developers');
        return $app_arr;
    }
    /**
    ...
    public function addQQWeiboPic($bindinfo,$params){
        ...
        include_once(BASE_API_PATH.DS.'snsapi'.DS.'qqweibo'.DS.'tencent.php');
    ...
    public function addSinaWeiboUpload($bindinfo,$params){
        include_once(BASE_API_PATH.DS.'snsapi'.DS.'sinaweibo'.DS.'config.php');
        include_once(BASE_API_PATH.DS.'snsapi'.DS.'sinaweibo'.DS.'saetv2.ex.class.php');
        $c = new SaeTClientV2( getConfig('share_sinaweibo_appid') , getConfig('share_sinaweibo_appkey') , $bindinfo['snsbind_accesstoken']);
        if($params['images']) {
