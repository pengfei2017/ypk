...
class evaluate_goodsModel extends Model
{
    ...
    public function getEvaluateGoodsInfoByGoodsID($goods_id)
    {
        ...
        $info = read_db_cache($goods_id, $prefix);
        if (empty($info)) {
            $info = array();
            ...
            write_db_cache($goods_id, $info, $prefix);
        }
        return $info;
    }
    ...
    public function getEvaluateGoodsInfoByCommonidID($goods_commonid)
    {
        ...
        $info = read_db_cache($goods_commonid, $prefix);
        if (empty($info)) {
            $info = array();
            $info['good_percent'] = 100;
            $info['normal_percent'] = 0;
            ...
            write_db_cache($goods_commonid, $info, $prefix, 24 * 60); // 缓存周期1天。
        }
        return $info;
    }
    ...
    public function addEvaluateGoodsArray($param, $goodsid_array)
    {
        ...
        if ($result && !empty($goodsid_array)) {
            foreach ($goodsid_array as $goods_id) {
                delete_db_cache($goods_id, 'goods_evaluation');
            }
        }
        return $result;
    }
     ...
     * 例：delete_db_cache($goods_id, 'goods_evaluation');
     */
