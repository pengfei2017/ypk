...
class advModel extends Model
{
    ...
    public function makeApAllCache()
    {
        ...
        foreach ((array)$ap_list as $v) {
            ...
            $apId = (int)$v['ap_id'];
            if (getConfig('cache_open')) {
                write_file_cache("adv_{$apId}", $v);
            } else {
                write_file_cache(BASE_PATH . '/cache/adv/' . $apId . '.php', $v);
            }
        }
    }
    ...
    {
        $apId = (int)$apId;
        return read_file_cache("adv_{$apId}", array($this, 'getApByCacheId'));
    }

    /**
     * 通过缓存id获取广告，生成缓存时使用
     *
     * @param string $apCacheId 格式为 adv_{ap_id}
     */
    public function getApByCacheId($apCacheId)
    {
        $apId = substr($apCacheId, strlen('adv_'));
