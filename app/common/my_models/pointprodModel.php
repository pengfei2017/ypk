...
class pointprodModel extends Model {
    ...
    public function getPointProdViewNum($prod_id,$pgoods_view = ''){
        ...
        if (getConfig('cache_open')){//如果开启缓存，则读取缓存的浏览次数
            $prod_info = read_db_cache($prod_id, 'pointprod', 'pgoods_view,view_updatetime');
            if ($prod_info){
                $is_data = false;
            }
        }
    ...
    public function editPointProdViewnum($prod_id){
        ...
        $cache_arr = array();
        $tmptime = time();
        if (!getConfig('cache_open')){//直接更新数据库浏览次数
            ...
            if (empty($prod_info)){//如果兑换礼品的浏览次数缓存不存在，则查询兑换礼品数据库信息，建立缓存
                ...
                write_db_cache($prod_id, array('pgoods_view'=>$viewnum,'view_updatetime'=>$tmptime), 'pointprod');
            } else {
                $viewnum = intval($prod_info['pgoods_view']) + 1;
                if (($prod_info['view_updatetime']+3600) < $tmptime){//如果缓存时间超出1小时，则将更新进入数据库，时间初始为当前时间
                    ...
                    write_db_cache($prod_id, array('pgoods_view'=>$viewnum), 'pointprod');
                }
            }
        }
