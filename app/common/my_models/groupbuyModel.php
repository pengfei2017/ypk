...
class groupbuyModel extends Model{
     ...
     * @param int $store_id 如果提供医生编号，判断是否为该医生活动，如果不是返回null
     * @return array 抢购信息
     *
     */
    ...
    private function _rGoodsGroupbuyCache($goods_commonid) {
        return read_db_cache($goods_commonid, 'goods_groupbuy');
    }
    ...
    private function _wGoodsGroupbuyCache($goods_commonid, $info) {
        return write_db_cache($goods_commonid, $info, 'goods_groupbuy');
    }
    ...
    private function _dGoodsGroupbuyCache($goods_commonid) {
        return delete_db_cache($goods_commonid, 'goods_groupbuy');
    }
