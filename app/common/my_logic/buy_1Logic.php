<?php
/**
 * Created by hepengfei.
 * User: Administrator
 * Date: 2016/11/21
 * Time: 23:40
 */

namespace Ypk\MyLogic;
...
class buy_1Logic
{
     ...
     * @param unknown $store_cart_list 以医生ID分组的购物车商品信息
     * @return array
     */
    ...
    /**
     * 取得医生级优惠 - 跟据商品金额返回每个医生当前符合的一条活动规则，如果有赠品，则自动追加到购买列表，价格为0
     * @param unknown $store_goods_total 每个医生的商品金额小计，以医生ID为下标
     * @return array($premiums_list,$mansong_rule_list) 分别为赠品列表[下标自增]，医生满送规则列表[医生ID为下标]
     */
    public function getMansongRuleCartListByTotal($store_goods_total)
    {
        if (!getConfig('promotion_allow') || empty($store_goods_total) || !is_array($store_goods_total)) return array(array(), array());
    ...
    /**
     * 重新计算每个医生最终商品总金额(最初计算金额减去各种优惠/加运费)
     * @param array $store_goods_total 医生商品总金额
     * @param array $preferential_array 医生优惠活动内容
     * @param string $preferential_type 优惠类型
     * @return array 返回扣除优惠后的医生商品总金额
     */
    public function reCalcGoodsTotal($store_goods_total, $preferential_array = array(), $preferential_type)
    {
        ...
        switch ($preferential_type) {
            case 'mansong':
                if (!getConfig('promotion_allow')) return $store_goods_total;
            ...
            case 'voucher':
                if (!getConfig('voucher_allow')) return $store_goods_total;
    ...
    /**
     * 取得医生可用的代金券
     * @param array $store_goods_total array(医生ID=>商品总金额)
     * @return array
     */
    public function getStoreAvailableVoucherList($store_goods_total, $member_id)
    {
        if (!getConfig('voucher_allow')) return array();
        $voucher_list = array();
    ...
    {
        if (!getConfig('redpacket_allow')) return array();
        $condition = array();
    ...
    /**
     *
     * @param array $store_order_total 每个医生应付总金额(含运费)
     ...
     * @param array $store_goods_total (医生ID=>商品总金额)
     * @return array
     */
    ...
    public function getXianshiInfo(& $goods_info, $quantity)
    {
        ...
        if (!getConfig('promotion_allow') || empty($goods_info['xianshi_info']) || !empty($goods_info['groupbuy_info']) || $goods_info['ifsole']) return;
    ...
    public function getOfflineGoodsPay($buy_list)
    {
        ...
        if ($offline_pay) {
            //下单里包括平台自营商品并且平台已开启货到付款，则显示货到付款项及对应商品数量,取出支持货到付款的医生ID组成的数组，目前就一个，DEFAULT_PLATFORM_STORE_ID
    ...
    /**
     * 计算每个医生(所有医生级优惠活动)总共优惠多少金额,商品金额+运费-最终结算金额=优惠了多少
     * @param array $store_goods_total 最初医生商品总金额
     * @param array $store_freight_total 各医生运费
     * @param array $store_final_goods_total 去除各种医生级促销后，最终医生商品总金额(不含运费)
     * @return array
     */
    ...
    /**
     * 跟据商品金额返回免运费医生ID及免运费下限金额描述[已设置并达到免运费金额标准]
     * 不再返回需要计算运费的医生ID组成的数组[未达到免运费标准或未设置免运费金额]
     * @param array $store_goods_total 每个医生的商品金额小计，以医生ID为下标
     * @return array
     */
    ...
    /**
     * 取得医生运费信息
     * 先将免运费的医生运费置0，然后算出医生里没使用运费模板的商品运费之和 ，存到iscalced下标中
     * 然后再计算使用运费模板的信息(array(医生ID=>运费模板ID)，放到tpl_ids下标里
     * @param array $buy_list 购买商品列表
     * @param array $free_freight_sid_list 免运费的医生ID数组
     */
    public function getStoreFreightList($buy_list = array(), $free_freight_sid_list)
    {
        //定义返回数组
        $return = array();
        //先将免运费的医生运费置0(格式:医生ID=>0)
        $freight_list = array();
        ...
        //然后算出医生里没使用运费模板(优惠套装商品除外)的商品运费之和(格式:医生ID=>运费)
        //定义数组，存放医生优惠套装商品运费总额 store_id=>运费
        $store_bl_goods_freight = array();
        foreach ($buy_list as $key => $goods_info) {
            //免运费医生的商品不需要计算，但如果设置了配送区域，则需要保存该模板ID
    ...
    /**
     * 根据地区选择计算出所有医生最终运费
     * @param array $freight_list 运费信息(医生ID，运费，运费模板ID)
     * @param int $city_id 市级ID
     * @return array 返回医生ID=>运费
     */
    public function calcStoreFreight($freight_list, $city_id)
    {
        ...
        //使用运费模板的信息(array(医生ID=>array(运费模板ID=>购买数量))
    ...
    public function editGoodsNum($buy_items)
    {
        foreach ($buy_items as $goods_id => $buy_num) {
            ...
            if (!$result) throw new \Exception(getLang('cart_step2_submit_fail'));
        }
    }

    /**
     * 取得医生级活动 - 每个医生可用的满即送活动规则列表
     * @param unknown $store_id_array 医生ID数组
     */
    public function getMansongRuleList($store_id_array)
    {
        if (!getConfig('promotion_allow') || empty($store_id_array) || !is_array($store_id_array)) return array();
        $model_mansong = Model('p_mansong');
        $mansong_rule_list = array();
    ...
    /**
     * 取得哪些医生有满免运费活动
     * @param array $store_id_array 医生ID数组
     * @return array
     */
    ...
    /**
     * 计算本次下单中每个医生订单是货到付款还是线上支付,医生ID=>付款方式[online在线支付offline货到付款]
     * @param array $store_id_array 医生ID数组
    ...
    {
        if (!getConfig('groupbuy_allow') || empty($goods_info['groupbuy_info']) || $goods_info['ifsole']) return;
    ...
    {
        if (!getConfig('promotion_allow') || empty($goods_info['sole_info'])) return;
    ...
    public function getBookInfo(& $goods_info = array())
    {
        ...
        if (!getConfig('promotion_allow') || $goods_info['book_down_time'] < TIMESTAMP) {
    ...
    {
        if (!getConfig('groupbuy_allow') || empty($cart_list)) return;
        foreach ($cart_list as $key => $cart_info) {
    ...
    {
        if (!getConfig('promotion_allow') || empty($cart_list)) return;
        foreach ($cart_list as $key => $cart_info) {
    ...
    private function _parseMansongRuleDesc($rule_info)
    {
        ...
        if (APP_ID == 'mobile') {
            ...
            if (!empty($rule_info['mansong_goods_name'])) {
                $goods_desc = " 送<a href='" . getUrl('shop/goods/index', array('goods_id' => $rule_info['goods_id'])) . "' title='{$rule_info['mansong_goods_name']}' target='_blank'>[赠品]</a>";
                $goods_desc .= '<a class="ncc-store-gift" title="' . $rule_info['mansong_goods_name'] . '" target="_blank" href="' . getUrl('shop/goods/index', array('goods_id' => $rule_info['goods_id'])) . '"><img alt="' . $rule_info['mansong_goods_name'] . '" src="' . cthumb($rule_info['goods_image'], 60) . '"></a>';
            }
