...
class buy_virtualLogic
{
    ...
    public function getBuyStepData($goods_id, $quantity, $member_id)
    {
        ...
        if (empty($goods_info)) {
            return queue_callback(false, '该商品不符合购买条件，可能的原因有：下架、不存在、过期等');
        }
        $member_info = Member::findFirst("member_id=" . $member_id);
        if ($member_info === false) {
            return queue_callback(false, '买家不存在，请重新登录');
        }
        if ($member_info->getMemberTypeId() > 1) { //表示购买者是平台或医务人员
            $store_info = Store::findFirst("member_id=" . $member_id);
            if ($store_info !== false) {
                if ($goods_info['store_id'] == $store_info->getStoreId()) {
                    return queue_callback(false, '不允许购买自己发布的商品');
                }
            }
        }
        ...
        if ($quantity > $goods_info['goods_storage']) {
            return queue_callback(false, '该商品库存不足');
        }
        ...
        return queue_callback(true, '', $return);
    }
    ...
    public function buyStep3($post, $member_id)
    {
        ...
        //应付总金额计算
        $pay_total = $goods_info['goods_price'] * $goods_info['quantity']; //单价乘以数量
        ...
        return queue_callback(true, '', array('order_id' => $order_info['order_id'], 'order_sn' => $order_info['order_sn']));
    }
    ...
    public function pdPay($order_info, $input, $buyer_info)
    {
        ...
        if ($available_pd_amount >= $order_amount) {
            ...
            $available_pd_amount -= $order_amount; //减少预存款
            ...
            $result = $model_vr_order->editOrder($data_order, array('order_id' => $order_info['order_id'])); //更新订单状态
            if (!$result) {
                throw new \Exception('订单更新失败');
            }
            ...
            if ($goods_model !== false) {
                $param['goods_name'] = $goods_model->getGoodsName();
            }
            QueueClient::push('sendVrCode', $param); //发送虚拟兑换码给买家
            // 支付成功发送医生消息
            $param = array();
            $param['code'] = 'new_order';
            $param['store_id'] = $order_info['store_id'];
    ...
    {
        if (!getConfig('groupbuy_allow') || empty($goods_info) || !is_array($goods_info)) return $goods_info;
