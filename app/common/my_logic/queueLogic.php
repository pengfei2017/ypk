...
use Ypk\Log;
use Ypk\sendMemberMsg;
use Ypk\sendStoreMsg;
use Ypk\Sms;
...
class queueLogic
{
    ...
    public function editGroupbuySaleCount($groupbuy_info)
    {
        ...
        if (!$update) {
            return queue_callback(false, '更新抢购信息失败groupbuy_id:' . $groupbuy_info['groupbuy_id']);
        } else {
            return queue_callback(true);
        }
    }
        ...
        foreach ($voucher_list as $store_id => $voucher_info) {
            ...
            if ($update) {
                ...
                if ($update) {
                    ...
                    $param['voucher_url'] = getUrl('member/member_voucher/index');
                    $send->send($param);
                } else {
                    return queue_callback(false, '更新代金券状态失败tpl:' . $voucher_info['voucher_t_id']);
                }
            } else {
                return queue_callback(false, '更新代金券状态失败vcode:' . $voucher_info['voucher_code']);
            }
        }
        return queue_callback(true);
    }
    ...
    public function editRptState($input_rpt_info, $pay_sn)
    {
        ...
        if ($update) {
            ...
            if ($update) {
                ...
                $param['rpacket_url'] = getUrl('member/member_redpacket/index');
                $send->send($param);
            } else {
                return queue_callback(false, '更新红包状态失败tpl:' . $input_rpt_info['rpacket_t_id']);
            }
        } else {
            return queue_callback(false, '更新红包状态失败vcode:' . $input_rpt_info['rpacket_code']);
        }
        return queue_callback(true);
    }
    ...
    public function createOrderUpdateStorage($goods_buy_quantity)
    {
        ...
        if (!$result) {
            return queue_callback(false, '变更商品库存与销量失败');
        } else {
            return queue_callback(true);
        }
    }
    ...
    public function createOrderUpdateChainStorage($goods_buy_quantity, $chain_id)
    {
        ...
        if (!$result) {
            return queue_callback(false, '变更门店自提点商品库存失败');
        } else {
            return queue_callback(true);
        }
    }
    ...
    public function updateGoodsFCode($fc_id)
    {
        ...
        if (!$update) {
            return queue_callback(false, '更新F码使用状态失败fc_id:' . $fc_id);
        } else {
            return queue_callback(true);
        }
    }
    ...
    {
        if (!is_array($cart['cart_ids']) || empty($cart['buyer_id'])) return queue_callback(true);
        $del = Model('cart')->delCart('db', array('buyer_id' => $cart['buyer_id'], 'cart_id' => array('in', $cart['cart_ids'])));
        if (!$del) {
            return queue_callback(false, '删除购物车数据失败');
        } else {
            return queue_callback(true);
        }
    }
    ...
    public function updateGoodsPromotionPriceByGoodsId($goods_id)
    {
        ...
        if (!$update) {
            return queue_callback(false, '根据商品ID更新促销价格失败');
        } else {
            return queue_callback(true);
        }
    }
    ...
    public function updateGoodsPromotionPriceByGoodsCommonId($goods_commonid)
    {
        ...
        if (!$update) {
            return queue_callback(false, '根据商品公共id更新促销价格失败');
        } else {
            return queue_callback(true);
        }
    }
    ...
    public function createGoodsFCode($param)
    {
        ...
        if (!empty($insert)) {
            ...
            if (!$insert) {
                return queue_callback(false, '生成商品F码失败goods_id:' . $param['goods_id']);
            }
        }
        return queue_callback(true);
    }
    ...
    public function createGoodsQRCode($param)
    {
        ...
        // 生成商品二维码
        require_once(RESOURCE_PATH . DS . 'phpqrcode' . DS . 'index.php');
        ...
        foreach ($param['goodsid_array'] as $goods_id) {
            // 生成商品二维码
            $PhpQRCode->set('date', getUrl('mb_app/index', array('goods_id' => $goods_id)));
    ...
    {
        if (empty($order_info)) return queue_callback(true);
    ...
    {
        if (empty($param) && !is_array($param)) return queue_callback(true);
        $condition = array();
        ...
        if (empty($code_list)) return queue_callback(true);
        ...
        $data = array();
        $data['site_name'] = getConfig('site_name'); //站点名称
        $data['goods_name'] = $param['goods_name']; //商品名称
        $data['vr_code'] = rtrim($content, ','); //兑换码
        $message = ncReplaceText($tpl_info['content'], $data); //生成包含兑换码的短信息
        $sms = new \Ypk\Sms();
        $result = $sms->send($param["buyer_phone"], $message . '购买的产品是：' . $param['goods_name']);
        if (!$result) {
            return queue_callback(false, '兑换码发送失败order_id:' . $param['order_id']);
        } else {
            return queue_callback(true);
        }
    }
    ...
    {
        if (!is_array($param['order_sn_list'])) return queue_callback(true);
        $data = array();
        ...
        foreach ($param['order_sn_list'] as $order_id => $v) {
            ...
            if (!$insert) {
                return queue_callback(false, '保存自提站订单信息失败order_sn:' . $v['order_sn']);
            }
        }
        return queue_callback(true);
    }
    ...
    public function sendPickupcode($param)
    {
        ...
        if (!$result) {
            return queue_callback(false, '发送提货码短信消息失败order_id:' . $param['order_id']);
        } else {
            return queue_callback(true);
        }
    }
    ...
    public function sendChainCode($order_info)
    {
        ...
        if (!$result) {
            return queue_callback(false, '发送门店提货码短信消息失败order_sn:' . $order_info['order_sn']);
        } else {
            return queue_callback(true);
        }
    }
    ...
    public function flushIndexer()
    {
        try {
            ...
            $obj_xs = new \XS(getConfig('fullindexer.appname'));
    ...
    public function buildPwdvoucher($t_id)
    {
        $t_id = intval($t_id);
        if ($t_id <= 0) {
            return queue_callback(false, '参数错误');
        }
        $model_voucher = Model('voucher');
        //查询代金券详情
        $where = array();
        ...
        if ($t_total <= 0) {
            return queue_callback(false, '代金券模板信息错误');
        }
        $is_succ = false;
        while ($t_total > 0) {
            $insert_arr = array();
    ...
    public function buildPwdRedpacket($t_id)
    {
        ...
        if ($t_total <= 0) {
            return queue_callback(false, '红包模板信息错误');
        }
        $is_succ = false;
        while ($t_total > 0) {
            $insert_arr = array();
    ...
    /**
     * 更新医生已有商品保障服务状态
     * @param array $param item_id 保障服务ID,store_id 医生ID
     */
    public function updateStoreGoodsContract($param)
    {
        ...
        if (!$c_info) {
            return queue_callback(false, '医生保障服务信息错误');
        }
     ...
     * @return array
     */
    public function create_member_tree_info($member_id)
    {
        return create_member_tree_info($member_id);
    }
     ...
     * @return array
     */
    public function update_points_and_reward($order_info)
    {
        return update_points_and_reward($order_info);
    }
