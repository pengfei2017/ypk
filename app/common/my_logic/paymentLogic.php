...
class paymentLogic
{
    ...
    public function getRealOrderInfo($pay_sn, $member_id = null)
    {
        ...
        if (empty($order_pay_info)) {
            return queue_callback(false, '该支付单不存在');
        }
        ...
        return queue_callback(true, '', $order_pay_info);
    }
    ...
    public function getOrderExtendList(& $order_list, $role = '')
    {
        ...
        if ($order_list[0]['order_type'] == 2) {
            ...
            if (!$result['data']['if_buyer_pay'] && $role != 'admin') {
                return queue_callback(false, '未找到需要支付的订单');
            }
    ...
    public function getVrOrderInfo($order_sn, $member_id = null)
    {
        ...
        if (empty($order_info)) {
            return queue_callback(false, '该订单不存在');
        }
    ...
    public function getPaymentInfo($payment_code)
    {
        if (in_array($payment_code, array('offline', 'predeposit')) || empty($payment_code)) {
            return queue_callback(false, '系统不支持选定的支付方式');
        }
        ...
        return queue_callback(true, '', $payment_info);
    }
    ...
    public function updatePdOrder($out_trade_no, $trade_no, $payment_info, $recharge_info)
    {
        ...
        try {
            ...
            //todo 会员预付款充值后更新会员金银铜等级
            $update_member_tree_level_params = array(
                "member_id" => $recharge_info['pdr_member_id'],
                "member_name" => $recharge_info['pdr_member_name'],
                "money" => $recharge_info['pdr_amount'],
                "update_type" => 1,
            );
