...
class buyLogic
{
    ...
    public function getCartList($cart_id, $member_id, $jjg = null, $orderdiscounts = array())
    {
        ...
        if (empty($buy_items)) {
            return queue_callback(false, '所购商品无效');
        }

        if (count($buy_items) > 50) {
            return queue_callback(false, '一次最多只可购买50种商品');
        }
        ...
        if (empty($store_cart_list) || !is_array($store_cart_list)) {
            return queue_callback(false, '提交数据错误');
        }

        return queue_callback(true, '', array(
    ...
    public function getGoodsList($cart_id, $member_id, $store_id, $orderdiscounts = array())
    {
        ...
        if (empty($goods_info)) {
            return queue_callback(false, '商品已下架或不存在');
        }

        //不能购买自己医生的商品
        if ($goods_info['store_id'] == $store_id) {
            return queue_callback(false, '不能购买自己发布的商品');
        }
        ...
        return queue_callback(true, '', array('goods_list' => $goods_list, 'store_cart_list' => $store_cart_list, 'is_book' => $goods_info['is_book']));
    }
    ...
    public function getBuyStep1Data($member_id, $data, $orderdiscount = 0)
    {
        ...
        return queue_callback(true, '', $result);
    }
    ...
    public function buyStep2($post, $member_id, $member_name, $member_email, $orderdiscount = 0, $member_level = 0)
    {
        ...
        try {
            ...
            return queue_callback(true, '', $this->_order_data);

        } catch (\Exception $e) {
            $model->rollback();
            return queue_callback(false, $e->getMessage());
        }
     ...
     * 如果医生未设置满免规则，且使用运费模板，按运费模板计算，如果其中有商品使用相同的运费模板,作为一种商品算运费
     * 如果未找到运费模板，按免运费处理
    ...
    public function changeAddr($freight_hash, $city_id, $area_id, $member_id)
    {
        ...
        //将hash解密，得到运费信息(医生ID，运费,运费模板ID),hash内容有效期为1小时
...
//             $order_platform_store_ids[$k] = null;
    ...
    public function checkFcode($goods_id, $fcode)
    {
        ...
        if ($fcode_info) {
            return queue_callback(true, '', $fcode_info);
        } else {
            return queue_callback(false, 'F码错误');
        }
    }
    ...
    private function _createOrderStep1()
    {
        ...
        if (!$post['chain']['id']) {
            ...
            if (!is_array($input_if_offpay_batch)) {
                throw new \Exception('订单保存出现异常[部分医生付款方式出现异常]，请重试');
            }
        } else {
    ...
    private function _createOrderStep3()
    {
        ...
        if (!$input_is_book) {
            ...
            //计算每个医生(所有医生级优惠活动，代金券，满减)总共优惠多少
            ...
            //重新计算优惠金额,将医生红包减去运费的余额追加到医生总优惠里
    ...
    private function _createOrderStep4()
    {
        ...
        if ($input_pay_name == 'chain' && $input_chain_id) {
            ...
            //每个医生订单是货到付款还是线上支付,医生ID=>付款方式[在线支付/货到付款]
...
//             $should_goods_total = $store_final_order_total[$store_id]-$store_freight_total[$store_id]+$promotion_total;
            ...
            $order['order_state'] = $store_pay_type_list[$store_id] == 'offline' ? ORDER_STATE_PAY : ORDER_STATE_NEW; //订单状态
            $order['order_amount'] = $store_final_order_total[$store_id]; //订单最终实际支付金额
...
//             }
            ...
            //代金券
            if (isset($input_voucher_list[$store_id])) {
                $order_common['promotion_info'][] = array('医生代金券', sprintf('使用%s元代金券 编码：%s', $input_voucher_list[$store_id]['voucher_price'], $input_voucher_list[$store_id]['voucher_code']));
            }
