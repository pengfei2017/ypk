...
class goodsLogic
{
    ...
    {
        $lang = getTranslation('member_store_goods_index');
    }
    ...
    public function saveGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc)
    {
        ...
        if ($count >= getConfig('sg_goods_limit') && getConfig('sg_goods_limit') != 0) {
            return queue_callback(false, '发布商品数量已超过最大总商品数量，请不要继续添加');
        }

        // 验证参数
        $error = $this->_validParam($param);
        if ($error != '') {
            return queue_callback(false, $error);
        }
        ...
        if (!isset($data[$gc_id]) || isset($data[$gc_id]['child']) || isset($data[$gc_id]['childchild'])) {
            return queue_callback(false, '您选择的分类不存在，请重新选择分类。');
        }
        ...
        if (!checkPlatformStoreBindingAllGoodsClass($store_id, $bind_all_gc)) {
            ...
            if (empty($rs)) {
                ...
                if (empty($rs2)) {
                    return queue_callback(false, '您的医生没有绑定该分类，请重新选择分类。');
                }
            }
        }
        ...
        if (!$common_id) {
            return queue_callback(false, '商品添加失败');
        }
        ...
        return queue_callback(true, '', $common_id);
    }
    ...
    public function updateGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc)
    {
        ...
        if ($common_id <= 0) {
            return queue_callback(false, '商品编辑失败');
        }
        // 验证参数
        $error = $this->_validParam($param);
        if ($error != '') {
    ...
    private function _validParam($param)
    {
        ...
        $obj_validate->validateparam = array(
            array(
                "input" => $param["g_name"],
                "require" => "true",
                "message" => getLang('store_goods_index_goods_name_null')
            ),
            array(
                "input" => $param["g_price"],
                "require" => "true",
                "validator" => "Double",
                "message" => getLang('store_goods_index_goods_price_null')
            )
        );
    ...
    private function _initCommonGoodsByParam($param, $store_id, $store_name, $store_state)
    {
        ...
        $common_array['goods_state'] = ($store_state != 1) ? 0 : intval($param['g_state']);            // 医生关闭时，商品下架
        ...
        $common_array['goods_verify'] = (getConfig('goods_verify') == 1) ? 10 : 1;
    ...
    private function _addGoods($param, $common_id, $common_array)
    {
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                ...
                $goods['goods_points'] = $param['goods_points'];
    ...
    public function uploadGoodsImage($image_name, $store_id, $album_limit)
    {
        ...
        if ($album_limit > 0) {
            ...
            if ($album_count >= $album_limit) {
                return queue_callback(false, getLang('store_goods_album_climit'));
            }
        }
        ...
        if (!$result) {
            return queue_callback(false, $upload->error);
        }
        ...
        if (!getConfig('oss.open')) {
            ...
            list($width, $height, $type, $attr) = getimagesize(getConfig('oss.img_url') . '/' . ATTACH_GOODS . '/' . $store_id . DS . $img_path);
        }
        ...
        return queue_callback(true, '', $data);
    }
    ...
    public function editSaveImage($img, $common_id, $store_id, $seller_id, $seller_name)
    {
        ...
        if ($rs) {
            ...
            return queue_callback(false, '商品图片编辑失败');
        }
    }
    ...
    public function goodsShow($commonid_array, $store_id, $seller_id, $seller_name)
    {
        ...
        if ($return) {
            ...
            return queue_callback(false, '商品上架失败');
        }
    }
    ...
    public function goodsUnShow($commonid_array, $store_id, $seller_id, $seller_name)
    {
        ...
        if ($return) {
            ...
            return queue_callback(false, '商品下架失败');
        }
    }
    ...
    public function goodsDrop($commonid_array, $store_id, $seller_id, $seller_name)
    {
        ...
        if ($return) {
            ...
            return queue_callback(false, '商品删除失败');
        }
