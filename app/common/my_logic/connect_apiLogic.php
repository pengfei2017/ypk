...
class connect_apiLogic
{
    ...
    {
        $state_array = array();
        $state_array['pc_qq'] = getConfig('qq_isuse');//PC网页端的QQ互联
        $state_array['pc_sn'] = getConfig('sina_isuse');//PC网页端的新浪微博
        $state_array['connect_qq'] = getConfig('app_qq_isuse');//手机客户端的QQ互联
        $state_array['connect_sn'] = getConfig('app_sina_isuse');//手机客户端的新浪微博
        $state_array['connect_wx'] = getConfig('app_weixin_isuse');//手机客户端的微信登录
        $state_array['connect_wap_wx'] = getConfig('wap_weixin_isuse');//wap的微信登录
        $state_array['connect_sms_reg'] = getConfig('sms_register');//手机注册
        $state_array['connect_sms_lgn'] = getConfig('sms_login');//手机登录
        $state_array['connect_sms_psd'] = getConfig('sms_password');//手机短信找回密码
        return $state_array;
    }
    ...
    {
        $weixin_appid = getConfig('wap_weixin_appid');

        $api_url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' . $weixin_appid .
            '&redirect_uri=' . urlencode(MOBILE_SITE_URL . '/member/connect_sms/index') .
    ...
    public function sendCaptcha($phone, $log_type)
    {
        ...
        if ($state == true) {
            ...
            $log_msg = '【' . getConfig('site_name') . '】您于' . date("Y-m-d");
            switch ($log_type) {
                case '1':
     ...
     * @param string $captcha 短信校验码
     * @param string $password 密码
     * @param string $client 客户端标识
     * @param int $member_type_id 身份id
     * @param int $inviteId 邀请人id
     * @param string $user_name 客户端提交的用户名
     * @return array
     */
    public function smsRegister($phone, $captcha, $password, $client, $member_type_id, $inviteId, $user_name)
    {
        ...
        if ($state == true) {
            ...
            $member['member_passwd'] = $password; //登录密码
            $member['member_email'] = ''; //电子邮件
            $member['member_mobile'] = $phone; //手机号码
            $member['member_mobile_bind'] = 1; //手机是否绑定
            $member['member_type_id'] = $member_type_id; //用户身份
            $member['inviter_id'] = $inviteId; //邀请人id
            if (intval($member_type_id) > 1) {
                $member['member_state'] = 0; //如果注册时选择的是医务人员，则把会员状态置为“关闭”状态，等待管理员审核后打开
            }
            $result = $model_member->addMember($member); //返回的是刚注册用户的id
            if ($result) {
                //$member = $model_member->getMemberInfo(array('member_name' => $member_name));
                $member = Member::findFirst("member_name='" . $member_name . "' or member_id=" . $result);
    ...
    {
        $weixin_appid = getConfig('weixin_appid');
        $weixin_secret = getConfig('weixin_secret');
        if ($api_type == 'api') {
            $weixin_appid = getConfig('app_weixin_appid');
            $weixin_secret = getConfig('app_weixin_secret');
        }
        if ($api_type == 'h5_web') {
            $weixin_appid = getConfig('wap_weixin_appid');
            $weixin_secret = getConfig('wap_weixin_secret');
        }
    ...
    {
        $client_id = getConfig('sina_wb_akey');
        $client_secret = getConfig('sina_wb_skey');
    ...
    public function getSinaUserInfo($code, $client = '', $token = array())
    {
        ...
        if ($client == 'ios' || $client == 'android') {
            $client_id = getConfig('app_sina_akey');
            $client_secret = getConfig('app_sina_skey');
        }
    ...
    {
        $client_id = getConfig('qq_appid');
        $client_secret = getConfig('qq_appkey');
    ...
    public function getQqUserInfo($code, $client = '', $token = '')
    {
        ...
        $client_secret = getConfig('qq_appkey');
        if ($client == 'ios' || $client == 'android') {
            $client_id = getConfig('app_qq_akey');
            $client_secret = getConfig('app_qq_skey');
        }
