...
class orderLogic
{
    ...
    public function changeOrderStateCancel($order_info, $role, $user = '', $msg = '', $if_update_account = true, $cancel_condition = array())
    {
        ...
        try {
            ...
            return queue_callback(true, '操作成功');

        } catch (\Exception $e) {
            $model_order->rollback();
            return queue_callback(false, '操作失败');
        }
    }
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '')
    {
        try {
            ...
            $update = $model_order->editOrder($update_order, array('order_id' => $order_id));//更新订单的状态
            if (!$update) {
                throw new \Exception('保存失败');
            }
            ...
            //记录到错误日志
            Log::record('========== WEB ==========' . PHP_EOL . $e->getMessage() . PHP_EOL . $e->getTraceAsString());
            return queue_callback(false, '操作失败');
        }
    }
    ...
    public function changeOrderSend($order_info, $role, $user = '', $post = array())
    {
        ...
        $param['param'] = array(
            ...
            'order_url' => getUrl('shop/member_order/show_order', array('order_id' => $order_id))
        );
    ...
    public function changeOrderReceivePay($order_list, $role, $user = '', $post = array())
    {
        ...
        foreach ($order_list as $order_info) {
            ...
            $param['param'] = array(
                'order_sn' => $order_info['order_sn'],
                'order_url' => getUrl('shop/member_order/show_order', array('order_id' => $order_info['order_id']))
            );
            QueueClient::push('sendMemberMsg', $param);
    ...
    public function getMemberOrderInfo($order_id, $member_id)
    {
        ...
        if ($order_id <= 0) {
            return queue_callback(false, '订单不存在');
        }
        ...
        return queue_callback(true, '', array('order_info' => $order_info, 'refund_all' => $refund_all, 'daddress_info' => $daddress_info));
    }
}