...
class Security
{
    ...
    {
        $token = encrypt(strval(TIMESTAMP), md5(MD5_KEY));
        echo "<input type='hidden' name='formhash' value='" . $token . "' />";
    }

    public static function getTokenValue()
    {
        return encrypt(strval(TIMESTAMP), md5(MD5_KEY));
    }
    ...
    public static function checkToken()
    {
        ...
        return $data && (TIMESTAMP - intval($data) < 5400);
    }
     ...
     * @return mixed 数组形式的返回结果
     */
    public static function getAddslashesForInput($array, $ignore = array())
    {
        if (!function_exists('htmlawed')) require(APP_PATH . '/common/library/htmLawed.php');
        if (!empty($array)) {
