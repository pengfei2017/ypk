...
class Sms
{
     ...
     * @param string $product_id 助通信息科技的默认产品id=676767
     * @return bool|string
     */
    public function send($mobile, $content, $product_id = '676767')
    {
        $hao_sms_type = getConfig('hao_sms_type');
        if ($hao_sms_type == 1) {
        ...
        if ($hao_sms_type == 3) {
            return $this->mysend_zhutong($mobile, $content, $product_id);
        }
    }
     ...
     *
     * @param string|array $mobile 如果数组的话，支持短信群发
     * @param string $content
     * @return bool
     */
    private function mysend_smsbao($mobile, $content)
    {
        $user_id = urlencode(getConfig('hao_sms_zh')); // 这里填写用户名 hao_sms_zhang_hao
        $pass = urlencode(getConfig('hao_sms_pw')); // 这里填登陆密码
    ...
    private function mysend_yunpian($mobile, $content)
    {
        ...
        if (!empty($plugin)) {
            define('PLUGIN_ROOT', BASE_API_PATH . '/smsapi');
     ...
     * @return bool|string
     */
    private function mysend_zhutong($mobile, $content, $product_id)
    {
        $zhutong = 'zhutong';
        $plugin = str_replace('\\', '', str_replace('/', '', str_replace('.', '', $zhutong)));
        if (!empty($plugin)) {
            define('PLUGIN_ROOT', BASE_API_PATH . '/smsapi');
            require_once(PLUGIN_ROOT . DS . $plugin . DS . 'sendAPI.php');
            $username = urlencode(getConfig('hao_sms_zh')); // 这里填写用户名 hao_sms_zhang_hao
            $password = urlencode(getConfig('hao_sms_pw')); // 这里填原密码
            if (!$mobile || !$content || !$username || !$password) return false;
            if (is_array($mobile)) $mobile = implode(",", $mobile);
            ...
            $content = $content . '【' . getConfig('hao_sms_signature') . '】';//短信内容加上短信签名
            $res = $sendAPI->sendSMS($content, $mobile);
            if (strpos($res, '1,') === 0) {
                return true;
            } else {
                Log::record('助通科技短信发送失败：失败原因：' . $res, '发送数据为：' . json_encode($data));
                return false;
            }
        } else {
            return false;
    ...
    {
        set_time_limit(0);
        define('SCRIPT_ROOT', BASE_API_PATH . '/emay/');
        require_once SCRIPT_ROOT . 'include/Client.php';
        /**
         * 网关地址
         */
        $gwUrl = getConfig('sms.gwUrl');
        /**
         * 序列号,请通过亿美销售人员获取
         */
        $serialNumber = getConfig('sms.serialNumber');
        /**
         * 密码,请通过亿美销售人员获取
         */
        $password = getConfig('sms.password');
        /**
         * 登录后所持有的SESSION KEY，即可通过login方法时创建
         */
        $sessionKey = getConfig('sms.sessionKey');
        /**
         * 连接超时时间，单位为秒
         */
        $connectTimeOut = 2;
