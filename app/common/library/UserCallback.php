...
use Ypk\Logic\ContractLogic;
use Ypk\Logic\SettingLogic;
use Ypk\Models\Navigation;
...
class UserCallback extends Model
{
    ...
    {
        $logic_setting = new SettingLogic();
        $array = $logic_setting->getListSetting();
        return $array;
    }
    ...
    private function _nav()
    {
        ...
        $list = Navigation::find(array('conditions' => '', 'order' => 'nav_sort'));
        if (count($list) > 0) {
            return $list->toArray();
        } else {
            return array();
    ...
    private function _admin_menu()
    {
        ...
        $_menu = array();
        foreach ($GLOBALS['admin_module_menus'] as $admin_module_menu) {
            if (file_exists(APP_PATH . DS . $admin_module_menu . '/include/menu.php')) {
                require(APP_PATH . DS . $admin_module_menu . '/include/menu.php');
            }
        }
        return $_menu;
    }
