...
class UploadFile
{
    ...
    {
        $this->config['thumb_type'] = getConfig('thumb.cut_type');
        //加载语言包
    ...
    public function upfile($field)
    {
        ...
        if ($this->upload_file['tmp_name'] == "") {
            $this->setError($this->translation->_('cant_find_temporary_files'));
            return false;
        }
        ...
        //验证是否是合法的上传文件
        if (!$this->upload_file['isUploadBase64File'] && !is_uploaded_file($this->upload_file['tmp_name'])) {
            $this->setError($this->translation->_('upload_file_attack'));
            return false;
        }

        //验证文件大小
        if ($this->upload_file['size'] == 0) {
            $error = $this->translation->_('upload_file_size_none');
            $this->setError($error);
            return false;
        }
        if ($this->upload_file['size'] > $this->max_size * 1024) {
            $error = $this->translation->_('upload_file_size_cant_over') . $this->max_size . 'KB';
            $this->setError($error);
            return false;
        }
        ...
        if (!in_array($this->ext, $this->allow_type)) {
            $error = $this->translation->_('image_allow_ext_is') . implode(',', $this->allow_type);
            $this->setError($error);
            return false;
        }
        ...
        if (!$image_info = @getimagesize($this->upload_file['tmp_name'])) {
            $error = $this->translation->_('upload_image_is_not_image');
            $this->setError($error);
            return false;
        }
...
//				}
        ...
        $upload_res = false;
        if (!$this->upload_file['isUploadBase64File']) {
            $upload_res = @move_uploaded_file($this->upload_file['tmp_name'], BASE_UPLOAD_PATH . DS . $this->save_path . DS . $this->file_name);
        } else {
            $upload_res = @copy($this->upload_file['tmp_name'], BASE_UPLOAD_PATH . DS . $this->save_path . DS . $this->file_name);
        }
        if ($upload_res) {
            //产生缩略图
            if ($ifresize) {
            ...
            $this->setError($this->translation->_('upload_file_fail'));
            return false;
        }
//      $this->setErrorFileName($this->upload_file['tmp_name']);
     ...
     * 裁剪指定图片
     *
     * @param string $pic_path 要裁剪的图片的路径
     */
...
//				$resizeImage->newImg($save_path.DS.$this->file_name,$thumb_width[$i],$thumb_height[$i],$scale[$i],$thumb_ext[$i].'.',$save_path,$this->filling);
    ...
    private function fileInputError()
    {
        switch ($this->upload_file['error']) {
            ...
            case 1:
                ...
                $this->setError($this->translation->_('upload_file_size_over'));
                return false;
                break;
            ...
            case 3:
                //文件只有部分被上传
                $this->setError($this->translation->_('upload_file_is_not_complete'));
                return false;
                break;

            case 4:
                //没有文件被上传
                $this->setError($this->translation->_('upload_file_is_not_uploaded'));
                return false;
                break;

            case 6:
                //找不到临时文件夹
                $this->setError($this->translation->_('upload_dir_chmod'));
                return false;
                break;

            case 7:
                //文件写入失败
                $this->setError($this->translation->_('upload_file_write_fail'));
                return false;
                break;
     ...
     * 设置存储路径
     *
     * @return mixed 字符串形式的返回结果
     */
    public function setPath()
    {
        $this->default_dir = str_replace('\\', '/', $this->default_dir);
        ...
        if (!is_writable(BASE_UPLOAD_PATH . DS . $this->default_dir)) {
            $this->setError($this->translation->_('upload_file_dir') . $this->default_dir . $this->translation->_('upload_file_dir_cant_touch_file'));
            return false;
        }
    ...
    {
        $subpath = '';
        switch (getConfig('image_dir_type')) {
            case "1":
                //按文件类型存放,例如/a.jpg
                $subpath = "";
                break;
