...
class Page
{
    ...
    {
        $lang = getTranslation();//,core_lang_index

        $this->pre_home = $lang->_('first_page');
        $this->pre_last = $lang->_('last_page');
        $this->pre_page = $lang->_('pre_page');
        $this->next_page = $lang->_('next_page');
        /**
         * 设置当前页码
         */
        ...
        switch ($this->style) {
            ...
            case '4':
                ...
                if ($this->getNowPage() <= 1) {
                    ...
                    $html_page .= '<li><a class="demo" href="' . $this->setShopPseudoStaticPageUrl2($this->getNowPage() - 1) . '">' . $this->left_inside_a_html . $this->pre_page . $this->right_inside_a_html . '</a></li>';
                }
                if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0) {
                    $html_page .= '<li>' . $this->left_inside_a_html . $this->next_page . $this->right_inside_a_html . '</li>';
                } else {
                    $html_page .= '<li><a class="demo" href="' . $this->setShopPseudoStaticPageUrl2($this->getNowPage() + 1) . '">' . $this->left_inside_a_html . $this->next_page . $this->right_inside_a_html . '</a></li>';
                }
                $html_page .= '</ul>';
                break;
            case '5':
                ...
                if ($this->getNowPage() <= 1) {
                    ...
                    $html_page .= '<li><a class="demo" href="' . $this->setShopPseudoStaticPageUrl2('1') . '">' . $this->left_inside_a_html . $this->pre_home . $this->right_inside_a_html . '</a></li>';
                    $html_page .= '<li><a class="demo" href="' . $this->setShopPseudoStaticPageUrl2($this->getNowPage() - 1) . '">' . $this->left_inside_a_html . $this->pre_page . $this->right_inside_a_html . '</a></li>';
                }
                $html_page .= $this->getNowBar();
                if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0) {
                    ...
                    $html_page .= '<li><a class="demo" href="' . $this->setShopPseudoStaticPageUrl2($this->getNowPage() + 1) . '">' . $this->left_inside_a_html . $this->next_page . $this->right_inside_a_html . '</a></li>';
                    $html_page .= '<li><a class="demo" href="' . $this->setShopPseudoStaticPageUrl2($this->getTotalPage()) . '">' . $this->left_inside_a_html . $this->pre_last . $this->right_inside_a_html . '</a></li>';
                }
                $html_page .= '</ul>';
                break;
            default:
    ...
    private function setPageHtml($page_name, $page)
    {
        ...
        if ($this->getNowPage() == $page) {
            ...
            if (in_array($this->style, array(4, 5))) {     // 商城伪静态使用
                $result = $this->left_html . "<a class='demo' href='" . $this->setShopPseudoStaticPageUrl2($page) . "'>" . $this->left_inside_a_html . $page_name . $this->right_inside_a_html . "</a>" . $this->right_html;
            } else {                                      // 普通分页使用
    ...
    {
        $uri = getUrlParamArray();
        $url = $uri['module'] . '/' . $uri['controller'] . '/' . $uri['action'];

        //地址存在页面参数
        $getArray = $_GET;
        foreach ($getArray as $key => $val) {
            if ($key == '_url' || $key == $this->page_name) {
                unset($getArray[$key]);
            }
        }

        $url = getUrl($url, $getArray);
        if (strpos($url, '?') === false) {
            $url .= "?" . $this->page_name . "=";
        } else {
            $url .= "&" . $this->page_name . "=";
    ...
    private function setShopPseudoStaticPageUrl2($page)
    {
        $urlParamArray = getUrlParamArray();
        $module = $urlParamArray['module'];
        unset($urlParamArray['module']);
        $controller = $urlParamArray['controller'];
        unset($urlParamArray['controller']);
        $action = $urlParamArray['action'];
        unset($urlParamArray['action']);
        $urlParamArray[$this->page_name] = $page;
        return getUrl('shop/' . $controller . '/' . $action, $urlParamArray);
    }
}